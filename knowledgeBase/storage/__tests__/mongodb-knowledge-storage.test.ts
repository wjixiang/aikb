...
describe('MongoDB Knowledge Storage', () => {
  ...
  describe('Knowledge Content Storage', () => {
    test('should create new knowledge content', async () => {
      ...
      const result = await knowledgeContentStorage.create_new_knowledge_content(knowledgeData);
    ...
    test('should get knowledge content by ID', async () => {
      ...
      const created = await knowledgeContentStorage.create_new_knowledge_content(knowledgeData);
      const retrieved = await knowledgeContentStorage.get_knowledge_content_by_id(created.id);
    ...
    test('should update knowledge content', async () => {
      ...
      const updated = await knowledgeContentStorage.update_knowledge_content(created.id, updatedData);
    ...
    test('should delete knowledge content', async () => {
      ...
      const created = await knowledgeContentStorage.create_new_knowledge_content(knowledgeData);
      const deleteResult = await knowledgeContentStorage.delete_knowledge_content_by_id(created.id);
      
      expect(deleteResult).toBe(true);
      
      // Verify it's deleted
      await expect(knowledgeContentStorage.get_knowledge_content_by_id(created.id)).rejects.toThrow();
    });
    ...
    test('should search knowledge contents', async () => {
      ...
      await knowledgeContentStorage.create_new_knowledge_content(knowledgeData1);
      await knowledgeContentStorage.create_new_knowledge_content(knowledgeData2);

      const searchResults = await knowledgeContentStorage.search_knowledge_contents('search');
  ...
  describe('Knowledge Vector Storage', () => {
    const testVector = Array(1536).fill(0).map((_, i) => Math.random());

    test('should store knowledge vector', async () => {
      const knowledgeId = 'test_knowledge_vector_1';
      
      await expect(
        knowledgeVectorStorage.store_knowledge_vector(knowledgeId, testVector, { test: true })
    ...
    test('should get knowledge vector', async () => {
      ...
      await knowledgeVectorStorage.store_knowledge_vector(knowledgeId, testVector, metadata);
      const result = await knowledgeVectorStorage.get_knowledge_vector(knowledgeId);
    ...
    test('should update knowledge vector', async () => {
      ...
      const newVector = Array(1536).fill(0).map((_, i) => Math.random());
      const newMetadata = { test: 'updated' };
      
      await knowledgeVectorStorage.store_knowledge_vector(knowledgeId, testVector);
      await knowledgeVectorStorage.update_knowledge_vector(knowledgeId, newVector, newMetadata);
      const result = await knowledgeVectorStorage.get_knowledge_vector(knowledgeId);
    ...
    test('should delete knowledge vector', async () => {
      ...
      await knowledgeVectorStorage.store_knowledge_vector(knowledgeId, testVector);
      const deleteResult = await knowledgeVectorStorage.delete_knowledge_vector(knowledgeId);
    ...
    test('should batch store knowledge vectors', async () => {
      const vectors = [
        {
          ...
          vector: Array(1536).fill(0).map((_, i) => Math.random()),
      ...
      await expect(knowledgeVectorStorage.batch_store_knowledge_vectors(vectors)).resolves.not.toThrow();
      
      const result1 = await knowledgeVectorStorage.get_knowledge_vector('batch_test_1');
      const result2 = await knowledgeVectorStorage.get_knowledge_vector('batch_test_2');
  ...
  describe('Knowledge Graph Storage', () => {
    test('should create knowledge link', async () => {
      ...
      await expect(knowledgeGraphStorage.create_new_link(sourceId, targetId)).resolves.not.toThrow();
    });
  });
  ...
  describe('Knowledge Storage Integration', () => {
    test('should create and retrieve knowledge instance', async () => {
      ...
      const created = await knowledgeStorage.create_new_knowledge(knowledgeData, sourceId);
      const knowledge = await knowledgeStorage.get_knowledge_by_id(created.id);
      
      expect(knowledge).not.toBeNull();
      expect(knowledge?.get_id()).toBe(created.id);
      expect(knowledge?.render_to_markdown_string()).toContain('Integration Test Knowledge');
    });
    ...
    test('should handle knowledge with children', async () => {
      ...
      const createdParent = await knowledgeStorage.create_new_knowledge(parentData, sourceId);
      
      // Create child knowledge
      const createdChild = await knowledgeStorage.create_new_knowledge(childData, createdParent.id);
      ...
      const parentKnowledge = await knowledgeStorage.get_knowledge_by_id(createdParent.id);
