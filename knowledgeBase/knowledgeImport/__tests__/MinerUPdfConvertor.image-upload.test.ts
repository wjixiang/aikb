...
describe('MinerUPdfConvertor Image Upload', () => {
  ...
  it('should include uploadedImages in ConversionResult when images are found', async () => {
    ...
    mockUploadToS3.mockResolvedValue('https://test-bucket.s3.amazonaws.com/images/test-image.jpg');
    ...
    const result = await converter.convertPdfToMarkdownFromS3('https://test-s3-url.com/file.pdf');
  ...
  it('should update markdown content with S3 URLs', async () => {
    ...
    expect(result.success).toBe(true);
    expect(result.data).toContain('https://test-bucket.s3.amazonaws.com/images/test-image.jpg');
