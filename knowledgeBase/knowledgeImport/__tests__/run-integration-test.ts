...
async function runIntegrationTest() {
    ...
    if (!isTokenValid) {
        console.error('❌ Token validation failed. Please check your token and try again.');
        console.log('');
        ...
        if (result.success) {
            ...
            if (result.data) {
                ...
                if (typeof result.data === 'object') {
                    ...
                    console.log('   Content:', JSON.stringify(result.data).substring(0, 200) + '...');
                }
            }
            ...
            if (result.taskId) {
                ...
                if (status.extract_progress) {
                    console.log('   Progress:', `${status.extract_progress.extracted_pages}/${status.extract_progress.total_pages} pages`);
                }
            }
