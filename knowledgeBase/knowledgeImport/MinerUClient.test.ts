...
describe(MinerUClient, () => {
    ...
    beforeEach(() => {
        ...
        testDownloadDir = path.join(__dirname, 'test-downloads', Date.now().toString());
        ...
        mockWaitForTaskCompletion = vi.spyOn(client as any, 'waitForTaskCompletion');
    ...
    describe('processSingleFile', () => {
        it('should return full path of downloaded .zip', async () => {
            ...
            expect(mockWaitForTaskCompletion).toHaveBeenCalledWith(testTaskId, testOptions);
        ...
        it('should use default download directory when none provided', async () => {
            ...
            const expectedZipPath = path.join(defaultDownloadDir, `${testTaskId}.zip`);
            ...
            // Assert
            expect(mockWaitForTaskCompletion).toHaveBeenCalledWith(testTaskId, undefined);
