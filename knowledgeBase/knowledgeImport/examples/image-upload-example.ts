      ...
      if (result.uploadedImages && result.uploadedImages.length > 0) {
        ...
        if (result.data) {
          ...
          const imageLines = lines.filter(line => 
            line.includes('http') && 
            (line.includes('.jpg') || line.includes('.png') || line.includes('.gif') || line.includes('.svg'))
          );
          ...
          if (imageLines.length > 0) {
            ...
            if (imageLines.length > 3) {
              console.log(`... and ${imageLines.length - 3} more image references`);
            }
          } else {
...
function explainImageUploadProcess() {
  ...
  console.log('• Images stored as: images/documents/example/report.pdf/image1.jpg');
  ...
  console.log('• images/filename.jpg → https://bucket.s3.amazonaws.com/images/documents/example/report.pdf/filename.jpg');
  console.log('• ./images/filename.jpg → https://bucket.s3.amazonaws.com/images/documents/example/report.pdf/filename.jpg');
  console.log('• /absolute/path/images/filename.jpg → https://bucket.s3.amazonaws.com/images/documents/example/report.pdf/filename.jpg');
}
...
export function processConversionResult(result: ConversionResult) {
  ...
  if (result.data) {
    ...
    console.log(`Markdown content ready for storage (${result.data.length} chars)`);
  }
  
  // Save image information
  if (result.uploadedImages && result.uploadedImages.length > 0) {
    console.log(`Image URLs ready for reference: ${result.uploadedImages.length} images`);
