  ...
  .action(async (options) => {
    try {
      ...
      if (options.interactive) {
        ...
        const creationMethod = await selectFromOptions('选择知识创建方式:', [
          '手动输入知识',
          '从自然语言创建',
        ]);
        ...
        if (options.natural_language) {
          ...
          if (!options.scope || !options.content) {
            displayError(
              '在非交互模式下，必须提供 --scope 和 --content 选项，或使用 --natural-language 选项',
            );
            process.exit(1);
          }
      ...
      const entity =
        await storage.entityStorage.entityContentStorage.get_entity_by_id(
          entityId,
        );
      ...
      if (useNaturalLanguage) {
        ...
        const knowledge =
          await entityInstance.create_subordinate_knowledge_from_text(
            naturalLanguageText,
        ...
        console.log(
          `  内容: ${knowledge.getData().content.substring(0, 100)}${knowledge.getData().content.length > 100 ? '...' : ''}`,
        );
      } else {
        ...
        const savedKnowledge = await tempKnowledge.save(
          storage.knowledgeStorage,
        );
        ...
        console.log(
          `  内容: ${knowledgeData.content.substring(0, 100)}${knowledgeData.content.length > 100 ? '...' : ''}`,
        );
      }
    } catch (error: any) {
