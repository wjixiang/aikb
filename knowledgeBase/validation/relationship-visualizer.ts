...
export class RelationshipVisualizer {
  ...
  async generateDotGraph(entityId: string): Promise<string> {
    ...
    try {
      const entity = await this.entityStorage.entityContentStorage.get_entity_by_id(entityId);
      if (!entity) {
      ...
      dotLines.push(`  "${entityId}" [label="${entityLabel}", fillcolor=lightblue];`);
      ...
      for (const knowledge of subordinateKnowledge) {
        ...
        const knowledgeLabel = this.escapeLabel(`${knowledge.getData().scope}\\n(知识)`);
        dotLines.push(`  "${knowledgeId}" [label="${knowledgeLabel}", fillcolor=lightgreen];`);
  ...
  private async addKnowledgeToDot(
    ...
    for (const child of children) {
      ...
      dotLines.push(`  "${childId}" [label="${childLabel}", fillcolor=${color}];`);
      dotLines.push(`  "${knowledge.get_id()}" -> "${childId}" [label="包含"];`);
  ...
  async generateHtmlVisualization(entityId: string): Promise<string> {
    ...
    try {
      ...
      htmlLines.push('    .entity { background-color: #e3f2fd; padding: 15px; margin: 10px 0; border-radius: 8px; border-left: 5px solid #2196f3; }');
      htmlLines.push('    .knowledge { background-color: #e8f5e8; padding: 10px; margin: 5px 0 5px 20px; border-radius: 5px; border-left: 3px solid #4caf50; }');
      htmlLines.push('    .child-knowledge { background-color: #fff9c4; padding: 8px; margin: 3px 0 3px 40px; border-radius: 3px; border-left: 2px solid #ffc107; }');
      htmlLines.push('    .relationship { color: #666; font-style: italic; margin: 2px 0; }');
      htmlLines.push('    .stats { background-color: #f5f5f5; padding: 10px; margin: 20px 0; border-radius: 5px; }');
      htmlLines.push('  </style>');
      htmlLines.push('</head>');
      htmlLines.push('<body>');
  ...
  private async addKnowledgeToHtml(
    ...
    htmlLines.push(`    <p class="relationship">知识ID: ${knowledge.get_id()}</p>`);
    
    if (data.childKnowledgeId.length > 0) {
      htmlLines.push(`    <p class="relationship">包含 ${data.childKnowledgeId.length} 个子知识</p>`);
    }
  ...
  async generateRelationshipStats(entityId: string): Promise<any> {
    ...
    try {
      ...
      // 分析知识层次结构
      for (const knowledge of subordinateKnowledge) {
        const knowledgeStats = await this.analyzeKnowledgeHierarchy(knowledge, 1);
        stats.knowledge.items.push(knowledgeStats);
        stats.hierarchy.maxDepth = Math.max(stats.hierarchy.maxDepth, knowledgeStats.maxDepth);
  ...
  private async analyzeKnowledgeHierarchy(
    ...
    for (const child of children) {
      const childStat = await this.analyzeKnowledgeHierarchy(child, currentDepth + 1);
   ...
   * 获取实体的所有下属知识（递归）
   */
  private async getSubordinateKnowledge(entityId: string): Promise<Knowledge[]> {
    const links = await this.knowledgeStorage.knowledgeGraphStorage.get_knowledge_links_by_source(entityId);
    const knowledgeItems: Knowledge[] = [];
    
    for (const link of links) {
      const knowledge = await this.knowledgeStorage.get_knowledge_by_id(link.targetId);
      if (knowledge) {
  ...
  async generateMermaidDiagram(entityId: string): Promise<string> {
    ...
    try {
      ...
      for (let i = 0; i < subordinateKnowledge.length; i++) {
        ...
        mermaidLines.push(`    ${knowledgeId}["${knowledge.getData().scope}<br/>知识"]`);
        mermaidLines.push(`    E --> ${knowledgeId}`);
        
        // 添加子知识
        await this.addKnowledgeToMermaid(mermaidLines, knowledge, knowledgeId, 1);
      }
      
      // 添加样式
      mermaidLines.push('    classDef entity fill:#e3f2fd,stroke:#2196f3,stroke-width:2px');
      mermaidLines.push('    classDef knowledge fill:#e8f5e8,stroke:#4caf50,stroke-width:2px');
      mermaidLines.push('    classDef childKnowledge fill:#fff9c4,stroke:#ffc107,stroke-width:2px');
      mermaidLines.push('    class E entity');
      
      const mermaidDiagram = mermaidLines.join('\n');
      this.logger.info(`Mermaid图表生成成功，大小: ${mermaidDiagram.length} 字符`);
  ...
  private async addKnowledgeToMermaid(
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      // 递归处理子知识
      if (child.getChildren().length > 0) {
        await this.addKnowledgeToMermaid(mermaidLines, child, childId, level + 1);
      }
    }
  }
    ...
    try {
      ...
      switch (format) {
        ...
        case 'mermaid':
          ...
          break;
        case 'json':
          content = JSON.stringify(await this.generateRelationshipStats(entityId), null, 2);
          break;
        default:
