...
describe('Entity Knowledge Relationship Validation', () => {
  ...
  describe('Complete Entity Knowledge Creation', () => {
    it('should validate complete entity-knowledge creation flow', async () => {
      ...
      try {
        ...
        // 2. 创建临床表现知识
        const clinicalManifestationsKnowledge = await createClinicalManifestationsKnowledge(nephroticSyndromeEntity);
        
        // 3. 创建症状实体并建立关系
        const symptomEntities = await createSymptomEntities();
        await linkSymptomsToKnowledge(clinicalManifestationsKnowledge, symptomEntities);
        
        // 4. 验证关系结构
        const isStructureValid = await validateRelationshipStructure(nephroticSyndromeEntity);
        
        // 5. 验证双向查询
        const isBidirectionalValid = await validateBidirectionalQueries(nephroticSyndromeEntity);
  ...
  describe('Knowledge Hierarchy Validation', () => {
    it('should validate knowledge hierarchy structure', async () => {
      ...
      try {
        ...
        const testEntity = await Entity.create_entity_with_entity_data(testEntityData).save(entityStorage);
  ...
  async function createSymptomEntities(): Promise<Entity[]> {
    ...
    for (const symptomData of symptoms) {
      const entity = await Entity.create_entity_with_entity_data(symptomData).save(entityStorage);
      symptomEntities.push(entity);
      logger.info(`创建症状实体成功: ${symptomData.name[0]}，ID: ${entity.get_id()}`);
    }
    
    return symptomEntities;
  }

  async function linkSymptomsToKnowledge(clinicalKnowledge: any, symptomEntities: Entity[]) {
    // 为每个症状创建相关知识并链接
    for (const symptomEntity of symptomEntities) {
      const symptomName = symptomEntity.get_definition().split('是指')[0];
      const symptomKnowledge = await knowledgeWorkflow.create_simple_knowledge_from_text(
        `${symptomName}是肾病综合征的重要临床表现之一。`,
        symptomEntity,
        '与肾病综合征的关系',
      );
      
      logger.info(`创建症状知识并链接成功: ${symptomName}，知识ID: ${symptomKnowledge.get_id()}`);
    }
  }

  async function validateRelationshipStructure(mainEntity: Entity): Promise<boolean> {
    try {
      // 获取主实体的所有下属知识
      const subordinateKnowledge = await mainEntity.get_subordinate_knowledge(knowledgeStorage);
  ...
  async function validateBidirectionalQueries(mainEntity: Entity): Promise<boolean> {
    try {
      // 1. 从实体查询知识
      const entityToKnowledge = await mainEntity.get_subordinate_knowledge(knowledgeStorage);
      ...
      for (const knowledge of entityToKnowledge) {
        const links = await knowledgeStorage.knowledgeGraphStorage.get_knowledge_links_by_source(
          mainEntity.get_id()
        );
        
        for (const link of links) {
          // 验证链接的目标确实是知识
          const targetKnowledge = await knowledgeStorage.get_knowledge_by_id(link.targetId);
          if (targetKnowledge) {
