...
export class ValidationRunner {
  ...
  async runAllValidations(): Promise<{
    ...
    try {
      ...
      const entityKnowledgeRelationshipResults = await runEntityKnowledgeRelationshipValidation();
      ...
      this.logger.info('所有验证测试完成', { overall, duration: Date.now() - startTime });
  ...
  private async runVisualizationTests(): Promise<{
    ...
    try {
      ...
      const visualizer = new RelationshipVisualizer(knowledgeStorage, entityStorage);
      ...
      const overall = htmlGeneration && dotGeneration && mermaidGeneration && statsGeneration;
   ...
   * 测试可视化格式生成
   */
  private async testVisualizationFormat(generator: () => Promise<any>): Promise<boolean> {
    try {
  ...
  private calculateOverallResult(
    ...
    return entityKnowledgeRelationship.overall && 
  ...
  private generateSummary(
    ...
    const totalTests = this.countTotalTests(entityKnowledgeRelationship) +
                      this.countTotalTests(complexScenario) +
                      this.countTotalTests(visualization);
    
    const passedTests = this.countPassedTests(entityKnowledgeRelationship) +
    ...
    return {
      ...
      categories: {
        entityKnowledgeRelationship: {
          ...
          failed: this.countTotalTests(entityKnowledgeRelationship) - this.countPassedTests(entityKnowledgeRelationship),
        },
        complexScenario: {
          total: this.countTotalTests(complexScenario),
          passed: this.countPassedTests(complexScenario),
          failed: this.countTotalTests(complexScenario) - this.countPassedTests(complexScenario),
        },
        visualization: {
          total: this.countTotalTests(visualization),
          passed: this.countPassedTests(visualization),
          failed: this.countTotalTests(visualization) - this.countPassedTests(visualization),
        },
      },
    };
  }
  ...
  private countTotalTests(results: any): number {
    ...
    for (const key in results) {
      if (typeof results[key] === 'boolean') {
        count++;
      } else if (typeof results[key] === 'object' && !Array.isArray(results[key])) {
  ...
  async generateDetailedReport(results: any): Promise<string> {
    ...
    reportLines.push(`- 实体-知识创建流程: ${ekResults.entityKnowledgeCreation ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push(`- 知识层次结构: ${ekResults.knowledgeHierarchy ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push(`- 向量相似性搜索: ${ekResults.vectorSimilaritySearch ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push('');
    ...
    reportLines.push(`- 多实体关联: ${csResults.multiEntity ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push(`- 深度层次结构: ${csResults.deepHierarchy ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push(`- 复杂查询: ${csResults.complexQuery ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push(`- 并发操作: ${csResults.concurrentOperation ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push(`- 大数据量: ${csResults.bigData ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push('');
    ...
    reportLines.push(`- HTML生成: ${vizResults.htmlGeneration ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push(`- DOT图生成: ${vizResults.dotGeneration ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push(`- Mermaid图生成: ${vizResults.mermaidGeneration ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push(`- 统计信息生成: ${vizResults.statsGeneration ? '✅ 通过' : '❌ 失败'}`);
    reportLines.push('');
   ...
   * 保存验证报告到文件
   */
  async saveReportToFile(results: any, outputPath: string = './validation-report.md'): Promise<void> {
    try {
