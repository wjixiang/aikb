...
describe('Complex Scenario Tests', () => {
  ...
  describe('Multi-Entity Scenario', () => {
    it('should test multi-entity relationship scenario', async () => {
      ...
      try {
        ...
        // 2. 创建实体间的关系
        await createEntityRelationship(diseaseEntity, complicationEntity, '可能导致');
        ...
        const complicationKnowledge = await createKnowledgeForEntity(complicationEntity, {
          scope: '临床表现',
        ...
        // 5. 验证关系结构
        const diseaseRelations = await getEntityRelations(diseaseEntity.get_id());
        const complicationRelations = await getEntityRelations(complicationEntity.get_id());
  ...
  describe('Deep Hierarchy Scenario', () => {
    it('should test deep hierarchy scenario', async () => {
      ...
      try {
        ...
        // 4. 生成可视化
        const mermaidDiagram = await visualizer.generateMermaidDiagram(rootEntity.get_id());
        const stats = await visualizer.generateRelationshipStats(rootEntity.get_id());
    ...
    it('should test complex query scenario', async () => {
      ...
      try {
        ...
        if (entities.length < 2) {
          debugLog('实体数量不足，无法进行复杂查询测试', { entityCount: entities.length });
          expect(false).toBe(true);
          return;
        }
        ...
        // 2. 测试多跳查询
        debugLog('开始测试多跳查询');
        const multiHopResults = await testMultiHopQuery(entities[0], entities[entities.length - 1]);
        debugLog('多跳查询测试完成', { multiHopResults });
        
        // 3. 测试路径查找
        debugLog('开始测试路径查找');
        const pathResults = await testPathFinding(entities[0], entities[entities.length - 1]);
  ...
  describe('Concurrent Operation Scenario', () => {
    it('should test concurrent operation scenario', async () => {
      ...
      try {
        ...
        // 4. 验证实体-知识关系
        const subordinateKnowledge = await baseEntity.get_subordinate_knowledge(knowledgeStorage);
  ...
  async function createEntity(entityData: EntityData): Promise<Entity> {
    return await Entity.create_entity_with_entity_data(entityData).save(entityStorage);
  }

  async function createKnowledgeForEntity(entity: Entity, knowledgeData: KnowledgeData): Promise<any> {
    return await knowledgeWorkflow.create_simple_knowledge_from_text(
  ...
  async function createEntityRelationship(sourceEntity: Entity, targetEntity: Entity, relationType: string): Promise<void> {
  ...
  async function getEntityRelations(entityId: string): Promise<any[]> {
    return await entityStorage.entityGraphStorage.get_entity_relations(entityId);
  }

  async function calculateMaxDepth(knowledge: any, currentDepth: number = 1): Promise<number> {
  ...
  async function createMedicalEntityNetwork(): Promise<Entity[]> {
    ...
    // 创建实体间的关系
    await createEntityRelationship(createdEntities[0], createdEntities[1], '可能导致');
    await createEntityRelationship(createdEntities[0], createdEntities[2], '可能导致');
    await createEntityRelationship(createdEntities[1], createdEntities[2], '可能导致');
  ...
  async function testMultiHopQuery(sourceEntity: Entity, targetEntity: Entity): Promise<boolean> {
    try {
      // 获取源实体的所有相关实体
      ...
      for (const relation of sourceRelations) {
        const intermediateRelations = await getEntityRelations(relation.targetId);
        const hasPathToTarget = intermediateRelations.some(r => r.targetId === targetEntity.get_id());
  ...
  async function testPathFinding(sourceEntity: Entity, targetEntity: Entity): Promise<boolean> {
    try {
      // 简化的路径查找测试
      const sourceRelations = await getEntityRelations(sourceEntity.get_id());
      const hasDirectPath = sourceRelations.some(r => r.targetId === targetEntity.get_id());
  ...
  async function testSimilaritySearch(entity: Entity): Promise<boolean> {
    try {
      ...
      const similarEntities = await entityStorage.entityVectorStorage.find_similar_vectors(
        queryVector,
        3,
        0.5
      );
  ...
  async function testAggregationQuery(entity: Entity): Promise<boolean> {
    try {
      ...
      const totalChildren = subordinateKnowledge.reduce((sum, k) => sum + k.getChildren().length, 0);
