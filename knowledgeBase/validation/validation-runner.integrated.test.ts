  ...
  describe('Visualization Tests', () => {
    it('should test HTML visualization generation', async () => {
      ...
      try {
        ...
        // 创建测试知识
        const testKnowledge = await knowledgeWorkflow.create_simple_knowledge_from_text(
          '这是用于测试可视化功能的示例知识内容。',
          testEntity,
          '测试知识',
        );
        ...
        // 测试HTML可视化生成
        const htmlResult = await visualizer.generateHtmlVisualization(testEntity.get_id());
    ...
    it('should test DOT graph generation', async () => {
      ...
      try {
        ...
        // 测试DOT图生成
        const dotResult = await visualizer.generateDotGraph(testEntity.get_id());
    ...
    it('should test Mermaid diagram generation', async () => {
      ...
      try {
        ...
        // 测试Mermaid图生成
        const mermaidResult = await visualizer.generateMermaidDiagram(testEntity.get_id());
    ...
    it('should test relationship statistics generation', async () => {
      ...
      try {
        ...
        // 测试关系统计信息生成
        const statsResult = await visualizer.generateRelationshipStats(testEntity.get_id());
  ...
  describe('Integration Test', () => {
    it('should run all visualization tests together', async () => {
      ...
      try {
        ...
        // 并行运行所有可视化测试
        const [htmlResult, dotResult, mermaidResult, statsResult] = await Promise.all([
        ...
        const mermaidGeneration = mermaidResult !== null && mermaidResult !== undefined;
        const statsGeneration = statsResult !== null && statsResult !== undefined;
  ...
  describe('Report Generation', () => {
    it('should generate validation report', async () => {
      ...
      try {
        ...
        reportLines.push(`- 总体结果: ${testResults.overall ? '✅ 通过' : '❌ 失败'}`);
        reportLines.push('');
        
        const report = reportLines.join('\n');
        
        const isValid = report.length > 0 && report.includes('实体-知识关系验证报告');
