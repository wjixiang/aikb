...
export class ComplexScenarioTest {
  ...
  constructor() {
    ...
    const mockVectorStorage = {
      ...
      find_similar_vectors: async (vector: number[], limit?: number, threshold?: number) => {
        // 返回模拟的相似向量结果
        return [
          {
    ...
    this.knowledgeWorkflow = new KnowledgeCreationWorkflow(this.knowledgeStorage);
    this.visualizer = new RelationshipVisualizer(this.knowledgeStorage, this.entityStorage);
  }
  ...
  async testMultiEntityScenario(): Promise<boolean> {
    ...
    try {
      ...
      const diseaseEntity = await this.createEntity({
        ...
        tags: ['疾病', '代谢疾病', '慢性病'],
        definition: '糖尿病是一组以高血糖为特征的代谢性疾病，由胰岛素分泌缺陷或其生物作用受损引起。',
      });
      
      const complicationEntity = await this.createEntity({
        name: ['糖尿病肾病', 'Diabetic Nephropathy'],
        tags: ['并发症', '肾脏疾病'],
        definition: '糖尿病肾病是糖尿病最常见的微血管并发症之一，是导致终末期肾病的主要原因。',
      });
      
      const treatmentEntity = await this.createEntity({
        name: ['胰岛素治疗', 'Insulin Therapy'],
        tags: ['治疗', '药物治疗'],
        definition: '胰岛素治疗是糖尿病管理的重要手段，通过补充外源性胰岛素来控制血糖水平。',
      });
      
      // 2. 创建实体间的关系
      await this.createEntityRelationship(diseaseEntity, complicationEntity, '可能导致');
      await this.createEntityRelationship(diseaseEntity, treatmentEntity, '需要');
      
      // 3. 为每个实体创建相关知识
      const diseaseKnowledge = await this.createKnowledgeForEntity(diseaseEntity, {
        scope: '病因和发病机制',
        content: '糖尿病的发病机制复杂，主要包括胰岛素抵抗和胰岛素分泌不足两个方面。1型糖尿病主要由自身免疫导致胰岛β细胞破坏；2型糖尿病则与遗传因素和环境因素共同作用有关。',
        childKnowledgeId: [],
      });
      
      const complicationKnowledge = await this.createKnowledgeForEntity(complicationEntity, {
        scope: '临床表现',
        content: '糖尿病肾病早期无明显症状，随着病情进展可出现蛋白尿、水肿、高血压等表现。晚期可发展为肾功能衰竭。',
        childKnowledgeId: [],
      });
      ...
      const crossReferenceKnowledge = await this.createCrossReferenceKnowledge(
        ...
        {
          scope: '疾病与并发症的关系',
          content: '长期血糖控制不佳是导致糖尿病肾病的主要危险因素。良好的血糖管理可以显著降低糖尿病肾病的发生风险。',
          childKnowledgeId: [],
        }
      );
      
      // 5. 验证关系结构
      const diseaseRelations = await this.getEntityRelations(diseaseEntity.get_id());
      const complicationRelations = await this.getEntityRelations(complicationEntity.get_id());
      
      const isValid = diseaseRelations.length >= 2 && complicationRelations.length >= 1;
  ...
  async testDeepHierarchyScenario(): Promise<boolean> {
    ...
    try {
      ...
      const rootEntity = await this.createEntity({
        ...
        tags: ['疾病', '循环系统疾病'],
        definition: '心血管疾病是指影响心脏和血管的疾病，包括冠心病、高血压、心力衰竭等多种疾病。',
      });
      ...
      const rootKnowledge = await rootEntity.create_subordinate_knowledge_from_text(
        hierarchyText,
      ...
      // 4. 生成可视化
      const mermaidDiagram = await this.visualizer.generateMermaidDiagram(rootEntity.get_id());
      const stats = await this.visualizer.generateRelationshipStats(rootEntity.get_id());
  ...
  async testComplexQueryScenario(): Promise<boolean> {
    ...
    try {
      ...
      if (entities.length < 2) {
        this.debugLog('实体数量不足，无法进行复杂查询测试', { entityCount: entities.length });
        return false;
      }
      
      // 2. 测试多跳查询
      this.debugLog('开始测试多跳查询');
      const multiHopResults = await this.testMultiHopQuery(entities[0], entities[entities.length - 1]);
      this.debugLog('多跳查询测试完成', { multiHopResults });
      
      // 3. 测试路径查找
      this.debugLog('开始测试路径查找');
      const pathResults = await this.testPathFinding(entities[0], entities[entities.length - 1]);
      ...
      const isValid = multiHopResults && pathResults && similarityResults && aggregationResults;
      ...
      this.debugLog('复杂查询场景测试失败', { error: error.message, stack: error.stack });
      return false;
    }
  }
  ...
  async testConcurrentOperationScenario(): Promise<boolean> {
    ...
    try {
      ...
      for (let i = 0; i < 10; i++) {
        const promise = this.knowledgeWorkflow.create_simple_knowledge_from_text(
      ...
      // 4. 验证实体-知识关系
      const subordinateKnowledge = await baseEntity.get_subordinate_knowledge(this.knowledgeStorage);
  ...
  private async createEntity(entityData: EntityData): Promise<Entity> {
    return await Entity.create_entity_with_entity_data(entityData).save(this.entityStorage);
  }

  private async createKnowledgeForEntity(entity: Entity, knowledgeData: KnowledgeData): Promise<any> {
    return await this.knowledgeWorkflow.create_simple_knowledge_from_text(
  ...
  private async createEntityRelationship(sourceEntity: Entity, targetEntity: Entity, relationType: string): Promise<void> {
  ...
  private async createCrossReferenceKnowledge(
    ...
    // 为第一个实体创建知识
    const knowledge1 = await this.createKnowledgeForEntity(entity1, knowledgeData);
  ...
  private async getEntityRelations(entityId: string): Promise<any[]> {
    return await this.entityStorage.entityGraphStorage.get_entity_relations(entityId);
  }

  private async calculateMaxDepth(knowledge: any, currentDepth: number = 1): Promise<number> {
  ...
  private async createMedicalEntityNetwork(): Promise<Entity[]> {
    ...
    try {
      ...
      const entityDataList = [
        {
          name: ['代谢综合征'],
          tags: ['疾病', '代谢疾病'],
          definition: '代谢综合征是一组代谢紊乱的集合，包括中心性肥胖、高血压、高血糖和血脂异常。',
        },
        {
          name: ['胰岛素抵抗'],
          tags: ['病理状态', '代谢异常'],
          definition: '胰岛素抵抗是指细胞对胰岛素的反应性降低，需要更高浓度的胰岛素才能产生正常的生物学效应。',
        },
        {
          name: ['肥胖症'],
          tags: ['疾病', '代谢疾病'],
        ...
        },
        {
          name: ['高血压'],
          tags: ['疾病', '心血管疾病'],
          definition: '高血压是指动脉血压持续升高的疾病，是心血管疾病的重要危险因素。',
        },
        {
          name: ['血脂异常'],
          tags: ['疾病', '代谢疾病'],
          definition: '血脂异常是指血液中脂质水平异常，包括高胆固醇、高甘油三酯等。',
        },
      ];
      
      this.logger.info('开始创建医疗实体', { entityCount: entityDataList.length });
      ...
      for (let i = 0; i < entities.length - 1; i++) {
        ...
        await this.createEntityRelationship(entities[i], entities[i + 1], '相关');
        this.logger.info('关系创建成功');
      }
      
      this.logger.info('医疗实体网络创建完成', { totalEntities: entities.length });
      return entities;
    } catch (error) {
  ...
  private async testMultiHopQuery(sourceEntity: Entity, targetEntity: Entity): Promise<boolean> {
    try {
      // 测试多跳查询（通过图存储）
      ...
      this.debugLog('多跳查询测试失败', { error: error.message, stack: error.stack });
      return false;
    }
  }

  private async testPathFinding(sourceEntity: Entity, targetEntity: Entity): Promise<boolean> {
    try {
      // 测试路径查找
      const sourceId = sourceEntity.get_id();
      const targetId = targetEntity.get_id();
      ...
      const sourceExists = await this.entityStorage.entityContentStorage.get_entity_by_id(sourceId);
      const targetExists = await this.entityStorage.entityContentStorage.get_entity_by_id(targetId);
      
      this.debugLog('Entity existence check', {
        sourceExists: !!sourceExists,
        targetExists: !!targetExists,
        sourceData: sourceExists ? { id: sourceExists.id, name: sourceExists.name } : null,
        targetData: targetExists ? { id: targetExists.id, name: targetExists.name } : null,
      });
      
      // Check if relations exist for source entity
      const sourceRelations = await this.entityStorage.entityGraphStorage.get_entity_relations(sourceId);
      ...
      this.debugLog('路径查找测试失败', { error: error.message, stack: error.stack });
      return false;
    }
  }
  ...
  private async testSimilaritySearch(entity: Entity): Promise<boolean> {
    try {
      ...
      const similarEntities = await this.entityStorage.entityVectorStorage.find_similar_vectors(
        queryVector,
        5,
        0.5
      );
  ...
  private async testAggregationQuery(entity: Entity): Promise<boolean> {
    try {
      // 测试聚合查询
      const subordinateKnowledge = await entity.get_subordinate_knowledge(this.knowledgeStorage);
...
export async function runComplexScenarioTests() {
  ...
  console.log(`并发操作: ${results.concurrentOperation ? '✅ 通过' : '❌ 失败'}`);
