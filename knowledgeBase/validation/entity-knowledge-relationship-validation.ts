...
export class EntityKnowledgeRelationshipValidation {
  private logger = createLoggerWithPrefix('EntityKnowledgeRelationshipValidation');
  ...
  async validateCompleteEntityKnowledgeCreation(): Promise<boolean> {
    ...
    try {
      // 1. 创建主实体：肾病综合征
      const nephroticSyndromeEntity = await this.createNephroticSyndromeEntity();
      
      // 2. 创建临床表现知识
      const clinicalManifestationsKnowledge = await this.createClinicalManifestationsKnowledge(nephroticSyndromeEntity);
      
      // 3. 创建症状实体并建立关系
      const symptomEntities = await this.createSymptomEntities();
      await this.linkSymptomsToKnowledge(clinicalManifestationsKnowledge, symptomEntities);
      
      // 4. 验证关系结构
      const isStructureValid = await this.validateRelationshipStructure(nephroticSyndromeEntity);
      
      // 5. 验证双向查询
      const isBidirectionalValid = await this.validateBidirectionalQueries(nephroticSyndromeEntity);
  ...
  private async createNephroticSyndromeEntity(): Promise<Entity> {
    const entityData: EntityData = {
      ...
      tags: ['疾病', '肾脏疾病', '综合征'],
      definition: '肾病综合征是一组由多种病因引起的临床症候群，以大量蛋白尿、低蛋白血症、水肿和高脂血症为特征。',
    };
    
    const entity = await Entity.create_entity_with_entity_data(entityData).save(this.entityStorage);
  ...
  private async createClinicalManifestationsKnowledge(parentEntity: Entity) {
    const clinicalText = '肾病综合征的临床表现可总结为"三高一低"：大量蛋白尿、低蛋白血症、水肿和高脂血症。这些表现共同构成了肾病综合征的典型临床特征。';
    
    const knowledge = await this.knowledgeWorkflow.create_simple_knowledge_from_text(
      clinicalText,
      parentEntity,
      '临床表现',
    );
  ...
  private async createSymptomEntities(): Promise<Entity[]> {
    const symptoms = [
      {
        ...
        tags: ['症状', '尿液异常'],
        definition: '大量蛋白尿是指24小时尿蛋白定量超过3.5g，是肾病综合征的主要特征之一。',
      },
      {
        name: ['低蛋白血症', 'Hypoproteinemia'],
        tags: ['症状', '血液异常'],
        definition: '低蛋白血症是指血浆白蛋白水平低于30g/L，由于大量蛋白尿导致蛋白质丢失所致。',
      },
      {
        name: ['水肿', 'Edema'],
        tags: ['症状', '体液异常'],
        definition: '水肿是由于低蛋白血症导致血浆胶体渗透压降低，液体从血管内渗出到组织间隙所致。',
      },
      {
        name: ['高脂血症', 'Hyperlipidemia'],
        tags: ['症状', '代谢异常'],
        definition: '高脂血症是机体代偿性增加肝脏脂蛋白合成以补偿低蛋白血症的结果。',
      },
    ];
    ...
    for (const symptomData of symptoms) {
      const entity = await Entity.create_entity_with_entity_data(symptomData).save(this.entityStorage);
      symptomEntities.push(entity);
      this.logger.info(`创建症状实体成功: ${symptomData.name[0]}，ID: ${entity.get_id()}`);
    }
   ...
   * 将症状实体链接到临床表现知识
   */
  private async linkSymptomsToKnowledge(clinicalKnowledge: any, symptomEntities: Entity[]) {
    // 为每个症状创建相关知识并链接
    for (const symptomEntity of symptomEntities) {
      const symptomName = symptomEntity.get_definition().split('是指')[0];
      const symptomKnowledge = await this.knowledgeWorkflow.create_simple_knowledge_from_text(
        `${symptomName}是肾病综合征的重要临床表现之一。`,
        symptomEntity,
        '与肾病综合征的关系',
      );
      
      this.logger.info(`创建症状知识并链接成功: ${symptomName}，知识ID: ${symptomKnowledge.get_id()}`);
    }
  }
   ...
   * 验证关系结构
   */
  private async validateRelationshipStructure(mainEntity: Entity): Promise<boolean> {
    try {
      // 获取主实体的所有下属知识
      const subordinateKnowledge = await mainEntity.get_subordinate_knowledge(this.knowledgeStorage);
      ...
      // 验证临床表现知识存在
      const clinicalKnowledge = subordinateKnowledge.find(k => 
        k.getData().scope === '临床表现'
      );
   ...
   * 验证双向查询
   */
  private async validateBidirectionalQueries(mainEntity: Entity): Promise<boolean> {
    try {
      // 1. 从实体查询知识
      const entityToKnowledge = await mainEntity.get_subordinate_knowledge(this.knowledgeStorage);
      ...
      for (const knowledge of entityToKnowledge) {
        const links = await this.knowledgeStorage.knowledgeGraphStorage.get_knowledge_links_by_source(
          mainEntity.get_id()
        );
        
        for (const link of links) {
          // 验证链接的目标确实是知识
          const targetKnowledge = await this.knowledgeStorage.get_knowledge_by_id(link.targetId);
          if (targetKnowledge) {
      ...
      const isValid = entityToKnowledge.length > 0 && knowledgeToEntity.length > 0;
  ...
  async validateKnowledgeHierarchy(): Promise<boolean> {
    ...
    try {
      ...
      const testEntity = await Entity.create_entity_with_entity_data(testEntityData).save(this.entityStorage);
      ...
      const hasValidMarkdown = markdown.includes('#') && markdown.includes('##');
  ...
  async runAllValidations(): Promise<{
    ...
    const results = {
      entityKnowledgeCreation: await this.validateCompleteEntityKnowledgeCreation(),
      knowledgeHierarchy: await this.validateKnowledgeHierarchy(),
      vectorSimilaritySearch: await this.validateVectorSimilaritySearch(),
      overall: false,
    };
    
    results.overall = results.entityKnowledgeCreation && 
...
export async function runEntityKnowledgeRelationshipValidation() {
  ...
  console.log(`实体-知识创建流程: ${results.entityKnowledgeCreation ? '✅ 通过' : '❌ 失败'}`);
  console.log(`知识层次结构: ${results.knowledgeHierarchy ? '✅ 通过' : '❌ 失败'}`);
  console.log(`向量相似性搜索: ${results.vectorSimilaritySearch ? '✅ 通过' : '❌ 失败'}`);
