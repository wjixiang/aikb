...
class MockMarkdownPartCache extends MarkdownPartCache {
  private parts: Map<string, Map<number, { content: string; status: string }>> = new Map();
  ...
  async storePartMarkdown(itemId: string, partIndex: number, markdownContent: string): Promise<void> {
    ...
    this.parts.get(itemId)!.set(partIndex, { content: markdownContent, status: 'completed' });
  }
...
class MockPdfPartTracker implements IPdfPartTracker {
  ...
  async updatePartStatus(itemId: string, partIndex: number, status: any, error?: string): Promise<void> {
    await this.ensureInitialized();
    const processingStatus = this.processingStatus.get(itemId);
    if (!processingStatus) return;

    // Update part status arrays
    processingStatus.pendingParts = processingStatus.pendingParts.filter(i => i !== partIndex);
    processingStatus.processingParts = processingStatus.processingParts.filter(i => i !== partIndex);
    processingStatus.completedParts = processingStatus.completedParts.filter(i => i !== partIndex);
    processingStatus.failedParts = processingStatus.failedParts.filter(i => i !== partIndex);
    ...
    if (processingStatus.completedParts.length === processingStatus.totalParts) {
      processingStatus.status = 'completed';
      processingStatus.endTime = Date.now();
    } else if (processingStatus.failedParts.length > 0 && processingStatus.completedParts.length === 0) {
