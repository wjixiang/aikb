...
export async function testPdfAnalysisOptimization(): Promise<void> {
  ...
  try {
    ...
    const originalDownloadPdfFromS3 = (analyzerService as any)
      .downloadPdfFromS3;
    const originalGetPageCount = (analyzerService as any).getPageCount;
    const originalExtractPdfMetadata = (analyzerService as any)
      .extractPdfMetadata;
    ...
    (analyzerService as any).extractPdfMetadata = async (
      pdfBuffer: Buffer,
      s3Url: string,
    ) => {
      return {
    ...
    logger.info(
      `Analysis completed: ${capturedAnalysisMessage ? 'YES' : 'NO'}`,
    );

    if (capturedAnalysisMessage) {
      logger.info(`Page count: ${capturedAnalysisMessage.pageCount}`);
      logger.info(
        `Requires splitting: ${capturedAnalysisMessage.requiresSplitting}`,
      );
      logger.info(
        `PDF metadata passed: ${capturedAnalysisMessage.pdfMetadata ? 'YES' : 'NO'}`,
      );
      logger.info(
        `S3 URL passed: ${capturedAnalysisMessage.s3Url ? 'YES' : 'NO'}`,
      );
      logger.info(
        `S3 Key passed: ${capturedAnalysisMessage.s3Key ? 'YES' : 'NO'}`,
      );
    ...
    if (conversionRequest.pdfMetadata) {
      logger.info(
        '✅ OPTIMIZATION: Conversion worker has PDF metadata from analysis phase',
      );
    ...
    logger.info(
      '✅ Splitting is now handled directly in the PDF analyzer service',
    );
    logger.info('✅ Splitting worker has been removed from the architecture');
    logger.info(
      '✅ PDF parts are created and uploaded to S3 during analysis phase',
    );
    logger.info(
      '✅ Part conversion requests are sent directly from the analyzer',
    );
