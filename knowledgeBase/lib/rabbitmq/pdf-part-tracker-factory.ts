...
export class PdfPartTrackerFactory {
  ...
  static getInstance(): IPdfPartTracker {
    if (!this.instance) {
      ...
      logger.info(`Creating new PDF part tracker instance with ${storageType} storage`);
   ...
   */
  static getInstanceWithStorage(storageType: 'mongodb' | 'elasticsearch', elasticsearchUrl?: string): IPdfPartTracker {
    logger.info(`Creating new PDF part tracker instance with ${storageType} storage`);
    
    if (storageType === 'elasticsearch') {
      const url = elasticsearchUrl || process.env.ELASTICSEARCH_URL || 'http://elasticsearch:9200';
 ...
 */
export function getPdfPartTrackerWithStorage(storageType: 'mongodb' | 'elasticsearch', elasticsearchUrl?: string): IPdfPartTracker {
  return PdfPartTrackerFactory.getInstanceWithStorage(storageType, elasticsearchUrl);
}