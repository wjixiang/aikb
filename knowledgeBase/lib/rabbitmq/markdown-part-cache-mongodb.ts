...
import { MarkdownPartCache, MarkdownPartCacheError, MarkdownPartInfo } from './markdown-part-cache';
...
export class MongoDBMarkdownPartCache extends MarkdownPartCache {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // 获取集合引用
      this.partsCollection = db.collection(MongoDBMarkdownPartCache.PARTS_COLLECTION);
      this.metadataCollection = db.collection(MongoDBMarkdownPartCache.METADATA_COLLECTION);
  ...
  private ensureInitialized(): void {
    if (!this.isInitialized || !this.db || !this.partsCollection || !this.metadataCollection) {
      throw new MarkdownPartCacheError(
  ...
  async getPartMarkdown(itemId: string, partIndex: number): Promise<string | null> {
    try {
      ...
      this.logOperationError('getPartMarkdown', error as Error, itemId, partIndex);
    ...
    try {
      ...
      if (result.matchedCount === 0) {
        ...
        if (status === 'failed') {
          ...
          this.logOperationSuccess('updatePartStatus', itemId, partIndex, { status });
          return;
        }
      ...
      this.logOperationError('updatePartStatus', error as Error, itemId, partIndex);
  ...
  async cleanup(itemId: string): Promise<void> {
    try {
      ...
      // 删除元数据
      const metadataResult = await this.metadataCollection!.deleteOne({ itemId });
  ...
  private async checkAndUpdateOverallStatus(itemId: string): Promise<void> {
    try {
      ...
      // 检查是否所有部分都已完成
      const completedCount = allParts.filter(part => part.status === 'completed').length;
      const failedCount = allParts.filter(part => part.status === 'failed').length;
      
      let newStatus = 'processing';
      if (completedCount === allParts.length) {
        newStatus = 'completed';
      } else if (failedCount > 0 && completedCount + failedCount === allParts.length) {
        newStatus = 'failed';
      }
