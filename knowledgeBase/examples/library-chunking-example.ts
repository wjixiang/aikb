...
async function main() {
  try {
    ...
    const vectorDimensions = parseInt(
    ...
    logger.info(
      `Found ${textSearchResults.length} chunks matching 'machine learning'`,
    );

    textSearchResults.forEach((chunk, index) => {
      logger.info(
        `Result ${index + 1}: ${chunk.title} - ${chunk.content.substring(0, 100)}...`,
      );
    });
    ...
    const queryVector = new Array(vectorDimensions)
      .fill(0)
      .map((_, i) => Math.random());
    ...
    logger.info(
      `Found ${similarChunksGeneral.length} similar chunks using general method`,
    );
    similarChunksGeneral.forEach((chunk, index) => {
      logger.info(
        `Similar ${index + 1}: ${chunk.title} (similarity: ${chunk.similarity.toFixed(3)})`,
      );
    });
    ...
    logger.info(
      `Found ${similarChunksInItemMethod.length} similar chunks using item-specific method`,
    );
    ...
    logger.info(
      `Found ${itemSearchResults1.length} chunks in specific item using general method`,
    );
    ...
    logger.info(
      `Found ${itemSearchResultsMethod.length} chunks in specific item using item-specific method`,
    );
    ...
    const queryVector2 = new Array(vectorDimensions)
      .fill(0)
      .map((_, i) => Math.random());

    // Search in first item
    const similarInItem1 = await library.findSimilarChunksInItem(
      item.metadata.id!,
      queryVector2,
      3,
    );
    logger.info(
      `Found ${similarInItem1.length} similar chunks in item 1: "${item.metadata.title}"`,
    );

    // Search in second item
    const similarInItem2 = await library.findSimilarChunksInItem(
      item2.metadata.id!,
      queryVector2,
      3,
    );
    logger.info(
      `Found ${similarInItem2.length} similar chunks in item 2: "${item2.metadata.title}"`,
    );

    // Search across both items
    const similarInBoth = await library.findSimilarChunks(
      queryVector2,
      5,
      0.5,
      [item.metadata.id!, item2.metadata.id!],
    );
    logger.info(
      `Found ${similarInBoth.length} similar chunks across both items`,
    );
    ...
    // Search in first item
    const textResults1 = await library.searchChunksInItem(
      item.metadata.id!,
      searchQuery,
      5,
    );
    logger.info(
      `Found ${textResults1.length} text matches in item 1: "${item.metadata.title}"`,
    );

    // Search in second item
    const textResults2 = await library.searchChunksInItem(
      item2.metadata.id!,
      searchQuery,
      5,
    );
    logger.info(
      `Found ${textResults2.length} text matches in item 2: "${item2.metadata.title}"`,
    );
    ...
    const queryVector3 = new Array(vectorDimensions)
      .fill(0)
      .map((_, i) => Math.random());
    const itemSimilarChunks = await libraryItem.findSimilarInChunks(
      queryVector3,
      3,
      0.5,
    );
    logger.info(
      `Found ${itemSimilarChunks.length} similar chunks within the item`,
    );
    ...
    logger.info(
      `Re-processed with paragraph strategy: ${newParagraphChunks.length} chunks`,
    );
