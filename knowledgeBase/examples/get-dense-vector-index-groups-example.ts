...
async function demonstrateGetDenseVectorIndexGroups() {
  ...
  try {
    ...
    console.log(
      `Available Dense Vector Index Groups for item "${item.metadata.title}":`,
    );

    if (denseVectorIndexGroups.length === 0) {
      console.log(
        '  No dense vector index groups found. This item may not have any chunks with embeddings.',
      );
    } else {
      ...
      if (denseVectorIndexGroups.length > 0) {
        ...
        console.log(
          `Found ${chunksFromGroup.length} chunks in group "${selectedGroup}"`,
        );
      }
    }
  } catch (error) {
...
async function checkMultipleItems() {
  ...
  try {
    ...
    for (const item of items) {
      ...
      console.log(
        `  Available groups: ${groups.length > 0 ? groups.join(', ') : 'None'}`,
      );
      console.log('');
    }
  } catch (error) {
  ...
  try {
    ...
    for (const group of groups) {
      ...
      results.forEach((result, index) => {
        console.log(
          `    ${index + 1}. ${result.title} (similarity: ${result.similarity.toFixed(2)})`,
        );
      });
