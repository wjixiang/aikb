...
async function knowledgeStorageExample() {
  ...
  try {
    ...
    const knowledgeData = {
      scope: 'Algorithms',
      content: 'Algorithms are step-by-step procedures for solving problems and performing tasks.',
      childKnowledgeId: [],
    };
    
    const tempKnowledge = entity.create_knowledge_with_knowledge_data(knowledgeData);
    ...
    const dummyVector = Array(1536).fill(0).map((_, i) => Math.random());
    ...
    await knowledge.update({
      ...
      content: 'Algorithms and data structures are fundamental concepts in computer science for organizing and processing information efficiently.',
    });
    console.log('Updated knowledge content');

    // Retrieve knowledge by ID
    const retrievedKnowledge = await knowledgeStorage.get_knowledge_by_id(knowledge.get_id());
    ...
    const searchResults = await knowledgeContentStorage.search_knowledge_contents('algorithm');
    console.log(`Found ${searchResults.length} knowledge items matching 'algorithm'`);

    // List all knowledge
    const allKnowledge = await knowledgeContentStorage.list_all_knowledge_contents();
...
async function advancedKnowledgeExample() {
  ...
  try {
    ...
    const tempRootKnowledge = new TKnowledge(rootKnowledgeData, { get_id: () => 'source_entity' } as any);
    ...
    const reinforcementLearningData = {
      ...
      content: 'Reinforcement learning learns optimal actions through trial and error.',
      childKnowledgeId: [],
    };

    const supervisedLearning = await rootKnowledge.subdivide(supervisedLearningData);
    const unsupervisedLearning = await rootKnowledge.subdivide(unsupervisedLearningData);
    const reinforcementLearning = await rootKnowledge.subdivide(reinforcementLearningData);
    ...
    const updatedRoot = await knowledgeStorage.get_knowledge_by_id(rootKnowledge.get_id());
    if (updatedRoot) {
      console.log(`Updated children count: ${updatedRoot.getChildren().length}`);
