...
async function simpleKnowledgeTest() {
  ...
  try {
    ...
    const knowledgeData: KnowledgeData = {
      ...
      content: 'This is a test knowledge item for verifying MongoDB storage functionality.',
      childKnowledgeId: [],
    };
    
    // Create temporary knowledge
    const tempKnowledge = new TKnowledge(knowledgeData, { get_id: () => 'test_source' } as any);
    ...
    // Retrieve vector
    const retrievedVector = await knowledgeVectorStorage.get_knowledge_vector(knowledge.get_id());
    if (retrievedVector) {
      console.log(`Retrieved vector with ${retrievedVector.vector.length} dimensions`);
    ...
    const searchResults = await knowledgeContentStorage.search_knowledge_contents('test');
    console.log(`Found ${searchResults.length} knowledge items matching 'test'`);
