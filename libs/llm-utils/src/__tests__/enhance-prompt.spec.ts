// npx vitest run sllm-utils/__tests__/enhance-prompt.spec.ts

import type { ProviderSettings } from 'llm-types';

import { singleCompletionHandler } from '../single-completion-handler';
import { buildApiHandler, type SingleCompletionHandler } from 'llm-api';
import { supportPrompt } from 'llm-shared/support-prompt';
...
describe('enhancePrompt', () => {
  ...
  it('enhances prompt using custom enhancement prompt when provided', async () => {
    ...
    const customEnhancePromptWithTemplate =
      customEnhancePrompt + '\n\n${userInput}';
  ...
  it('throws error for empty prompt input', async () => {
    await expect(singleCompletionHandler(mockApiConfig, '')).rejects.toThrow(
  ...
  it('throws error for missing API configuration', async () => {
    await expect(
      singleCompletionHandler({} as ProviderSettings, 'Test prompt'),
    ).rejects.toThrow('No valid API configuration provided');
  });
