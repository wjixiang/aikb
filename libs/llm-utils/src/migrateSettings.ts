...
import { GlobalFileNames } from 'llm-shared/globalFileNames';
...
export async function migrateSettings(
  ...
  const fileMigrations = [
    ...
    {
      oldName: 'cline_custom_modes.json',
      newName: deprecatedCustomModesJSONFilename,
    },
    {
      oldName: 'cline_mcp_settings.json',
      newName: GlobalFileNames.mcpSettings,
    },
  ];

  try {
    const settingsDir = await getSettingsDirectoryPath(
    ...
    if (!(await fileExistsAtPath(settingsDir))) {
      outputChannel.appendLine(
        'No settings directory found, no migrations necessary',
      );
      return;
    }
    ...
    try {
      for (const migration of fileMigrations) {
        ...
        if (oldFileExists && !newFileExists) {
          ...
          outputChannel.appendLine(
            `Renamed ${migration.oldName} to ${migration.newName}`,
          );
        } else {
          outputChannel.appendLine(
 ...
 */
async function migrateCustomModesToYaml(
  settingsDir: string,
  outputChannel: vscode.OutputChannel,
): Promise<void> {
  ...
  try {
    ...
    outputChannel.appendLine(
      `Error reading custom_modes.json: ${fileError}. Skipping migration.`,
    );
  }
}
...
async function migrateDefaultCommands(
  ...
  try {
    ...
    const allowedCommands =
      context.globalState.get<string[]>('allowedCommands');
    ...
    const filteredCommands = allowedCommands.filter((cmd) => {
      ...
      return !oldDefaultCommands.some(
        (oldDefault) => cmdLower === oldDefault.toLowerCase(),
      );
    });
    ...
    if (filteredCommands.length < originalLength) {
      ...
      outputChannel.appendLine(
        '[Default Commands Migration] No old default commands found in allowed list',
      );
    }
    ...
    await context.globalState.update(migrationKey, true);
    outputChannel.appendLine(
      '[Default Commands Migration] Migration marked as complete',
    );
  } catch (error) {
    outputChannel.appendLine(
      `[Default Commands Migration] Error migrating default commands: ${error}`,
    );
  }
}
