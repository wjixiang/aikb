...
export class GitVersionControlService implements IGitVersionControl {
  ...
  private async dbCommitToDomain(dbCommit: CommitModel): Promise<Commit> {
    ...
    if (!gitObject) {
      throw new NotFoundException(
        `GitObject not found for commit ${dbCommit.id}`,
      );
    }

    // Get parent commits
    const parentCommits =
      await this.versionControlDbService.commitParent.findMany({
  ...
  async initRepository(repositoryId: string): Promise<void> {
    ...
    const existingRepo =
      await this.versionControlDbService.repository.findUnique({
        where: { repositoryId },
      });
  ...
  async createCommit(options: {
    ...
    if (!branch) {
      throw new NotFoundException(
        `Branch ${branchName} not found in repository ${repositoryId}`,
      );
    }
    ...
    if (parentCommitIds && parentCommitIds.length > 0) {
      for (const parentId of parentCommitIds) {
        const parentCommit =
          await this.versionControlDbService.commit.findUnique({
  ...
  async createBranch(options: {
    ...
    if (existingBranch) {
      throw new ConflictException(
        `Branch ${branchName} already exists in repository ${repositoryId}`,
      );
    }
    ...
    const newBranch = await this.versionControlDbService.branch.create({
      data: {
        branchId: this.generateObjectId({
          repositoryId,
          branchName,
          timestamp: new Date(),
        }),
  ...
  async mergeBranch(options: {
    ...
    const { repositoryId, sourceBranch, targetBranch, author, message } =
      options;
    ...
    if (!source) {
      throw new NotFoundException(
        `Source branch ${sourceBranch} not found in repository ${repositoryId}`,
      );
    }

    if (!target) {
      throw new NotFoundException(
        `Target branch ${targetBranch} not found in repository ${repositoryId}`,
      );
    }

    // Get commits from both branches
    const sourceCommits =
      await this.versionControlDbService.branchCommit.findMany({
    ...
    const newCommitIds = sourceCommitIds.filter(
      (id) => !targetCommitIds.includes(id),
    );
    ...
    const changeSet: ChangeSet = {
      added: allChanges.filter((c) =>
        c.diff && typeof c.diff === 'object' && 'changeType' in c.diff
          ? (c.diff as Diff).changeType === 'add'
          : false,
      ),
      modified: allChanges.filter((c) =>
        c.diff && typeof c.diff === 'object' && 'changeType' in c.diff
          ? (c.diff as Diff).changeType === 'modify'
          : false,
      ),
      deleted: allChanges.filter((c) =>
        c.diff && typeof c.diff === 'object' && 'changeType' in c.diff
          ? (c.diff as Diff).changeType === 'delete'
          : false,
      ),
    };
    ...
    const mergeCommit = await this.createCommit({
      ...
      author,
      changes: changeSet,
      parentCommitIds: [target.headCommitId, source.headCommitId].filter(
        (id) => id,
      ),
    });
  ...
  async getCommitHistory(options: {
    ...
    if (!branch) {
      throw new NotFoundException(
        `Branch ${targetBranchName} not found in repository ${repositoryId}`,
      );
    }
  ...
  async compareCommits(
    ...
    for (const change of changes1) {
      const diffChange: DiffChange = {
        ...
        oldContent:
          ...
          'old' in (change.diff as any)
            ? (change.diff as any).old
            : undefined,
        newContent:
          change.changeType === 'add' &&
          change.diff &&
          typeof change.diff === 'object' &&
          'new' in (change.diff as any)
            ? (change.diff as any).new
            : undefined,
    ...
    for (const change of changes2) {
      const existsInCommit1 = changes1.some(
        (c1) => c1.path === change.path && c1.type === change.type,
      );
        ...
        const diffChange: DiffChange = {
          ...
          oldContent:
            change.changeType === 'delete'
              ...
              : undefined,
          newContent:
            change.changeType === 'add' ? (change.diff as any)?.new : undefined,
          diff: change.diff as string | undefined,
        };
  ...
  async resetToCommit(
    ...
    if (!currentBranch) {
      throw new NotFoundException(
        `Current branch ${repository.currentBranch} not found in repository ${repositoryId}`,
      );
    }
