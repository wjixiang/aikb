...
describe('EntityStorageMemoryService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<EntityStorageMemoryService>(
  ...
  describe('update', () => {
    ...
    it('should update existing entity', async () => {
      ...
      const updates = {
        nomanclature: [
          { name: 'Updated Entity', acronym: 'UE', language: 'en' as const },
        ],
      };
  ...
  describe('search', () => {
    beforeEach(async () => {
      ...
      await service.create(
        createTestEntity({
          ...
          abstract: {
            description:
              'Study of computer algorithms that improve automatically',
            embedding: { config: {} as any, vector: [1, 0, 0] },
          },
        }),
      );
      ...
      await service.create(
        createTestEntity({
          nomanclature: [
            { name: 'Deep Learning', acronym: 'DL', language: 'en' as const },
          ],
          abstract: {
            description: 'Subset of machine learning using neural networks',
            embedding: { config: {} as any, vector: [0, 1, 0] },
          },
        }),
      );
      ...
      await service.create(
        createTestEntity({
          nomanclature: [
            { name: '机器学习', acronym: null, language: 'zh' as const },
          ],
          abstract: {
            description: '计算机算法的自动改进研究',
            embedding: { config: {} as any, vector: [0, 0, 1] },
          },
        }),
      );
    });
    ...
    it('should find entities by name', async () => {
      ...
      expect(
        results.some((r) => r.nomanclature[0].name === 'Machine Learning'),
      ).toBe(true);
    });
  ...
  describe('findBySimilarity', () => {
    beforeEach(async () => {
      await service.create(
        createTestEntity({
          nomanclature: [
            { name: 'Similar 1', acronym: null, language: 'en' as const },
          ],
          abstract: {
            description: 'First similar entity',
            embedding: { config: {} as any, vector: [1, 0, 0] },
          },
        }),
      );
      ...
      await service.create(
        createTestEntity({
          nomanclature: [
            { name: 'Similar 2', acronym: null, language: 'en' as const },
          ],
          abstract: {
            description: 'Second similar entity',
            embedding: { config: {} as any, vector: [0.9, 0.1, 0] },
          },
        }),
      );
      ...
      await service.create(
        createTestEntity({
          nomanclature: [
            { name: 'Different', acronym: null, language: 'en' as const },
          ],
          abstract: {
    ...
    beforeEach(async () => {
      for (let i = 0; i < 5; i++) {
        await service.create(
          createTestEntity({
            nomanclature: [
              { name: `Entity ${i}`, acronym: null, language: 'en' as const },
            ],
            abstract: {
              description: `Description ${i}`,
              embedding: { config: {} as any, vector: [i, 0, 0] },
            },
          }),
        );
      }
