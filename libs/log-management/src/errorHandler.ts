import createLoggerWithPrefix from './logger.js';
...
export class ChunkingErrorHandler {
  ...
  static handleEmbeddingError(
    ...
    if (context.operation === 'embedChunk') {
      this.logger.warn(
        `Returning empty embedding for ${context.chunkId} due to error`,
      );
      return null;
    }
  ...
  static handleSearchError(
    ...
    // instead of throwing an error, depending on the context
    if (
      context.operation === 'searchChunks' ||
      context.operation === 'findSimilarChunks'
    ) {
      this.logger.warn(
        `Returning empty results for ${context.operation} due to error`,
      );
      return [];
    }
  ...
  static validateParams(
    ...
    if (
      params.queryVector &&
      (!Array.isArray(params.queryVector) || params.queryVector.length === 0)
    ) {
    ...
    if (errors.length > 0) {
      const error = new Error(
        `Invalid parameters for ${operation}: ${errors.join(', ')}`,
      );
      this.logger.error(`Parameter validation failed for ${operation}`, {
        errors,
        params,
      });
      throw error;
  ...
  static async withRetry<T>(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        this.logger.warn(
          `Operation ${context.operation} failed, retrying in ${delay}ms (attempt ${attempt + 1}/${maxRetries + 1})`,
          {
            error: error instanceof Error ? error.message : 'Unknown error',
          },
        );
