...
export class ORPCPasswordResetController {
  ...
  passwordReset() {
    return {
      request: implement(authContract.passwordReset.request).handler(
        async ({ input }: { input: PasswordResetRequestDto }) => {
          try {
      ...
      confirm: implement(authContract.passwordReset.confirm).handler(
        async ({ input }: { input: PasswordResetConfirmDto }) => {
          try {
