...
export function getModelParams(
  options: GetModelParamsOptions<'anthropic'>,
): AnthropicModelParams;
export function getModelParams(
  options: GetModelParamsOptions<'openai'>,
): OpenAiModelParams;
export function getModelParams(
  options: GetModelParamsOptions<'gemini'>,
): GeminiModelParams;
export function getModelParams(
  options: GetModelParamsOptions<'openrouter'>,
): OpenRouterModelParams;
export function getModelParams({
  format,
  modelId,
  ...
  if (shouldUseReasoningBudget({ model, settings })) {
    ...
    const minThinkingTokens = isGemini25Pro
      ? GEMINI_25_PRO_MIN_THINKING_TOKENS
      : 1024;
