...
import { CacheResult, CachePointPlacement } from 'llm-types';
...
export class MultiPointStrategy extends CacheStrategy {
  ...
  public determineOptimalCachePoints(): CacheResult {
    ...
    const supportsSystemCache =
      this.config.modelInfo.cachableFields.includes('system');
    const supportsMessageCache =
      this.config.modelInfo.cachableFields.includes('messages');
    ...
    const useSystemCache =
      ...
      this.meetsMinTokenThreshold(this.systemTokenCount);
    ...
    if (!supportsMessageCache) {
      return this.formatResult(
        systemBlocks,
        this.messagesToContentBlocks(this.config.messages),
      );
    }

    const placements = this.determineMessageCachePoints(
      minTokensPerPoint,
      remainingCachePoints,
    );
    const messages = this.messagesToContentBlocks(this.config.messages);
    let cacheResult = this.formatResult(
      systemBlocks,
      this.applyCachePoints(messages, placements),
    );
  ...
  private determineMessageCachePoints(
    ...
    const lastPreviousIndex =
      previousPlacements[previousPlacements.length - 1].index;
    ...
    if (newMessagesTokens >= minTokensPerPoint) {
      ...
      if (remainingCachePoints > previousPlacements.length) {
        ...
        for (let i = 0; i < tokensBetweenPlacements.length - 1; i++) {
          const gap =
            tokensBetweenPlacements[i] + tokensBetweenPlacements[i + 1];
          if (gap < smallestGap) {
            smallestGap = gap;
            smallestGapIndex = i;
          }
        ...
        if (newMessagesTokens >= requiredTokenThreshold) {
          ...
          for (let i = 0; i < previousPlacements.length; i++) {
            if (i !== smallestGapIndex && i !== smallestGapIndex + 1) {
              ...
              const startOfRange =
                i === 0 ? 0 : previousPlacements[i - 1].index + 1;
  ...
  private findOptimalPlacementForRange(
    ...
    if (lastUserMessageIndex >= 0) {
      ...
      for (const placement of previousPlacements) {
        if (
          placement.index < startIndex &&
          placement.index > previousCachePointIndex
        ) {
