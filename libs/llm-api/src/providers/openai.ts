  ...
  implements SingleCompletionHandler
{
  ...
  override async *createMessage(
    ...
    const deepseekReasoner =
      modelId.includes('deepseek-reasoner') || enabledR1Format;
    ...
    if (
      ...
      yield* this.handleO3FamilyMessage(
        modelId,
        systemPrompt,
        messages,
        metadata,
    ...
    if (this.options.openAiStreamingEnabled ?? true) {
      ...
      if (deepseekReasoner) {
        ...
        convertedMessages = [
          systemMessage,
          ...convertToSimpleMessages(messages),
        ];
      } else {
  ...
  async completePrompt(prompt: string): Promise<string> {
    try {
      ...
      if (error instanceof Error) {
        throw new Error(
          `${this.providerName} completion error: ${error.message}`,
        );
      }
