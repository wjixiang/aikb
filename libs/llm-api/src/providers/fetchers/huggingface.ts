 ...
 */
function parseHuggingFaceModel(
  ...
  const pricing =
    provider?.pricing || model.providers.find((p) => p.pricing)?.pricing;

  // Include provider name in description if specific provider is given.
  const description = provider
    ? `${model.id} via ${provider.provider}`
    : `${model.id} via HuggingFace`;
...
export async function getHuggingFaceModels(): Promise<ModelRecord> {
  ...
  try {
    const response = await axios.get<HuggingFaceApiResponse>(
      HUGGINGFACE_API_URL,
      {
        headers: {
    ...
    if (!result.success) {
      console.error(
        'HuggingFace models response validation failed:',
        result.error.format(),
      );
      throw new Error('Invalid response format from HuggingFace API');
    }

    const validModels = result.data.data.filter(
      (model) => model.providers.length > 0,
    );
