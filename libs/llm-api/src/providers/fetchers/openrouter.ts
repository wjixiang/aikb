...
export type OpenRouterModelEndpoint = z.infer<
  typeof openRouterModelEndpointSchema
>;
...
type OpenRouterModelEndpointsResponse = z.infer<
  typeof openRouterModelEndpointsResponseSchema
>;
...
export async function getOpenRouterModelEndpoints(
  ...
  try {
    ...
    const result = openRouterModelEndpointsResponseSchema.safeParse(
      response.data,
    );
    const data = result.success ? result.data.data : response.data.data;
    ...
    if (!result.success) {
      console.error(
        'OpenRouter model endpoints response is invalid',
        result.error.format(),
      );
    }
...
export const parseOpenRouterModel = ({
  ...
  const supportsNativeTools = supportedParameters
  ...
  const modelInfo: ModelInfo = {
    ...
    supportsReasoningEffort: supportedParameters
      ? supportedParameters.includes('reasoning')
      : undefined,
    supportsNativeTools,
    supportedParameters: supportedParameters
      ? supportedParameters.filter(isModelParameter)
      : undefined,
