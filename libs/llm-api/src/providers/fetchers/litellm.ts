...
export async function getLiteLLMModels(
  ...
  try {
    ...
    urlObj.pathname =
      urlObj.pathname.replace(/\/+$/, '').replace(/\/+/g, '/') +
      '/v1/model/info';
    ...
    if (
      ...
      for (const model of response.data.data) {
        ...
        const litellmModelName = model?.litellm_params?.model as
          | string
          | undefined;
        ...
        models[modelName] = {
          maxTokens:
            modelInfo.max_output_tokens || modelInfo.max_tokens || 8192,
      ...
      console.error(
        'Error fetching LiteLLM models: Unexpected response format',
        response.data,
      );
      throw new Error(
    ...
    if (axios.isAxiosError(error) && error.response) {
      ...
      throw new Error(
        `Failed to fetch LiteLLM models: ${error.message || 'An unknown error occurred.'}`,
      );
    }
  }
}
