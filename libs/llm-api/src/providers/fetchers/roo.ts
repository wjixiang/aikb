import { RooModelsResponseSchema, type ModelInfo } from 'agent-lib/types';
 ...
 */
export async function getRooModels(
  baseUrl: string,
  apiKey?: string,
): Promise<ModelRecord> {
  ...
  try {
    ...
    try {
      ...
      if (!parsed.success) {
        console.error(
          'Error fetching Roo Code Cloud models: Unexpected response format',
          data,
        );
      ...
      for (const model of parsed.data.data) {
        ...
        const cacheReadPrice = pricing.input_cache_read
          ? parseApiPrice(pricing.input_cache_read)
          : undefined;
        const cacheWritePrice = pricing.input_cache_write
          ? parseApiPrice(pricing.input_cache_write)
          : undefined;
        ...
        models[modelId] = apiSettings
          ? { ...baseModelInfo, ...apiSettings }
          : baseModelInfo;
      }
    ...
    if (error.name === 'AbortError') {
      throw new Error(
        'Failed to fetch Roo Code Cloud models: Request timed out after 10 seconds.',
      );
    }
    ...
    if (error.message?.includes('HTTP')) {
      throw new Error(
        `Failed to fetch Roo Code Cloud models: ${error.message}. Check base URL and API key.`,
      );
    }
    ...
    throw new Error(
      `Failed to fetch Roo Code Cloud models: ${error.message || 'An unknown error occurred.'}`,
    );
  }
}
