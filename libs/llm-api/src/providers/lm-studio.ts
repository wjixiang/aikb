  ...
  implements SingleCompletionHandler
{
  ...
  override async *createMessage(
    ...
    try {
      const params: OpenAI.Chat.ChatCompletionCreateParamsStreaming & {
        ...
        temperature:
          this.options.modelTemperature ?? LMSTUDIO_DEFAULT_TEMPERATURE,
        stream: true,
      };

      if (
        this.options.lmStudioSpeculativeDecodingEnabled &&
        this.options.lmStudioDraftModelId
      ) {
      ...
      let outputTokens = 0;
      try {
        outputTokens = await this.countTokens([
  ...
  override getModel(): { id: string; info: ModelInfo } {
    ...
    if (
      models &&
      this.options.lmStudioModelId &&
      models[this.options.lmStudioModelId]
    ) {
      return {
