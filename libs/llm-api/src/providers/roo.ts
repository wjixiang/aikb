...
import {
  getModels,
  getModelsFromCache,
} from '../providers/fetchers/modelCache';
...
type RooChatCompletionParams =
  OpenAI.Chat.ChatCompletionCreateParamsStreaming & {
    reasoning?: RooReasoningParams;
  };

function getSessionToken(): string {
  const token = CloudService.hasInstance()
...
export class RooHandler extends BaseOpenAiCompatibleProvider<string> {
  ...
  constructor(options: ApiHandlerOptions) {
    ...
    this.fetcherBaseURL = baseURL.endsWith('/v1')
      ? baseURL.slice(0, -3)
      : baseURL;
  ...
  override async *createMessage(
    ...
    try {
      ...
      if (lastUsage) {
        ...
        const inputTokensForDownstream =
          apiProtocol === 'anthropic' ? nonCached : promptTokens;
      ...
      console.error(
        `[RooHandler] Error during message streaming: ${JSON.stringify(errorContext)}`,
      );
