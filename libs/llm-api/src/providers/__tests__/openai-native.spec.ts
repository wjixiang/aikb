      ...
      const mockFetch = vi.fn().mockResolvedValue({
        ...
        body: new ReadableStream({
          start(controller) {
            ...
            controller.enqueue(
              new TextEncoder().encode(
                'data: {"type":"response.text.delta","delta":" response"}\n\n',
              ),
            );
            controller.enqueue(
              new TextEncoder().encode(
    ...
    it('should handle Responses API with no content gracefully', async () => {
      ...
      const mockFetch = vi.fn().mockResolvedValue({
        ...
        body: new ReadableStream({
          start(controller) {
            controller.enqueue(
              new TextEncoder().encode('data: {"someField":"value"}\n\n'),
            );
            controller.enqueue(new TextEncoder().encode('data: [DONE]\n\n'));
            controller.close();
          },
      ...
      const contentChunks = chunks.filter(
        (c) => c.type === 'text' || c.type === 'reasoning',
      );
      ...
      const mockFetch = vi.fn().mockResolvedValue({
        ...
        body: new ReadableStream({
          start(controller) {
            ...
            controller.enqueue(
              new TextEncoder().encode(
                'data: {"type":"response.audio.delta","delta":"..."}\n\n',
              ),
            );
            controller.enqueue(
              new TextEncoder().encode(
...
describe('GPT-5 streaming event coverage (additional)', () => {
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ...
      body: new ReadableStream({
        start(controller) {
          ...
          controller.enqueue(
            new TextEncoder().encode(
              'data: {"type":"response.text.delta","delta":"The answer is..."}\n\n',
            ),
          );
          controller.enqueue(new TextEncoder().encode('data: [DONE]\n\n'));
          controller.close();
  ...
  it('should handle refusal delta events for GPT-5 and prefix output', async () => {
    ...
    const systemPrompt = 'You are a helpful assistant.';
    const messages: Anthropic.Messages.MessageParam[] = [
      { role: 'user', content: 'Do something disallowed' },
    ];
    const stream = handler.createMessage(systemPrompt, messages);
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ...
      body: new ReadableStream({
        start(controller) {
          ...
          controller.enqueue(
            new TextEncoder().encode(
              'data: {"type":"response.text.delta","delta":"Bad"\n\n',
            ),
          );
  ...
  describe('Codex Mini Model', () => {
    ...
    it('should handle codex-mini-latest streaming response', async () => {
      ...
      expect(textChunks.map((c) => c.text).join('')).toBe(
        'Hello from Codex Mini!',
      );
        ...
        input: [
          {
            role: 'user',
            content: [
              { type: 'input_text', text: 'Write a hello world function' },
            ],
          },
        ],
        stream: true,
