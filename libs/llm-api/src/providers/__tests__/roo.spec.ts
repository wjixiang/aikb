...
import { rooDefaultModelId } from 'agent-lib/types';
...
describe('RooHandler', () => {
  ...
  describe('authentication flow', () => {
    ...
    it('should handle undefined auth service gracefully', () => {
      ...
      const originalGetSessionToken =
        mockGetSessionTokenFn.getMockImplementation();
    ...
    it('should not yield tool call chunks when no tool calls present', async () => {
      mockCreate.mockResolvedValueOnce({
        [Symbol.asyncIterator]: async function* () {
          yield {
            choices: [
              { delta: { content: 'Regular text response' }, index: 0 },
            ],
          };
          yield {
