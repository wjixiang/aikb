...
const { filterMessagesForClaudeCode } = await import(
...
describe('ClaudeCodeHandler', () => {
  ...
  test('should filter messages and call runClaudeCode', async () => {
    ...
    const filteredMessages = [
      { role: 'user' as const, content: 'Hello (filtered)' },
    ];
  ...
  test('should log warning for unsupported tool_use content', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('tool_use is not supported yet'),
    );
