...
describe('Amazon Bedrock Inference Profiles', () => {
  ...
  describe('AWS_INFERENCE_PROFILE_MAPPING constant', () => {
    ...
    it('should be ordered by pattern length (descending)', () => {
      const lengths = AWS_INFERENCE_PROFILE_MAPPING.map(
    ...
    it('should have valid inference profile prefixes', () => {
      AWS_INFERENCE_PROFILE_MAPPING.forEach(
        ([regionPattern, inferenceProfile]) => {
  ...
  describe('getPrefixForRegion function', () => {
    it('should return correct prefix for US government regions', () => {
      ...
      expect(
        (handler as any).constructor.getPrefixForRegion('us-gov-east-1'),
      ).toBe('ug.');
      expect(
        (handler as any).constructor.getPrefixForRegion('us-gov-west-1'),
      ).toBe('ug.');
    });

    it('should return correct prefix for US commercial regions', () => {
      const handler = createHandler();
      expect((handler as any).constructor.getPrefixForRegion('us-east-1')).toBe(
        'us.',
      );
      expect((handler as any).constructor.getPrefixForRegion('us-west-1')).toBe(
        'us.',
      );
      expect((handler as any).constructor.getPrefixForRegion('us-west-2')).toBe(
        'us.',
      );
    });
    ...
    it('should return correct prefix for European regions', () => {
      const handler = createHandler();
      expect((handler as any).constructor.getPrefixForRegion('eu-west-1')).toBe(
        'eu.',
      );
      expect(
        (handler as any).constructor.getPrefixForRegion('eu-central-1'),
      ).toBe('eu.');
      expect(
        (handler as any).constructor.getPrefixForRegion('eu-north-1'),
      ).toBe('eu.');
      expect(
        (handler as any).constructor.getPrefixForRegion('eu-south-1'),
      ).toBe('eu.');
    });
    ...
    it('should return correct prefix for Asia Pacific regions', () => {
      ...
      expect(
        (handler as any).constructor.getPrefixForRegion('ap-southeast-2'),
      ).toBe('au.');
      expect(
        (handler as any).constructor.getPrefixForRegion('ap-southeast-4'),
      ).toBe('au.');
      // Japan regions (Tokyo and Osaka) get jp. prefix
      expect(
        (handler as any).constructor.getPrefixForRegion('ap-northeast-1'),
      ).toBe('jp.');
      expect(
        (handler as any).constructor.getPrefixForRegion('ap-northeast-3'),
      ).toBe('jp.');
      // Other APAC regions get apac. prefix
      expect(
        (handler as any).constructor.getPrefixForRegion('ap-southeast-1'),
      ).toBe('apac.');
      expect(
        (handler as any).constructor.getPrefixForRegion('ap-south-1'),
      ).toBe('apac.');
      expect((handler as any).constructor.getPrefixForRegion('ap-east-1')).toBe(
        'apac.',
      );
    });
    ...
    it('should return correct prefix for Canada regions', () => {
      const handler = createHandler();
      expect(
        (handler as any).constructor.getPrefixForRegion('ca-central-1'),
      ).toBe('ca.');
      expect((handler as any).constructor.getPrefixForRegion('ca-west-1')).toBe(
        'ca.',
      );
    });
    ...
    it('should return correct prefix for South America regions', () => {
      const handler = createHandler();
      expect((handler as any).constructor.getPrefixForRegion('sa-east-1')).toBe(
        'sa.',
      );
    });
    ...
    it('should return undefined for unsupported regions', () => {
      const handler = createHandler();
      expect(
        (handler as any).constructor.getPrefixForRegion('af-south-1'),
      ).toBeUndefined();
      expect(
        (handler as any).constructor.getPrefixForRegion('me-south-1'),
      ).toBeUndefined();
      expect(
        (handler as any).constructor.getPrefixForRegion('cn-north-1'),
      ).toBeUndefined();
      expect(
        (handler as any).constructor.getPrefixForRegion('invalid-region'),
      ).toBeUndefined();
    });
