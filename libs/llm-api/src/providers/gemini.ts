  ...
  implements SingleCompletionHandler
{
  ...
  async *createMessage(
    ...
    const isHybridReasoningModel =
      info.supportsReasoningBudget || info.requiredReasoningBudget;
    ...
    const geminiMessages = messages.filter(
      (message): message is Anthropic.Messages.MessageParam => {
  ...
  override getModel() {
    ...
    let id =
      modelId && modelId in geminiModels
  ...
  private extractCitationsOnly(
    ...
    const citationLinks = sources.map(
  ...
  async completePrompt(prompt: string): Promise<string> {
    try {
      ...
      if (candidate?.groundingMetadata) {
        const citations = this.extractCitationsOnly(
  ...
  public calculateCost({
    ...
    let cacheReadCost =
      cacheReadTokens > 0 ? cacheReadsPrice * (cacheReadTokens / 1_000_000) : 0;
