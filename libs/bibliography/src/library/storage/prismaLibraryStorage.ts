...
import { ItemMetadata, ItemArchive, SearchFilter, Collection, Citation, Author } from "../types";
...
export class PrismaLibraryStorage implements ILibraryStorage {
    ...
    private mapPrismaItemToItemMetadata(item: any): ItemMetadata {
        return {
            ...
            markdownContent: item.markdowns?.content || item.markdown_content || undefined,
            markdownUpdatedDate: item.markdowns?.date_modified || item.markdown_updated_date || undefined,
            archives: item.item_archives?.map(ia => ({
    ...
    // Metadata operations
    async saveMetadata(metadata: ItemMetadata): Promise<ItemMetadata & { id: string; }> {
    ...
    async getMetadataByHash(contentHash: string): Promise<ItemMetadata | null> {
        ...
        return itemArchive?.items ? this.mapPrismaItemToItemMetadata(itemArchive.items) : null;
    }
    ...
    async getCollections(): Promise<Collection[]> {
        ...
        return collections.map(collection => this.mapPrismaCollectionToCollection(collection));
    }
    ...
    async getCitations(itemId: string): Promise<Citation[]> {
        ...
        return citations.map(citation => this.mapPrismaCitationToCitation(citation));
    }
