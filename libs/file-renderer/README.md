...
- **文件存储与管理**: 基于S3兼容对象存储（阿里云OSS、MinIO、AWS S3等）
...
│  │  (S3/OSS/MinIO)  │  │ (File Convert) │  │   (Smart Pagination)    │  │
│  └──────────────────┘  └────────────────┘  └─────────────────────────┘  │
│  ┌──────────────────┐  ┌────────────────┐  ┌─────────────────────────┐  │
...
| 变量名 | 说明 | 默认值 |
|--------|------|--------|
| `S3_FORCE_PATH_STYLE` | 是否使用path-style (MinIO需要true) | `false` |
