...
describe('AlibabaEmbeddingProvider Native API Tests', () => {
  ...
  beforeEach(() => {
    if (!apiKey) {
      console.warn(
        'ALIBABA_API_KEY not found in environment variables. Skipping native API tests.',
      );
    }
  ...
  describe('embed method with real API calls', () => {
    it.skipIf(!apiKey)(
      'should successfully embed a single text',
      async () => {
    ...
    it.skipIf(!apiKey)(
      'should handle array input and return first embedding',
      async () => {
    ...
    it.skipIf(!apiKey)(
      'should return consistent embeddings for identical inputs',
      async () => {
      ...
      45000,
    ); // 45 second timeout for two API calls

    it.skipIf(!apiKey)(
      'should return different embeddings for different inputs',
      async () => {
  ...
  describe('performance tests with real API', () => {
    it.skipIf(!apiKey)(
      'should complete single embedding within reasonable time',
      async () => {
  ...
  describe('embedBatch method with real API calls', () => {
    it.skipIf(!apiKey)(
      'should successfully embed a small batch of texts',
      async () => {
        const testTexts = [
    ...
    it.skipIf(!apiKey)(
      'should handle a batch of 10 texts (maximum batch size)',
      async () => {
        const testTexts = Array.from(
          { length: 10 },
          (_, i) => `Test text ${i + 1} for batch processing`,
        );
    ...
    it.skipIf(!apiKey)(
      'should handle a batch larger than maximum size (15 texts)',
      async () => {
        const testTexts = Array.from(
          { length: 15 },
          (_, i) => `Test text ${i + 1} for large batch processing`,
        );
        ...
        console.log(
          'Large batch (15 texts) processed successfully with automatic batching',
        );
      },
      90000,
    );
    ...
    it.skipIf(!apiKey)(
      'should handle batch with special characters',
      async () => {
        const testTexts = [
    ...
    it.skipIf(!apiKey)(
      'should handle batch with varying text lengths',
      async () => {
        const testTexts = [
          'Short',
    ...
    it.skipIf(!apiKey)(
      'should return consistent embeddings for identical inputs in batch',
      async () => {
        const testTexts = [
        ...
        console.log(
          'Batch consistency test passed - identical inputs produce identical embeddings',
        );
      },
      45000,
    );
    ...
    it.skipIf(!apiKey)(
      'should handle multilingual text in batch',
      async () => {
        const testTexts = [
          'Hello world in English',
          'Bonjour le monde en français',
    ...
    it.skipIf(!apiKey)(
      ...
      async () => {
        const batch1 = Array.from(
          { length: 5 },
          (_, i) => `Concurrent batch 1 - text ${i + 1}`,
        );
        const batch2 = Array.from(
          { length: 5 },
          (_, i) => `Concurrent batch 2 - text ${i + 1}`,
        );
        const batch3 = Array.from(
          { length: 5 },
          (_, i) => `Concurrent batch 3 - text ${i + 1}`,
        );
        ...
        allResults.forEach((results, batchIndex) => {
          ...
          console.log(
            `Concurrent batch ${batchIndex + 1} completed successfully`,
          );
        });

        console.log(
          `3 concurrent batches (15 total embeddings) completed in ${duration}ms`,
        );
      },
      60000,
    );
  ...
  describe('embedBatch error handling with real API', () => {
    it.skipIf(!apiKey)(
      'should handle batch with invalid API key gracefully',
      async () => {
        const invalidProvider = new AlibabaEmbeddingProvider(
          'invalid-api-key',
          2,
    ...
    it.skipIf(!apiKey)(
      'should handle batch with null/undefined API key',
      async () => {
    ...
    it.skipIf(!apiKey)(
      'should handle batch with empty string API key',
      async () => {
