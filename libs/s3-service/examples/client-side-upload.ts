...
import { S3Service, createS3ServiceFromEnv, S3ServiceError, S3ServiceErrorType } from '../src/index';
...
async function batchSignedUrlsExample() {
  ...
  try {
    const files = [
      ...
      { name: 'documents/report.docx', type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' },
...
async function variableExpirationExample() {
  ...
  try {
    ...
    for (const scenario of scenarios) {
      ...
      console.log(`${scenario.name}: ${hours}h ${minutes}m - ${signedUrl.substring(0, 80)}...`);
    }
