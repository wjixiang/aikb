...
async function uploadPdfExample() {
  ...
  try {
    ...
    if (!fs.existsSync(pdfPath)) {
      ...
      console.log(
        'Please update the pdfPath variable to point to a real PDF file',
      );
      return;
    }

    // Upload with custom S3 key
    const result = await s3Service.uploadPdfFromPath(
      pdfPath,
    ...
    const downloadUrl = await s3Service.getSignedDownloadUrl(result.key, {
      expiresIn: 3600,
    });
...
async function uploadMultiplePdfsExample() {
  ...
  try {
    ...
    if (!fs.existsSync(pdfDirectory)) {
      ...
      console.log(
        'Please update the pdfDirectory variable to point to a real directory',
      );
      return;
    }
...
async function pdfWorkflowExample() {
  ...
  try {
    ...
    const uploadResult = await s3Service.uploadPdfFromPath(
      pdfPath,
    ...
    const downloadUrl = await s3Service.getSignedDownloadUrl(uploadResult.key, {
      expiresIn: 1800,
    });
...
async function pdfMetadataExample() {
  ...
  try {
    ...
    const pdfResult = await s3Service.uploadPdfFromPath(
      pdfPath,
    ...
    const metadataDownloadUrl = await s3Service.getSignedDownloadUrl(
      metadataResult.key,
    );
