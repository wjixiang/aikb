...
async function main() {
  ...
  // ============================================================
  // Step 3: Create PubMed Retrieve Agent (with components!)
  // ============================================================
  logger.info('[AgentRuntime Demo] Creating PubMed Retrieve Agent...');

  const pubmedAgentId = await runtime.createAgent({
    agent: {
    ...
    // NEW: Register components directly in createAgent()!
    components: [
  ...
  const analysisAgentId = await runtime.createAgent({
    agent: {
      sop: `You are a paper analysis expert.

Available tools:
- bibliography-search: Search and retrieve papers from PubMed
- runtime-task: Check and report tasks

Your role:
1. Analyze retrieved papers
2. Extract key information (authors, methodology, conclusions)
3. Provide structured summaries

When you receive a task, analyze it and provide detailed insights.`,
...
// Run the demo
main().catch((error) => {
  logger.error({ error }, '[AgentRuntime Demo] Error');
  process.exit(1);
});
