...
export class ToolManager implements IToolManager {
  ...
  registerProvider(provider: IToolProvider): void {
    if (this.providers.has(provider.id)) {
      console.warn(
        `[ToolManager] Provider "${provider.id}" is already registered. Overwriting.`,
      );
    }
    ...
    if (toolsPromise instanceof Promise) {
      ...
      toolsPromise
        .then((tools) => this._registerToolsFromProvider(provider, tools))
        .catch((error) =>
          console.error(
            `[ToolManager] Failed to refresh tools from provider "${provider.id}":`,
            error,
          ),
        );
    } else {
   ...
   */
  private _registerToolsFromProvider(
    ...
    for (const tool of tools) {
      this.toolRegistry.set(tool.toolName, {
        ...
        handler: async (params: any) =>
          provider.executeTool(tool.toolName, params),
      });
    }
  }
