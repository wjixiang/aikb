...
import { ApiStreamChunk, ApiStream } from "llm-api";
...
function createMockXmlToolCallStream(toolName: string, toolArgs: Record<string, any>): ApiStream {
    return (async function* () {
...
function createMockXmlMultipleToolCallStream(tools: Array<{ name: string, args: Record<string, any> }>): ApiStream {
    return (async function* () {
...
function createMockXmlToolCallWithReasoningStream(toolName: string, toolArgs: Record<string, any>, reasoning: string): ApiStream {
    return (async function* () {
        // Start with reasoning
        yield {
            type: 'reasoning',
...
describe('Task Entity Tests', () => {
    ...
    it('should execute a simple task with mocked text stream', async () => {
        ...
        try {
            ...
            console.log('Assistant message content:', newTask.assistantMessageContent);
        } finally {
    ...
    it('should test mock stream directly without complex workflow', async () => {
        ...
        newTask['attemptApiRequest'] = vi.fn().mockReturnValue(createMockTextStream(mockText));
    ...
    it('should handle reasoning stream correctly', async () => {
        ...
        const mockReasoning = "我需要分析用户的问题。用户询问糖尿病的诊断标准，这是一个医学相关的问题。我应该提供准确、权威的医学信息。";
        newTask['attemptApiRequest'] = vi.fn().mockReturnValue(
            createMockReasoningStream(mockReasoning)
        );
        ...
        console.log('Assistant message content with reasoning:', newTask.assistantMessageContent);
    }, 10000);
    ...
    it('should handle mixed stream with text, reasoning, and tool calls', async () => {
        ...
        newTask['attemptApiRequest'] = vi.fn().mockReturnValue(createMockMixedStream());
        ...
        const textBlocks = newTask.assistantMessageContent.filter(block => block.type === 'text');
        const toolUses = newTask.assistantMessageContent.filter(block => block.type === 'tool_use');
    ...
    it('should collect chunks from mock tool call stream directly', async () => {
        ...
        const toolCallPartial = chunks.find(chunk => chunk.type === 'tool_call_partial');
    ...
    it('should handle error stream correctly', async () => {
        ...
        newTask['attemptApiRequest'] = vi.fn().mockReturnValue(
            createMockErrorStream(errorMessage)
        );
    ...
    it('should handle XML tool call stream correctly', async () => {
        ...
        const newTask = new Task('test_task_id_xml_tool', xmlTestApiConfig, 100);
        ...
        try {
            ...
            console.log('XML tool call assistant message content:', newTask.assistantMessageContent);
        } finally {
