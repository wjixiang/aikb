...
describe('getRooModels', () => {
  ...
  it('should set defaultToolProtocol to native when default-native-tools tag is present', async () => {
    ...
    expect(models['test/native-tools-model'].defaultToolProtocol).toBe(
      'native',
    );
  });
  ...
  it('should set defaultToolProtocol to native for all models regardless of tags', async () => {
    ...
    expect(models['test/model-without-tool-tags'].supportsNativeTools).toBe(
      false,
    );
    expect(models['test/model-without-tool-tags'].defaultToolProtocol).toBe(
      'native',
    );
  });
  ...
  it('should apply API-provided settings to model info', async () => {
    ...
    expect(models['test/model-with-settings'].includedTools).toEqual([
      'apply_patch',
    ]);
    expect(models['test/model-with-settings'].excludedTools).toEqual([
      'apply_diff',
      'write_to_file',
    ]);
