...
describe('AnthropicHandler', () => {
  ...
  describe('constructor', () => {
    ...
    it('use apiKey for passing token if anthropicUseAuthToken is not set', () => {
      ...
      expect(mockAnthropicConstructor.mock.calls[0]![0]!.apiKey).toEqual(
        'test-api-key',
      );
      expect(
        mockAnthropicConstructor.mock.calls[0]![0]!.authToken,
      ).toBeUndefined();
    });
    ...
    it('use authToken for passing token if both of anthropicBaseUrl and anthropicUseAuthToken are set', () => {
      ...
      expect(
        mockAnthropicConstructor.mock.calls[0]![0]!.apiKey,
      ).toBeUndefined();
    });
  });
  ...
  describe('completePrompt', () => {
    ...
    it('should handle API errors', async () => {
      mockCreate.mockRejectedValueOnce(
        new Error('Anthropic completion error: API Error'),
      );
      await expect(handler.completePrompt('Test prompt')).rejects.toThrow(
        'Anthropic completion error: API Error',
      );
    });
  ...
  describe('reasoning block filtering', () => {
    ...
    it('should filter out internal reasoning blocks before sending to API', async () => {
      ...
      const calledMessages =
        mockCreate.mock.calls[mockCreate.mock.calls.length - 1][0].messages;
      expect(calledMessages).toHaveLength(3);

      // Check assistant message - should have reasoning block filtered out
      const assistantMessage = calledMessages.find(
    ...
    const messages: Anthropic.Messages.MessageParam[] = [
      {
        role: 'user',
        content: [
          { type: 'text' as const, text: "What's the weather in London?" },
        ],
      },
    ];
