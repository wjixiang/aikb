...
describe('convertToVsCodeLmMessages', () => {
  it('should convert simple string messages', () => {
    ...
    expect((result[0].content[0] as MockLanguageModelTextPart).value).toBe(
      'Hello',
    );
    expect(result[1].role).toBe('assistant');
    expect((result[1].content[0] as MockLanguageModelTextPart).value).toBe(
      'Hi there',
    );
  });
...
describe('extractTextCountFromMessage', () => {
  ...
  it('should extract text from LanguageModelTextPart', () => {
    const mockTextPart = new (vitest.mocked(vscode).LanguageModelTextPart)(
      'Text content',
    );
    const message = {
      role: 'user',
  ...
  it('should extract text from multiple LanguageModelTextParts', () => {
    const mockTextPart1 = new (vitest.mocked(vscode).LanguageModelTextPart)(
      'First part',
    );
    const mockTextPart2 = new (vitest.mocked(vscode).LanguageModelTextPart)(
      'Second part',
    );
    const message = {
      role: 'user',
  ...
  it('should extract text from LanguageModelToolResultPart', () => {
    ...
    const mockToolResultPart = new (vitest.mocked(
      vscode,
    ).LanguageModelToolResultPart)('tool-result-id', [mockTextPart]);
    const message = {
      role: 'user',
  ...
  it('should extract text from LanguageModelToolCallPart without input', () => {
    const mockToolCallPart = new (vitest.mocked(
      vscode,
    ).LanguageModelToolCallPart)('call-id', 'tool-name', {});
    const message = {
      role: 'assistant',
  ...
  it('should extract text from LanguageModelToolCallPart with input', () => {
    ...
    const mockToolCallPart = new (vitest.mocked(
      vscode,
    ).LanguageModelToolCallPart)('call-id', 'calculator', mockInput);
    const message = {
      role: 'assistant',
      content: [mockToolCallPart],
    } as any;
  ...
  it('should extract text from mixed content types', () => {
    ...
    const mockToolResultTextPart = new (vitest.mocked(
      vscode,
    ).LanguageModelTextPart)('Tool result');
    const mockToolResultPart = new (vitest.mocked(
      vscode,
    ).LanguageModelToolResultPart)('result-id', [mockToolResultTextPart]);
    const mockInput = { param: 'value' };
    const mockToolCallPart = new (vitest.mocked(
      vscode,
    ).LanguageModelToolCallPart)('call-id', 'tool', mockInput);
    ...
    const result = extractTextCountFromMessage(message);
    expect(result).toBe(
      `Text contentresult-idTool resulttoolcall-id${JSON.stringify(mockInput)}`,
    );
  });
  ...
  it('should handle ToolResultPart with multiple text parts', () => {
    ...
    const mockToolResultPart = new (vitest.mocked(
      vscode,
    ).LanguageModelToolResultPart)('result-id', [mockTextPart1, mockTextPart2]);
  ...
  it('should handle ToolResultPart with empty parts array', () => {
    const mockToolResultPart = new (vitest.mocked(
      vscode,
    ).LanguageModelToolResultPart)('result-id', []);
