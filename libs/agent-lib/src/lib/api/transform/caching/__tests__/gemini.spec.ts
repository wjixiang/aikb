...
describe('addCacheBreakpoints', () => {
  ...
  it('should add a breakpoint to the Nth user message', () => {
    ...
    expect(messages[frequency - 1].content).toEqual([
      { type: 'text', text: `User message ${frequency - 1}` },
    ]);
  });
  ...
  it('should handle assistant messages correctly when counting user messages', () => {
    ...
    const nthUserMessage = messages.filter((m) => m.role === 'user')[
      frequency - 1
    ];
    ...
    const nPlusOneUserMessage = messages.filter((m) => m.role === 'user')[
      frequency
    ];
    expect(nPlusOneUserMessage.content).toEqual([
      { type: 'text', text: `User message ${frequency + 1}` },
    ]);
  });
  ...
  it('should add breakpoints correctly with frequency 5', () => {
    ...
    expect(messages[11].content).toEqual([
      { type: 'text', text: 'User message 11' },
    ]);
  });
