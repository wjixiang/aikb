...
function asObjectSafe(value: any): object {
  ...
  try {
    ...
    console.warn(
      'Roo Code <Language Model API>: Failed to parse object:',
      error,
    );
    return {};
  }
 ...
 */
export function extractTextCountFromMessage(
      ...
      if (item instanceof vscode.LanguageModelToolCallPart) {
        ...
        if (item.input && Object.keys(item.input).length > 0) {
          try {
            ...
            console.error(
              'Roo Code <Language Model API>: Failed to stringify tool call input:',
              error,
            );
          }
        }
