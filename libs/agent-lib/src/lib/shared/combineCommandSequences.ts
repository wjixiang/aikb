    ...
    // Handle command sequences
      ...
      while (j < messages.length) {
        ...
        if (ask === 'command_output' || say === 'command_output') {
          ...
          const isDuplicate =
            previous && previous.type !== type && previous.text === text;
          ...
          if (text.length > 0 && !isDuplicate) {
            ...
            if (
              previous &&
              combinedText.length >
                combinedText.indexOf(COMMAND_OUTPUT_STRING) +
                  COMMAND_OUTPUT_STRING.length
            ) {
              combinedText += '\n';
            }
            combinedText += text;
