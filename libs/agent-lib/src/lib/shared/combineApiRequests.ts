...
export function combineApiRequests(messages: ClineMessage[]): ClineMessage[] {
  ...
  for (const msg of messages) {
    if (
      msg.type === 'say' &&
      (msg.say === 'api_req_started' || msg.say === 'api_req_finished')
    ) {
  ...
  for (const message of messages) {
    if (
      message.type !== 'say' ||
      (message.say !== 'api_req_started' && message.say !== 'api_req_finished')
    ) {
      result.push(message);
      continue;
    }
    ...
    const startIndex =
      startedIndices.length > 0 ? startedIndices.pop() : undefined;
