...
import type { ModeConfig, PromptComponent } from 'agent-lib/types';
...
describe('isToolAllowedForMode', () => {
  ...
  it('allows unrestricted tools', () => {
    expect(
      isToolAllowedForMode('read_file', 'markdown-editor', customModes),
    ).toBe(true);
    expect(
      isToolAllowedForMode('browser_action', 'markdown-editor', customModes),
    ).toBe(true);
  });
  ...
  describe('file restrictions', () => {
    it('allows editing matching files', () => {
      ...
      const cssResult = isToolAllowedForMode(
        'write_to_file',
        'css-editor',
        customModes,
        undefined,
    ...
    it('applies restrictions to both write_to_file and apply_diff', () => {
      ...
      // Test apply_diff
      const diffResult = isToolAllowedForMode(
        'apply_diff',
        'markdown-editor',
        customModes,
        undefined,
    ...
    it('allows architect mode to edit markdown files only', () => {
      ...
      expect(isToolAllowedForMode('browser_action', 'architect', [])).toBe(
        true,
      );
  ...
  it('handles non-existent modes', () => {
    expect(
      isToolAllowedForMode('write_to_file', 'non-existent', customModes),
    ).toBe(false);
  });
...
describe('FileRestrictionError', () => {
  ...
  describe('getFullModeDetails', () => {
    beforeEach(() => {
      vi.clearAllMocks();
      vi.mocked(addCustomInstructions).mockResolvedValue(
    ...
    it('applies prompt component overrides', async () => {
      ...
      const result = await getFullModeDetails(
        'debug',
        undefined,
        customModePrompts,
      );
...
describe('getModeSelection', () => {
  ...
  test('should return built-in mode details if no overrides', () => {
    ...
    expect(selection.baseInstructions).toBe(
      builtInAskMode.customInstructions || '',
    );
  });
  ...
  test('should prioritize promptComponent for built-in mode if no custom mode exists for that slug', () => {
    ...
    expect(selection.baseInstructions).toBe(
      promptComponentAsk.customInstructions,
    );
  });
  ...
  test('should fall back to default mode if slug does not exist in custom, prompt, or built-in modes', () => {
    ...
    expect(selection.baseInstructions).toBe(
      defaultMode.customInstructions || '',
    );
  });
  ...
  test('handles undefined customInstructions in customMode gracefully', () => {
    ...
    const selection = getModeSelection(
      'no-instr',
      undefined,
      modesWithoutCustomInstructions,
    );
  ...
  test('handles empty or undefined roleDefinition in customMode gracefully', () => {
    ...
    const selection2 = getModeSelection(
      'undefined-role',
      undefined,
      modesWithUndefinedRoleDef,
    );
  ...
  test("customMode's defined properties take precedence, undefined ones in customMode result in ''", () => {
    ...
    const selection = getModeSelection(
      'role-custom',
      promptComponentInstrOnly,
      customModeRoleOnlyList,
    );
  ...
  test("customMode's defined properties take precedence, empty string ones in customMode are used", () => {
    ...
    const selection = getModeSelection(
      'instr-custom',
      promptComponentRoleOnly,
      customModeInstrOnlyList,
    );
    // customMode is chosen
