...
describe('getModelMaxOutputTokens', () => {
  ...
  test('should return modelMaxTokens from settings when reasoning budget is required', () => {
    ...
    expect(getModelMaxOutputTokens({ modelId: 'test', model, settings })).toBe(
      4000,
    );
  });
...
describe('shouldUseReasoningEffort', () => {
  ...
  test('should handle different reasoning effort values', () => {
    ...
    expect(shouldUseReasoningEffort({ model, settings: settingsLow })).toBe(
      true,
    );
    expect(shouldUseReasoningEffort({ model, settings: settingsMedium })).toBe(
      true,
    );
    expect(shouldUseReasoningEffort({ model, settings: settingsHigh })).toBe(
      true,
    );
  });
