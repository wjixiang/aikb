...
export function checkExistKey(config: ProviderSettings | undefined) {
  ...
  if (
    config.apiProvider &&
    ['human-relay', 'fake-ai', 'claude-code', 'qwen-code', 'roo'].includes(
      config.apiProvider,
    )
  ) {
    return true;
  ...
  const providerSecretKeys = SECRET_STATE_KEYS.filter(
    (key) => !GLOBAL_SECRET_KEYS.includes(key as any),
  );
  const hasSecretKey = providerSecretKeys.some(
    (key) => config[key as keyof ProviderSettings] !== undefined,
  );
