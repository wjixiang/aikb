...
export async function autoImportSettings(
  ...
  try {
    ...
    outputChannel.appendLine(
      `[AutoImport] Checking for settings file at: ${resolvedPath}`,
    );
    ...
    if (result.success) {
      outputChannel.appendLine(
        `[AutoImport] Successfully imported settings from ${resolvedPath}`,
      );
      ...
      outputChannel.appendLine(
        `[AutoImport] Failed to import settings: ${result.error}`,
      );

      // Show a warning but don't fail the extension activation
      vscode.window.showWarningMessage(
        t('common:warnings.auto_import_failed', { error: result.error }),
      );
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    outputChannel.appendLine(
      `[AutoImport] Unexpected error during auto-import: ${errorMessage}`,
    );
