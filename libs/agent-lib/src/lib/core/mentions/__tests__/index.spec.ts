...
describe('parseMentions - URL error handling', () => {
  ...
  it('should handle timeout errors with appropriate message', async () => {
    ...
    expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
      'common:errors.url_fetch_error_with_url',
    );
    expect(result).toContain(
      'Error fetching content: Navigation timeout of 30000 ms exceeded',
    );
  });
  ...
  it('should handle DNS resolution errors', async () => {
    ...
    expect(result).toContain(
      'Error fetching content: net::ERR_NAME_NOT_RESOLVED',
    );
  });
  ...
  it('should handle network disconnection errors', async () => {
    ...
    expect(result).toContain(
      'Error fetching content: net::ERR_INTERNET_DISCONNECTED',
    );
  });
  ...
  it('should handle browser launch errors correctly', async () => {
    ...
    vi.mocked(mockUrlContentFetcher.launchBrowser).mockRejectedValue(
      launchError,
    );
    ...
    expect(result).toContain(
      'Error fetching content: Failed to launch browser',
    );
  ...
  it('should successfully fetch URL content when no errors occur', async () => {
    vi.mocked(mockUrlContentFetcher.urlToMarkdown).mockResolvedValue(
      '# Example Content\n\nThis is the content.',
    );
