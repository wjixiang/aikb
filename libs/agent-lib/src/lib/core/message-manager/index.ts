...
import { ClineMessage } from 'agent-lib/types';
...
export class MessageManager {
  ...
  async rewindToTimestamp(
    ...
    if (clineIndex === -1) {
      throw new Error(
        `Message with timestamp ${ts} not found in clineMessages`,
      );
    }
  ...
  private collectRemovedContextEventIds(fromIndex: number): ContextEventIds {
    ...
    for (let i = fromIndex; i < this.task.clineMessages.length; i++) {
      ...
      if (msg.say === 'condense_context' && msg.contextCondense?.condenseId) {
        ...
        console.log(
          `[MessageManager] Found condense_context to remove: ${msg.contextCondense.condenseId}`,
        );
      }

      // Collect truncationIds from sliding_window_truncation events
      if (
        msg.say === 'sliding_window_truncation' &&
        msg.contextTruncation?.truncationId
      ) {
  ...
  private async truncateClineMessages(toIndex: number): Promise<void> {
    await this.task.overwriteClineMessages(
      this.task.clineMessages.slice(0, toIndex),
    );
  }
    ...
    if (removedIds.condenseIds.size > 0) {
      apiHistory = apiHistory.filter((msg) => {
        if (
          msg.isSummary &&
          msg.condenseId &&
          removedIds.condenseIds.has(msg.condenseId)
        ) {
          console.log(
            `[MessageManager] Removing orphaned Summary with condenseId: ${msg.condenseId}`,
          );
          return false;
        }
        return true;
