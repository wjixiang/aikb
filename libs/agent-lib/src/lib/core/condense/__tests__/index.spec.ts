...
vi.mock('../../../api/transform/image-cleaning', () => ({
  maybeRemoveImageBlocks: vi.fn(
    (messages: ApiMessage[], _apiHandler: ApiHandler) => [...messages],
  ),
}));
...
describe('summarizeConversation', () => {
  ...
  it('should summarize conversation and insert summary message', async () => {
    ...
    const condensedMessages = result.messages.filter(
      (m) => m.condenseParent !== undefined,
    );
  ...
  it('should correctly format the request to the API', async () => {
    ...
    const mockCallArgs = (maybeRemoveImageBlocks as Mock).mock
      .calls[0][0] as any[];
      ...
      (m) =>
        ...
        (typeof m.content === 'string'
          ...
          : Array.isArray(m.content) &&
            m.content.some(
              (b: any) => b.type === 'text' && b.text === 'Initial ask',
            )),
    );
  ...
  it('should calculate newContextTokens correctly with systemPrompt', async () => {
    ...
    mockApiHandler.createMessage = vi
      .fn()
      .mockReturnValue(streamWithUsage) as any;
  ...
  it('should return error when new context tokens >= previous context tokens', async () => {
    ...
    mockApiHandler.createMessage = vi
      .fn()
      .mockReturnValue(streamWithLargeTokens) as any;
    ...
    mockApiHandler.countTokens = vi
      .fn()
      .mockImplementation(() => Promise.resolve(200)) as any;
  ...
  it('should successfully summarize when new context tokens < previous context tokens', async () => {
    ...
    mockApiHandler.createMessage = vi
      .fn()
      .mockReturnValue(streamWithSmallTokens) as any;

    // Mock countTokens to return a small value so total is < prevContextTokens
    mockApiHandler.countTokens = vi
      .fn()
      .mockImplementation(() => Promise.resolve(30)) as any;
  ...
  it('should return error when both condensing and main API handlers are invalid', async () => {
    ...
    expect(mockError).toHaveBeenCalledWith(
      expect.stringContaining(
        'Main API handler is also invalid for condensing',
      ),
    );
  ...
  it('should append tool_use blocks to summary message when first kept message has tool_result blocks', async () => {
    ...
    mockApiHandler.countTokens = vi
      .fn()
      .mockImplementation(() => Promise.resolve(50)) as any;
    ...
    const content = summaryMessage!
      .content as Anthropic.Messages.ContentBlockParam[];
    expect(content).toHaveLength(2);
    expect(content[0].type).toBe('text');
    expect((content[0] as Anthropic.Messages.TextBlockParam).text).toBe(
      'Summary of conversation',
    );
    expect(content[1].type).toBe('tool_use');
    expect((content[1] as Anthropic.Messages.ToolUseBlockParam).id).toBe(
      'toolu_123',
    );
    expect((content[1] as Anthropic.Messages.ToolUseBlockParam).name).toBe(
      'read_file',
    );
  ...
  it('should include user tool_result message in summarize request when preserving tool_use blocks', async () => {
    ...
    mockApiHandler.createMessage = vi
      .fn()
      .mockImplementation((_prompt, requestMessagesParam) => {
  ...
  it('should append multiple tool_use blocks for parallel tool calls', async () => {
    ...
    const messages: ApiMessage[] = [
      ...
      { role: 'assistant', content: 'Working...', ts: 2 },
      {
        role: 'assistant',
        content: [
          { type: 'text' as const, text: 'Launching parallel tools' },
          toolUseBlockA,
          toolUseBlockB,
        ],
        ts: 3,
    ...
    expect(summaryMessage).toBeDefined();
    expect(Array.isArray(summaryMessage!.content)).toBe(true);
    const summaryContent = summaryMessage!
      .content as Anthropic.Messages.ContentBlockParam[];
    expect(summaryContent[0]).toEqual({
      type: 'text',
    ...
    expect(preservedToolUses.map((block) => block.id)).toEqual([
      'toolu_parallel_1',
      'toolu_parallel_2',
    ]);
  });
...
describe('summarizeConversation with custom settings', () => {
  ...
  it('should use custom prompt when provided', async () => {
    ...
    const createMessageCalls = (mockMainApiHandler.createMessage as Mock).mock
      .calls;
  ...
  it('should use default systemPrompt when custom prompt is empty or not provided', async () => {
    ...
    let createMessageCalls = (mockMainApiHandler.createMessage as Mock).mock
      .calls;
    expect(createMessageCalls.length).toBe(1);
  ...
  it('should use condensingApiHandler when provided and valid', async () => {
    ...
    expect(
      (mockCondensingApiHandler.createMessage as Mock).mock.calls.length,
    ).toBe(1);
    expect((mockMainApiHandler.createMessage as Mock).mock.calls.length).toBe(
      0,
    );
  });
  ...
  it('should fall back to mainApiHandler if condensingApiHandler is invalid', async () => {
    ...
    expect(mockWarn).toHaveBeenCalledWith(
      expect.stringContaining(
        'Chosen API handler for condensing does not support message creation',
      ),
    );
