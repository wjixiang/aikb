...
export async function summarizeConversation(
  ...
  const summarySliceEnd = includeFirstKeptMessageInSummary
    ? keepStartIndex + 1
    : keepStartIndex;
  const messagesBeforeKeep =
    summarySliceEnd > 0 ? messages.slice(0, summarySliceEnd) : [];
  ...
  const recentSummaryExists = keepMessages.some(
    (message: ApiMessage) => message.isSummary,
  );
  ...
  if (!handlerToUse || typeof handlerToUse.createMessage !== 'function') {
    ...
    if (!handlerToUse || typeof handlerToUse.createMessage !== 'function') {
      ...
      console.error(
        'Main API handler is also invalid for condensing. Cannot proceed.',
      );
  ...
  const newContextTokens =
    outputTokens + (await apiHandler.countTokens(contextBlocks));
...
export function getMessagesSinceLastSummary(
  ...
  if (
    messagesSinceSummary.length > 0 &&
    messagesSinceSummary[0].role !== 'user'
  ) {
...
export function getEffectiveApiHistory(messages: ApiMessage[]): ApiMessage[] {
  ...
  return messages.filter((msg) => {
    ...
    if (
      ...
      existingTruncationIds.has(msg.truncationParent)
    ) {
      return false;
    }
    return true;
