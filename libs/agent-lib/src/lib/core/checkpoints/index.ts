...
export async function getCheckpointService(
  ...
  try {
    ...
    if (!globalStorageDir) {
      log(
        '[Task#getCheckpointService] globalStorageDir not found, disabling checkpoints',
      );
      task.enableCheckpoints = false;
      return undefined;
    }
    ...
    if (task.checkpointServiceInitializing) {
      ...
      await pWaitFor(
        () => {
          ...
          return (
            !!task.checkpointService && !!task?.checkpointService?.isInitialized
          );
        },
...
async function checkGitInstallation(
  ...
  try {
    ...
    if (!gitInstalled) {
      log(
        '[Task#getCheckpointService] Git is not installed, disabling checkpoints',
      );
      task.enableCheckpoints = false;
      task.checkpointServiceInitializing = false;
    ...
    service.on(
      'checkpoint',
      ({ fromHash: from, toHash: to, suppressMessage }) => {
        try {
          sendCheckpointInitWarn(task);
          ...
          task
            ...
            .catch((err) => {
              log(
                "[Task#getCheckpointService] caught unexpected error in say('checkpoint_saved')",
              );
              console.error(err);
            });
        } catch (err) {
          log(
            "[Task#getCheckpointService] caught unexpected error in on('checkpoint'), disabling checkpoints",
          );
          console.error(err);
          task.enableCheckpoints = false;
        }
    ...
    log(
      `[Task#getCheckpointService] Unexpected error during Git check: ${err.message}`,
    );
...
export async function checkpointSave(
  ...
  return service
    ...
    .catch((err) => {
      console.error(
        '[Task#checkpointSave] caught unexpected error, disabling checkpoints',
        err,
      );
      task.enableCheckpoints = false;
    });
  ...
  try {
    ...
    if (mode === 'restore') {
      ...
      const {
        ...
        totalCacheReads,
        totalCost,
      } = getApiMetrics(task.combineMessages(deletedMessages));
...
export async function checkpointDiff(
  ...
  if (['from-init', 'full'].includes(mode) && checkpoints.length < 1) {
    vscode.window.showInformationMessage(
      t('common:errors.checkpoint_no_first'),
    );
    return;
  }
  ...
  switch (mode) {
    case 'checkpoint':
      fromHash = commitHash;
      toHash =
        idx !== -1 && idx < checkpoints.length - 1
          ? checkpoints[idx + 1]
          : undefined;
  ...
  if (!fromHash) {
    vscode.window.showInformationMessage(
      t('common:errors.checkpoint_no_previous'),
    );
    return;
  }
  ...
  try {
    ...
    if (!changes?.length) {
      vscode.window.showInformationMessage(
        t('common:errors.checkpoint_no_changes'),
      );
      return;
    }
    ...
    await vscode.commands.executeCommand(
      ...
      changes.map((change) => [
        ...
        vscode.Uri.parse(
          `${DIFF_VIEW_URI_SCHEME}:${change.paths.relative}`,
        ).with({
