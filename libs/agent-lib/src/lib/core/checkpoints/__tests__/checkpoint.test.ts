...
describe('Checkpoint functionality', () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(
      checkpointsModule.RepoPerTaskCheckpointService.create,
    ).mockReturnValue(mockCheckpointService);
  });
  ...
  describe('checkpointSave', () => {
    ...
    it('should preserve checkpoint data through message deletion flow', async () => {
      ...
      expect(mockTask.pendingUserMessageCheckpoint.hash).toBe(
        'test-commit-hash',
      );
  ...
  describe('checkpointRestore', () => {
    ...
    it('should restore checkpoint for delete operation', async () => {
      ...
      expect(mockCheckpointService.restoreCheckpoint).toHaveBeenCalledWith(
        'abc123',
      );
    ...
    it('should disable checkpoints on error', async () => {
      mockCheckpointService.restoreCheckpoint.mockRejectedValue(
  ...
  describe('getCheckpointService', () => {
    ...
    it('should create new service if none exists', async () => {
      ...
      const checkpointsModule = await import('../../../services/checkpoints');
      expect(
        vi.mocked(checkpointsModule.RepoPerTaskCheckpointService.create),
  ...
  describe('getCheckpointService - initialization timeout behavior', () => {
    it('should send warning message when initialization is slow', async () => {
      ...
      const simulateConditionCheck = (elapsedMs: number) => {
        ...
        if (!warningShown && elapsedMs >= 5000) {
          ...
          provider?.postMessageToWebview({
            ...
            checkpointWarning: i18nModule.t(
        ...
        return (
          !!mockTask.checkpointService &&
          !!mockTask.checkpointService.isInitialized
        );
      };
      ...
      expect(mockProvider.postMessageToWebview).toHaveBeenCalledWith({
        type: 'checkpointInitWarning',
        checkpointWarning:
          'Checkpoint initialization is taking longer than 5 seconds...',
      });
    ...
    it('should use correct i18n keys for warning messages', async () => {
      ...
      const warningMessage = i18nModule.t(
        'common:errors.wait_checkpoint_long_time',
        { timeout: 5 },
      );
      ...
      const errorMessage = i18nModule.t(
        'common:errors.init_checkpoint_fail_long_time',
        { timeout: 30 },
      );
      expect(errorMessage).toBe(
        'Checkpoint initialization failed after 30 seconds',
      );
    });
  });
});
