...
describe('RooIgnoreController', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .rooignore patterns on initialization when file exists', async () => {
      ...
      expect(controller.rooIgnoreContent).toBe(
        'node_modules\n.git\nsecrets.json',
      );
    ...
    it('should handle errors when loading .rooignore', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Unexpected error loading .rooignore:',
        expect.any(Error),
      );
  ...
  describe('validateAccess', () => {
    ...
    it('should allow all access when no .rooignore content', async () => {
      ...
      expect(emptyController.validateAccess('node_modules/package.json')).toBe(
        true,
      );
      expect(emptyController.validateAccess('secrets/api-keys.json')).toBe(
        true,
      );
  ...
  describe('validateCommand', () => {
    ...
    it('should block file reading commands accessing ignored files', () => {
      ...
      expect(controller.validateCommand('grep pattern .git/config')).toBe(
        '.git/config',
      );
    ...
    it('should handle command arguments and flags correctly', () => {
      // Command with flags
      expect(
        controller.validateCommand('cat -n node_modules/package.json'),
      ).toBe('node_modules/package.json');

      // Command with multiple files (only first ignored file is returned)
      expect(
        controller.validateCommand(
          'grep pattern src/app.ts node_modules/index.js',
        ),
      ).toBe('node_modules/index.js');

      // Command with PowerShell parameter style
      expect(
        controller.validateCommand('Get-Content -Path secrets/api-keys.json'),
      ).toBe('secrets/api-keys.json');
      ...
      expect(
        controller.validateCommand(
          'Select-String -Path secrets/api-keys.json -Pattern key',
        ),
      ).toBe('secrets/api-keys.json');
    });
    ...
    it('should allow all commands when no .rooignore exists', async () => {
      ...
      expect(
        emptyController.validateCommand('cat node_modules/package.json'),
      ).toBeUndefined();
      expect(
        emptyController.validateCommand('grep pattern .git/config'),
      ).toBeUndefined();
    });
  });
