...
import type { ExperimentId } from 'agent-lib/types';
import { DEFAULT_TERMINAL_OUTPUT_CHARACTER_LIMIT } from 'agent-lib/types';
...
export async function getEnvironmentDetails(
  ...
  if (includeCurrentTime) {
    ...
    const timeZoneOffsetMinutes = Math.abs(
      Math.round((Math.abs(timeZoneOffset) - timeZoneOffsetHours) * 60),
    );
  ...
  if (isBrowserActive) {
    ...
    const configuredViewport =
      (state?.browserViewportSize as string | undefined) ?? '900x600';
