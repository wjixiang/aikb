...
describe('getEnvironmentDetails', () => {
  ...
  beforeEach(() => {
    ...
    (formatResponse.formatFilesList as Mock).mockReturnValue(
      'file1.ts\nfile2.ts',
    );
    (arePathsEqual as Mock).mockReturnValue(false);
    (Terminal.compressTerminalOutput as Mock).mockImplementation(
  ...
  it('should include recently modified files if any', async () => {
    (
      mockCline.fileContextTracker!.getAndClearRecentlyModifiedFiles as Mock
    ).mockReturnValue(['modified1.ts', 'modified2.ts']);
  ...
  it('should include inactive terminals with output', async () => {
    ...
    (TerminalRegistry.getTerminals as Mock).mockImplementation(
      (active: boolean) => (active ? [] : [mockInactiveTerminal]),
    );
  ...
  it('should include working directory for terminals', async () => {
    ...
    expect(result).toContain(
      '### Original command: `cd /some/path && npm start`',
    );
  ...
  it('should handle errors gracefully', async () => {
    ...
    await expect(
      getEnvironmentDetails(mockCline as Task),
  ...
  it('should include Browser Session Status with current viewport when active', async () => {
    (mockCline.browserSession as any).isSessionActive = vi
      .fn()
      .mockReturnValue(true);
    (mockCline.browserSession as any).getViewportSize = vi
      .fn()
