...
import { ANTHROPIC_DEFAULT_MAX_TOKENS } from 'agent-lib/types';
...
export function truncateConversation(
  ...
  const indicesToTruncate = new Set(
    visibleIndices.slice(1, messagesToRemove + 1),
  );
  ...
  const firstKeptVisibleIndex =
    visibleIndices[messagesToRemove + 1] ?? taggedMessages.length;
...
export function willManageContext({
  ...
  if (!autoCondenseContext) {
    ...
    const allowedTokens =
      contextWindow * (1 - TOKEN_BUFFER_PERCENTAGE) - reservedTokens;
