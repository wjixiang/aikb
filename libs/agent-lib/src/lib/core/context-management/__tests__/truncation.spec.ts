...
describe('Non-Destructive Sliding Window Truncation', () => {
  ...
  describe('truncateConversation()', () => {
    it('should tag messages with truncationParent instead of deleting', () => {
      ...
      const taggedMessages = result.messages.filter(
        (msg) => msg.truncationParent,
      );
  ...
  describe('getEffectiveApiHistory()', () => {
    it('should filter out truncated messages when truncation marker exists', () => {
      ...
      for (const msg of effective) {
        if (msg.truncationParent) {
          throw new Error(
            'Message with truncationParent should be filtered out',
          );
        }
      }
    });
    ...
    it('should include truncated messages when truncation marker is removed', () => {
      ...
      const messagesWithoutMarker = truncationResult.messages.filter(
        (msg) => !msg.isTruncationMarker,
      );
    ...
    it('should handle both condenseParent and truncationParent filtering', () => {
      ...
      const hasCondensedMessage = effective.some(
        (msg) => msg.condenseParent === 'condense-1',
      );
    ...
    it('should handle both condenseParent and truncationParent cleanup', () => {
      const messagesWithBoth: ApiMessage[] = [
        { role: 'user', content: 'Initial', ts: 1000 },
        {
          ...
          content: 'Msg 1',
          ts: 1100,
          condenseParent: 'orphan-condense',
        },
        {
          role: 'user',
          content: 'Msg 2',
          ts: 1200,
          truncationParent: 'orphan-truncation',
        },
    ...
    it('should preserve valid parent references', () => {
      const messagesWithValidParents: ApiMessage[] = [
        ...
        },
        {
          ...
          content: 'Msg 2',
          ts: 1200,
          truncationParent: 'valid-truncation',
        },
        {
          role: 'assistant',
  ...
  describe('Rewind past truncation integration', () => {
    it('should restore hidden messages when rewinding past truncation point', () => {
      ...
      const effectiveBeforeRewind = getEffectiveApiHistory(
      ...
      const markerIndex = truncationResult.messages.findIndex(
        (msg) => msg.isTruncationMarker,
      );
      const messagesAfterRewind = truncationResult.messages.slice(
        0,
        markerIndex,
      );
    ...
    it('should handle multiple truncations correctly', () => {
      ...
      const effectiveAfterFirst = getEffectiveApiHistory(
      ...
      const secondTruncation = truncateConversation(
        moreMessages,
        0.5,
        'task-1',
      );
      ...
      const effectiveAfterSecond = getEffectiveApiHistory(
        secondTruncation.messages,
      );
      ...
      const firstMarker = secondTruncation.messages.find(
        (msg) =>
          ...
          msg.truncationId === firstTruncation.truncationId,
      );
      const secondMarker = secondTruncation.messages.find(
        (msg) =>
          msg.isTruncationMarker &&
          msg.truncationId === secondTruncation.truncationId,
      );
    ...
    it('should handle rewinding when second truncation affects first truncation marker', () => {
      ...
      const afterSecondRewind = secondTruncation.messages.slice(
        0,
        secondMarkerIndex,
      );
