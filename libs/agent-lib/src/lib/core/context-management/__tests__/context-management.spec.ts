...
describe('Context Management', () => {
  ...
  describe('estimateTokenCount', () => {
    ...
    it('should estimate tokens for text blocks', async () => {
      ...
      const longerResult = await estimateTokenCount(
        longerContent,
        mockApiHandler,
      );
    ...
    it('should estimate tokens for image blocks based on data size', async () => {
      ...
      const smallImageTokens = await estimateTokenCount(
        smallImage,
        mockApiHandler,
      );
      const largerImageTokens = await estimateTokenCount(
        largerImage,
        mockApiHandler,
      );
    ...
    it('should estimate tokens for mixed content blocks', async () => {
      ...
      const imageOnlyResult = await estimateTokenCount(
        imageOnlyContent,
        mockApiHandler,
      );
  ...
  describe('manageContext', () => {
    ...
    it('should consider incoming content when deciding to truncate', async () => {
      ...
      const smallContentTokens = await estimateTokenCount(
        smallContent,
        mockApiHandler,
      );
      ...
      const baseTokensForSmall =
        availableTokens - smallContentTokens - dynamicBuffer - 10;
      ...
      const largeContentTokens = await estimateTokenCount(
        largeContent,
        mockApiHandler,
      );
      ...
      const baseTokensForLarge =
        availableTokens - Math.floor(largeContentTokens / 2);
      ...
      expect(resultWithLarge.prevContextTokens).toBe(
        baseTokensForLarge + largeContentTokens,
      );
      ...
      const veryLargeContentTokens = await estimateTokenCount(
        veryLargeContent,
        mockApiHandler,
      );
      ...
      const baseTokensForVeryLarge =
        availableTokens - Math.floor(veryLargeContentTokens / 2);
      ...
      expect(resultWithVeryLarge.messages).not.toEqual(
      ...
      expect(resultWithVeryLarge.prevContextTokens).toBe(
        baseTokensForVeryLarge + veryLargeContentTokens,
      );
    });
