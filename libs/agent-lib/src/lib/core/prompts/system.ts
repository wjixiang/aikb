...
async function generatePrompt(
  ...
  if (!context) {
    throw new Error(
      'Extension context is required for generating system prompt',
    );
  }
...
export const SYSTEM_PROMPT = async (
  ...
  const fileCustomSystemPrompt = await loadSystemPromptFile(
    cwd,
    mode,
    variablesForPrompt,
  );
  ...
  if (fileCustomSystemPrompt) {
    const { roleDefinition, baseInstructions: baseInstructionsForFile } =
      getModeSelection(mode, promptComponent, customModes);
