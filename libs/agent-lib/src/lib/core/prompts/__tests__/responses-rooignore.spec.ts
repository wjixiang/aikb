...
import {
  RooIgnoreController,
  LOCK_TEXT_SYMBOL,
} from '../../ignore/RooIgnoreController';
...
describe('RooIgnore Response Formatting', () => {
  ...
  describe('formatResponse.rooIgnoreError', () => {
    ...
    it('should format error message for ignored files', () => {
      const errorMessage = formatResponse.rooIgnoreError(
        'secrets/api-keys.json',
      );

      // Verify error message format
      expect(errorMessage).toContain(
        'Access to secrets/api-keys.json is blocked by the .rooignore file settings',
      );
      expect(errorMessage).toContain(
        'continue in the task without using this file',
      );
      expect(errorMessage).toContain(
        'ask the user to update the .rooignore file',
      );
    });
  ...
  describe('formatResponse.formatFilesList with RooIgnoreController', () => {
    ...
    it('should format files list with lock symbols for ignored files', async () => {
      ...
      expect(result).toMatch(
        new RegExp(`${LOCK_TEXT_SYMBOL}.*node_modules/package.json`, 'i'),
      );
      expect(result).toMatch(
        new RegExp(`${LOCK_TEXT_SYMBOL}.*\\.git/HEAD`, 'i'),
      );
      expect(result).toMatch(
        new RegExp(`${LOCK_TEXT_SYMBOL}.*secrets/keys.json`, 'i'),
      );
