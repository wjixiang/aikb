...
import { ModeConfig } from 'agent-lib/types';
...
const customInstructionsMock = vi.mocked(
  await import('../sections/custom-instructions'),
);
...
vi.mock('vscode', () => ({
  ...
  workspace: {
    ...
    getWorkspaceFolder: vi
      .fn()
      .mockReturnValue({ uri: { fsPath: '/test/path' } }),
  },
  window: {
...
describe('addCustomInstructions', () => {
  ...
  it('should prioritize mode-specific rules for code mode', async () => {
    ...
    expect(instructions).toMatchFileSnapshot(
      './__snapshots__/add-custom-instructions/code-mode-rules.snap',
    );
  });
  ...
  it('should prioritize mode-specific rules for ask mode', async () => {
    ...
    expect(instructions).toMatchFileSnapshot(
      './__snapshots__/add-custom-instructions/ask-mode-rules.snap',
    );
  });
  ...
  it('should prioritize mode-specific instructions after global ones', async () => {
    ...
    const globalIndex = instructionParts.findIndex((part) =>
      part.includes('First instruction'),
    );
    const modeSpecificIndex = instructionParts.findIndex((part) =>
      part.includes('Second instruction'),
    );
