...
import { isLanguage } from 'agent-lib/types';
...
async function readTextFilesFromDirectory(
  ...
  try {
    ...
    const filteredFiles = fileContents.filter(
      (item): item is { filename: string; content: string; sortKey: string } =>
        item !== null,
    );
...
export async function addCustomInstructions(
  ...
  if (
    typeof modeCustomInstructions === 'string' &&
