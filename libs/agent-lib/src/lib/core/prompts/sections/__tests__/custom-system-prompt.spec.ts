...
describe('loadSystemPromptFile', () => {
  ...
  it('should re-throw unexpected errors from readFile', async () => {
    ...
    await expect(
      loadSystemPromptFile(mockCwd, mockMode, mockVariables),
  ...
  it('should handle mixed used workspace and unused variables', async () => {
    const template =
      'Workspace: {{workspace}}, Unused: {{unusedVar}}, Another: {{another}}';
    mockedReadFile.mockResolvedValue(template);
    ...
    expect(result).toBe(
      'Workspace: /path/to/workspace, Unused: {{unusedVar}}, Another: {{another}}',
    );
    expect(mockedReadFile).toHaveBeenCalledTimes(1);
    expect(mockedReadFile).toHaveBeenCalledWith(expectedFilePath, 'utf-8');
  });
  ...
  it('should handle templates with placeholders not present in variables', async () => {
    const template =
      'Workspace: {{workspace}}, Missing: {{missingPlaceholder}}';
    mockedReadFile.mockResolvedValue(template);
    ...
    expect(result).toBe(
      'Workspace: /path/to/workspace, Missing: {{missingPlaceholder}}',
    );
    expect(mockedReadFile).toHaveBeenCalledTimes(1);
    expect(mockedReadFile).toHaveBeenCalledWith(expectedFilePath, 'utf-8');
  });
