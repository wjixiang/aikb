...
describe('custom-instructions path detection', () => {
  it('should use exact path comparison instead of string includes', () => {
    ...
    // Test the new logic
    const isGlobalForGlobalDir =
      path.resolve(globalRooDir) === path.resolve(globalRooDir);
    const isGlobalForProjectDir =
      path.resolve(projectRooDir) === path.resolve(globalRooDir);
    ...
    const oldLogicGlobal = globalRooDir.includes(
      path.join(fakeHomeDir, '.roo'),
    );
    const oldLogicProject = projectRooDir.includes(
      path.join(fakeHomeDir, '.roo'),
    );
