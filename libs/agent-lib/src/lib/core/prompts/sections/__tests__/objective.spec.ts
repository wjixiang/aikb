...
describe('getObjectiveSection', () => {
  ...
  it('should include analysis guidance', () => {
    ...
    expect(objective).toContain(
      'analyze the file structure provided in environment_details',
    );
    expect(objective).toContain(
      'think about which of the provided tools is the most relevant',
    );
  });
  ...
  it('should include parameter inference guidance', () => {
    ...
    expect(objective).toContain(
      'DO NOT invoke the tool (not even with fillers for the missing params)',
    );
