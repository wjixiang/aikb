...
describe('loadRuleFiles', () => {
  ...
  it('should read files from nested subdirectories in .roo/rules/', async () => {
    ...
    readFileMock.mockImplementation((filePath: PathLike) => {
      ...
      if (
        normalizedPath === '/fake/path/.roo/rules/subdir/subdir2/nested2.txt'
      ) {
...
describe('addCustomInstructions', () => {
  ...
  it('should follow symlinks when loading AGENTS.md', async () => {
    ...
    expect(readFileMock).toHaveBeenCalledWith(
      expect.stringContaining('actual-agents-file.md'),
      'utf-8',
    );
  });
  ...
  it('should handle AGENTS.md as a regular file when not a symlink', async () => {
    ...
    expect(readlinkMock).not.toHaveBeenCalledWith(
      expect.stringContaining('AGENTS.md'),
    );
...
describe('Rules directory reading', () => {
  it.skipIf(process.platform === 'win32')(
    'should follow symbolic links in the rules directory',
    async () => {
      // Simulate .roo/rules directory exists
      statMock.mockResolvedValueOnce({
        isDirectory: vi.fn().mockReturnValue(true),
      ...
      expect(readlinkMock).toHaveBeenCalledWith(
        '/fake/path/.roo/rules/link.txt',
      );
      expect(readlinkMock).toHaveBeenCalledWith(
        '/fake/path/.roo/rules/link_dir',
      );
  ...
  it.skipIf(process.platform === 'win32')(
    'should correctly format multiple files from directory',
    async () => {
      // Simulate .roo/rules directory exists
      statMock.mockResolvedValueOnce({
        isDirectory: vi.fn().mockReturnValue(true),
