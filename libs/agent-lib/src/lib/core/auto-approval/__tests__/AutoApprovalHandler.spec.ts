import { GlobalState, ClineMessage } from 'agent-lib/types';
...
describe('AutoApprovalHandler', () => {
  ...
  describe('checkAutoApprovalLimits', () => {
    it('should proceed when no limits are set', async () => {
      ...
      const result = await handler.checkAutoApprovalLimits(
        mockState,
        messages,
        mockAskForApproval,
      );
    ...
    it('should check request limit before cost limit', async () => {
      ...
      const result1 = await handler.checkAutoApprovalLimits(
        mockState,
        messages,
        mockAskForApproval,
      );
      ...
      const result2 = await handler.checkAutoApprovalLimits(
        mockState,
        messages,
        mockAskForApproval,
      );
  ...
  describe('request limit handling', () => {
    ...
    it('should calculate request count from messages', async () => {
      ...
      await handler.checkAutoApprovalLimits(
        mockState,
        messages,
        mockAskForApproval,
      );
  ...
  describe('cost limit handling', () => {
    ...
    it('should handle floating-point precision correctly', async () => {
      ...
      mockAskForApproval.mockResolvedValue({ response: 'yesButtonClicked' });
      const result3 = await handler.checkAutoApprovalLimits(
        mockState,
        messages,
        mockAskForApproval,
      );
  ...
  describe('combined limits', () => {
    it('should handle both request and cost limits', async () => {
      ...
      let result = await handler.checkAutoApprovalLimits(
        mockState,
        messages,
        mockAskForApproval,
      );
      ...
      result = await handler.checkAutoApprovalLimits(
        mockState,
        messages,
        mockAskForApproval,
      );
