...
import type { GlobalState, ProviderSettings, ModelInfo } from 'agent-lib/types';
...
vi.mock('../../../utils/storage', () => ({
  getTaskDirectoryPath: vi
    .fn()
    .mockImplementation((globalStoragePath, taskId) =>
      Promise.resolve(`${globalStoragePath}/tasks/${taskId}`),
    ),
  getSettingsDirectoryPath: vi
    .fn()
    .mockImplementation((globalStoragePath) =>
      Promise.resolve(`${globalStoragePath}/settings`),
    ),
}));
...
describe('Cline', () => {
  ...
  beforeEach(() => {
    ...
    mockProvider.getTaskWithId = vi.fn().mockImplementation(async (id) => ({
      ...
      apiConversationHistoryFilePath:
        '/mock/storage/path/tasks/123/api_conversation_history.json',
  ...
  describe('getEnvironmentDetails', () => {
    describe('API conversation handling', () => {
      ...
      it.skip('should handle image blocks based on model capabilities', async () => {
        ...
        vi.spyOn(clineWithImages.api, 'createMessage').mockImplementation(
          imagesSpy,
        );
        vi.spyOn(clineWithoutImages.api, 'createMessage').mockImplementation(
          noImagesSpy,
        );
        ...
        await clineWithImages.recursivelyMakeClineRequests([
          { type: 'text', text: 'test request' },
        ]);
        await clineWithoutImages.recursivelyMakeClineRequests([
          { type: 'text', text: 'test request' },
        ]);
        ...
        if (imagesCalls[0]?.[1]?.[0]?.content) {
          ...
          expect(imagesCalls[0][1][0].content[1]).toHaveProperty(
            'type',
            'image',
          );
        }
      ...
      it.skip('should handle API retry with countdown', async () => {
        ...
        vi.spyOn(await import('delay'), 'default').mockImplementation(
          mockDelay,
        );
        ...
        const errorMessage = saySpy.mock.calls.find((call) =>
          call[1]?.includes(mockError.message),
        )?.[1];
      ...
      describe('processUserContentMentions', () => {
        it('should process mentions in task and feedback tags', async () => {
          ...
          const toolResult1 =
            processedContent[2] as Anthropic.ToolResultBlockParam;
          const content1 = Array.isArray(toolResult1.content)
            ? toolResult1.content[0]
            : toolResult1.content;
          ...
          const toolResult2 =
            processedContent[3] as Anthropic.ToolResultBlockParam;
          const content2 = Array.isArray(toolResult2.content)
            ? toolResult2.content[0]
            : toolResult2.content;
    ...
    describe('Subtask Rate Limiting', () => {
      ...
      it('should not apply rate limiting if enough time has passed', async () => {
        ...
        const mockTime =
          performance.now() + (mockApiConfig.rateLimitSeconds + 1) * 1000;
    ...
    describe('Dynamic Strategy Selection', () => {
      ...
      it('should use MultiSearchReplaceDiffStrategy by default', async () => {
        ...
        expect(task.diffStrategy).toBeInstanceOf(
    ...
    describe('getApiProtocol', () => {
      it('should determine API protocol based on provider and model', async () => {
        ...
        expect(openrouterClaudeTask.apiConfiguration.apiProvider).toBe(
          'openrouter',
        );
        ...
        expect(openrouterGptTask.apiConfiguration.apiProvider).toBe(
          'openrouter',
        );
      ...
      it('should handle edge cases for API protocol detection', async () => {
        ...
        expect(
          undefinedProviderTask.apiConfiguration.apiProvider,
        ).toBeUndefined();
    ...
    describe('submitUserMessage', () => {
      ...
      it('should handle undefined provider gracefully', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          '[Task#submitUserMessage] Provider reference lost',
        );
        expect(mockProvider.postMessageToWebview).not.toHaveBeenCalled();
  ...
  describe('abortTask', () => {
    ...
    it('should handle errors during disposal gracefully', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Error during task'),
        mockError,
      );
    ...
    describe('Stream Failure Retry', () => {
      it('should not abort task on stream failure, only on user cancellation', async () => {
        ...
        const abortTaskSpy = vi
          .spyOn(task, 'abortTask')
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          expect.stringContaining('Stream failed, will retry'),
        );
    ...
    describe('cancelCurrentRequest', () => {
      it('should cancel the current HTTP request via AbortController', () => {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('Aborting current HTTP request'),
        );
      ...
      it('should be called during dispose', () => {
        ...
        vi.spyOn(task, 'removeAllListeners').mockImplementation(
          () => task as any,
        );
...
describe('Queued message processing after condense', () => {
  ...
  it('does not cross-drain queues between separate tasks', async () => {
    ...
    const spyA = vi
      .spyOn(taskA, 'submitUserMessage')
      .mockResolvedValue(undefined);
    const spyB = vi
      .spyOn(taskB, 'submitUserMessage')
      .mockResolvedValue(undefined);
