...
import type { ProviderSettings, ModelInfo } from 'agent-lib/types';
...
describe('Task reasoning preservation', () => {
  ...
  it('should append reasoning to assistant message when preserveReasoning is true', async () => {
    ...
    const addToApiHistorySpy = vi.spyOn(
      task as any,
    ...
    expect(
      (task.apiConversationHistory[0].content[0] as { text: string }).text,
    ).toContain('<think>');
    expect(
      (task.apiConversationHistory[0].content[0] as { text: string }).text,
    ).toContain('</think>');
    expect(
      (task.apiConversationHistory[0].content[0] as { text: string }).text,
    ).toContain('Here is my response to your question.');
    expect(
      (task.apiConversationHistory[0].content[0] as { text: string }).text,
    ).toContain('Let me think about this step by step. First, I need to...');
  });
  ...
  it('should NOT append reasoning to assistant message when preserveReasoning is false', async () => {
    ...
    expect(
      (task.apiConversationHistory[0].content[0] as { text: string }).text,
    ).toBe('Here is my response to your question.');
    expect(
      (task.apiConversationHistory[0].content[0] as { text: string }).text,
    ).not.toContain('<think>');
  });
  ...
  it('should embed encrypted reasoning as first assistant content block', async () => {
    ...
    (task as any).saveApiConversationHistory = vi
      .fn()
  ...
  it('should store plain text reasoning from streaming for all providers', async () => {
    ...
    const reasoningText =
      "Let me analyze this carefully. First, I'll consider the requirements...";
