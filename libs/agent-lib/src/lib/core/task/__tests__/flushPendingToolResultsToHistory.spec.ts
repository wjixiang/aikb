...
import type { GlobalState, ProviderSettings } from 'agent-lib/types';
...
describe('flushPendingToolResultsToHistory', () => {
  ...
  it('should add timestamp to saved messages', async () => {
    ...
    expect((task.apiConversationHistory[0] as any).ts).toBeGreaterThanOrEqual(
      beforeTs,
    );
    expect((task.apiConversationHistory[0] as any).ts).toBeLessThanOrEqual(
      afterTs,
    );
  });
});
