...
describe('Native Tools Filtering by Mode', () => {
  describe('attemptApiRequest native tool filtering', () => {
    it('should filter native tools based on mode restrictions', async () => {
      ...
      const { TOOL_GROUPS, ALWAYS_AVAILABLE_TOOLS } = await import(
        '../../../shared/tools'
      );
      ...
      architectMode.groups.forEach((groupEntry) => {
        const groupName =
          typeof groupEntry === 'string' ? groupEntry : groupEntry[0];
        const toolGroup = TOOL_GROUPS[groupName];
        if (toolGroup) {
          toolGroup.tools.forEach((tool) => {
            if (
              isToolAllowedForMode(tool as any, 'architect', [architectMode])
            ) {
    ...
    it('should filter MCP tools based on use_mcp_tool permission', async () => {
      ...
      expect(
        isToolAllowedForMode('use_mcp_tool', 'test-mode-with-mcp', [
          modeWithMcp,
        ]),
      ).toBe(true);
      ...
      expect(
        isToolAllowedForMode('use_mcp_tool', 'test-mode-no-mcp', [
          modeWithoutMcp,
        ]),
      ).toBe(false);
    });
    ...
    it('should always include always-available tools regardless of mode', async () => {
      ...
      ALWAYS_AVAILABLE_TOOLS.forEach((tool) => {
        expect(
          isToolAllowedForMode(tool as any, 'restrictive', [restrictiveMode]),
        ).toBe(true);
      });
    });
  });
