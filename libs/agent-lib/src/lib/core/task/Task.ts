...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
  ...
  constructor({
    ...
    this.assistantMessageParser =
      toolProtocol !== 'native' ? new AssistantMessageParser() : undefined;
  ...
  public updateApiConfiguration(newApiConfiguration: ProviderSettings): void {
    ...
    const parserStateCorrect =
      (shouldUseXmlParser && this.assistantMessageParser) ||
      (!shouldUseXmlParser && !this.assistantMessageParser);
  ...
  private async resumeTaskFromHistory() {
    ...
    if (!useNative) {
      const conversationWithoutToolBlocks = existingApiConversationHistory.map(
        (message) => {
          if (Array.isArray(message.content)) {
            const newContent = message.content.map((block) => {
              if (block.type === 'tool_use') {
                ...
                const formattedText = formatToolInvocation(
                  block.name,
                  params,
                  protocol,
                );
  ...
  public async abortTask(isAbandoned = false) {
    ...
    try {
      ...
      console.error(
        `Error saving messages during abort for task ${this.taskId}.${this.instanceId}:`,
        error,
      );
    }
  }
  ...
  public async *attemptApiRequest(retryAttempt: number = 0): ApiStream {
    ...
    const messagesSinceLastSummary =
      getMessagesSinceLastSummary(effectiveHistory);
    ...
    try {
      ...
      if (
        ...
        retryAttempt < MAX_CONTEXT_WINDOW_RETRIES
      ) {
        console.warn(
          `[Task#${this.taskId}] Context window exceeded for model ${this.api.getModel().id}. ` +
    ...
    for (const msg of messages) {
      ...
      if (msg.role === 'assistant') {
        ...
        const hasEncryptedReasoning =
          ...
          typeof (first as any).encrypted_content === 'string';
  ...
  private broadcastBrowserSessionUpdate(): void {
    ...
    try {
      ...
      panelManager
        .updateBrowserSession(browserSessionMessages, isBrowserSessionActive)
        .catch((error: Error) => {
