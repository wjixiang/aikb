...
import type { ToolName, ClineAsk, ToolProgressStatus } from 'agent-lib/types';
...
import {
  getSimpleReadFileToolDescription,
  simpleReadFileTool,
} from '../tools/simpleReadFileTool';
import { shouldUseSingleFileRead, TOOL_PROTOCOL } from 'agent-lib/types';
...
} from '../tools/AttemptCompletionTool';
...
import { isNativeProtocol } from 'agent-lib/types';
...
export async function presentAssistantMessage(cline: Task) {
  if (cline.abort) {
    throw new Error(
      `[Task#presentAssistantMessage] task ${cline.taskId}.${cline.instanceId} aborted`,
    );
  }
  ...
  switch (block.type) {
    case 'mcp_tool_use': {
      ...
      const toolDescription = () =>
        `[mcp_tool: ${mcpBlock.serverName}/${mcpBlock.toolName}]`;
      ...
      const askApproval = async (
        ...
        if (text) {
          ...
          pushToolResult(
            formatResponse.toolResult(
              formatResponse.toolApprovedWithFeedback(text, toolProtocol),
              images,
            ),
          );
        }
      ...
      if (!mcpBlock.partial) {
        ...
        TelemetryService.instance.captureToolUsage(
          cline.taskId,
          'use_mcp_tool',
          toolProtocol,
        );
    ...
    case 'tool_use': {
        ...
        switch (block.name) {
          ...
          case 'apply_diff':
            ...
            if (block.params.path) {
              ...
              const match = block.params.args.match(
                /<file>.*?<path>([^<]+)<\/path>/s,
              );
              if (match) {
                const firstPath = match[1];
                // Check if there are multiple files
                const fileCount = (block.params.args.match(/<file>/g) || [])
                  .length;
                if (fileCount > 1) {
      ...
      const pushToolResult = (content: ToolResponse) => {
        ...
        if (toolProtocol === TOOL_PROTOCOL.XML) {
          ...
          !isMultipleNativeToolCallsEnabled
        ) {
      ...
      try {
        ...
        const hasStarted = messages.some(
          (m: any) => m.say === 'browser_action_result',
        );
        let isClosed = false;
      ...
      if (!block.partial) {
        ...
        if (!repetitionCheck.allowExecution && repetitionCheck.askUser) {
          ...
          const { response, text, images } = await cline.ask(
            ...
            repetitionCheck.askUser.messageDetail.replace(
              '{toolName}',
              block.name,
            ),
          );
          ...
          if (response === 'messageResponse') {
            ...
            TelemetryService.instance.captureConsecutiveMistakeError(
              cline.taskId,
            );
          }
      ...
      switch (block.name) {
        case 'write_to_file':
          ...
          break;
        case 'update_todo_list':
          await updateTodoListTool.handle(
            cline,
            block as ToolUse<'update_todo_list'>,
            {
              askApproval,
              handleError,
              pushToolResult,
        ...
        case 'apply_diff': {
          ...
          if (toolProtocol === TOOL_PROTOCOL.NATIVE) {
            await applyDiffToolClass.handle(
              cline,
              block as ToolUse<'apply_diff'>,
              {
                askApproval,
                handleError,
                pushToolResult,
        ...
        case 'search_and_replace':
          await checkpointSaveAndMark(cline);
          await searchAndReplaceTool.handle(
            cline,
            block as ToolUse<'search_and_replace'>,
            {
              askApproval,
              handleError,
              pushToolResult,
        ...
        case 'search_replace':
          await checkpointSaveAndMark(cline);
          await searchReplaceTool.handle(
            cline,
            block as ToolUse<'search_replace'>,
            {
              askApproval,
              handleError,
              pushToolResult,
        ...
        case 'read_file':
          ...
          const modelId = cline.api.getModel().id;
          if (
            shouldUseSingleFileRead(modelId) &&
            toolProtocol !== TOOL_PROTOCOL.NATIVE
          ) {
          ...
          break;
        case 'fetch_instructions':
          await fetchInstructionsTool.handle(
            cline,
            block as ToolUse<'fetch_instructions'>,
            {
              askApproval,
              handleError,
              pushToolResult,
        ...
        case 'codebase_search':
          await codebaseSearchTool.handle(
            cline,
            block as ToolUse<'codebase_search'>,
            {
              askApproval,
              handleError,
              pushToolResult,
          ...
          break;
        case 'list_code_definition_names':
          await listCodeDefinitionNamesTool.handle(
            cline,
            block as ToolUse<'list_code_definition_names'>,
            {
              askApproval,
              handleError,
              pushToolResult,
        ...
        case 'access_mcp_resource':
          await accessMcpResourceTool.handle(
            cline,
            block as ToolUse<'access_mcp_resource'>,
            {
              askApproval,
              handleError,
              pushToolResult,
          ...
          break;
        case 'ask_followup_question':
          await askFollowupQuestionTool.handle(
            cline,
            block as ToolUse<'ask_followup_question'>,
            {
              askApproval,
              handleError,
              pushToolResult,
        ...
        case 'run_slash_command':
          await runSlashCommandTool.handle(
            cline,
            block as ToolUse<'run_slash_command'>,
            {
              askApproval,
              handleError,
              pushToolResult,
        ...
        case 'generate_image':
          await checkpointSaveAndMark(cline);
          await generateImageTool.handle(
            cline,
            block as ToolUse<'generate_image'>,
            {
              askApproval,
              handleError,
              pushToolResult,
        ...
        default: {
          ...
          if (toolProtocol === TOOL_PROTOCOL.NATIVE && toolCallId) {
            ...
            pushToolResult(
              formatResponse.toolError(errorMessage, toolProtocol),
            );
          }
          break;
        }
  ...
  if (!block.partial || cline.didRejectTool || cline.didAlreadyUseTool) {
    ...
    if (
      ...
      cline.assistantMessageContent.length - 1
    ) {
...
async function checkpointSaveAndMark(task: Task) {
  ...
  try {
    ...
    console.error(
      `[Task#presentAssistantMessage] Error saving checkpoint: ${error.message}`,
      error,
    );
  }
}
