import { type ToolName, toolNames, type FileEntry } from 'agent-lib/types';
import {
  type ToolUse,
  type McpToolUse,
  type ToolParamName,
      ...
      switch (toolCall.name) {
        case 'read_file':
          if (args.files && Array.isArray(args.files)) {
            nativeArgs = {
              files: this.convertFileEntries(args.files),
  ...
  public static parseDynamicMcpTool(toolCall: {
    ...
    try {
      ...
      if (!serverName || !toolName) {
        console.error(
          `Could not extract server_name or tool_name from: ${toolCall.name}`,
        );
        return null;
      }
