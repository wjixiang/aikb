...
import {
  ...
  parseAssistantMessage as parseAssistantMessageV1,
  ...
  describe(`parseAssistantMessageV${index + 1}`, () => {
    ...
    describe('tool use parsing', () => {
      it('should parse a simple tool use', () => {
        ...
        const result = parser(message).filter(
          (block) => !isEmptyTextContent(block),
        );
    ...
    describe('mixed content parsing', () => {
      it('should parse text followed by a tool use', () => {
        const message =
          "Here's the file content: <read_file><path>src/file.ts</path></read_file>";
        const result = parser(message);
      ...
      it('should parse a tool use followed by text', () => {
        const message =
          "<read_file><path>src/file.ts</path></read_file>Here's what I found in the file.";
        const result = parser(message).filter(
          (block) => !isEmptyTextContent(block),
        );
	...
	return true;
        ...
        expect(toolUse.params.content).toContain(
          '// This has XML-like content: </content>',
        );
	...
	<execute_command><command>node src/index.ts</command></execute_command>`;
        ...
        // Second text block
        expect(result[2].type).toBe('text');
        expect((result[2] as TextContent).content).toContain(
          "Now let's modify the file:",
        );
        ...
        expect((result[4] as TextContent).content).toContain(
          "Let's run the code:",
        );
