...
const measureMemoryUsage = (
  ...
  if (global.gc) {
    ...
    console.warn(
      'No garbage collection hook! Run with --expose-gc for more accurate memory measurements.',
    );
  }
...
const runBenchmark = () => {
  const maxNameLength = testCases.reduce(
    (max, testCase) => Math.max(max, testCase.name.length),
    0,
  );
  ...
  for (const testCase of testCases) {
    ...
    const v2Time = measureExecutionTime(
      parseAssistantMessageV2,
      testCase.input,
    );
    const timeRatio = v1Time / v2Time;

    const v1Memory = measureMemoryUsage(
      parseAssistantMessageV1,
      testCase.input,
    );
    const v2Memory = measureMemoryUsage(
      parseAssistantMessageV2,
      testCase.input,
    );
