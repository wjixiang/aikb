...
describe('importExport', () => {
  ...
  describe('importSettings', () => {
    ...
    it('should import settings successfully from a valid file', async () => {
      (vscode.window.showOpenDialog as Mock).mockResolvedValue([
        { fsPath: '/mock/path/settings.json' },
      ]);
    ...
    it('should return success: false when file content is invalid', async () => {
      ...
      expect(result).toEqual({
        success: false,
        error: '[providerProfiles.currentApiConfigName]: Required',
      });
      expect(fs.readFile).toHaveBeenCalledWith(
        '/mock/path/settings.json',
        'utf-8',
    ...
    it('should not clobber existing api configs', async () => {
      ...
      const mockFileContent = JSON.stringify({
        ...
        providerProfiles: {
          ...
          apiConfigs: {
            default: { apiProvider: 'anthropic' as const, id: 'anthropic' },
          },
        },
      });
      ...
      if (result.success && 'providerProfiles' in result) {
        ...
        expect(result.providerProfiles?.apiConfigs['default'].apiProvider).toBe(
          'anthropic',
        );
      }
    });
    ...
    it('should call updateCustomMode for each custom mode in config', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockCustomModesManager.updateCustomMode).toHaveBeenCalledTimes(
        customModes.length,
      );
    ...
    it('should return error when provided file path does not exist', async () => {
      ...
      expect(showErrorMessageSpy).toHaveBeenCalledWith(
        expect.stringContaining('errors.settings_import_failed'),
      );
  ...
  describe('exportSettings', () => {
    ...
    it('should export settings to the selected file location', async () => {
      ...
      const mockProviderProfiles = {
        currentApiConfigName: 'test',
        apiConfigs: {
          test: { apiProvider: 'openai' as ProviderName, id: 'test-id' },
        },
    ...
    it('should handle errors during the export process', async () => {
      ...
      (safeWriteJson as Mock).mockRejectedValueOnce(
        new Error('Safe write error'),
      );
    ...
    it('should handle errors during directory creation', async () => {
      ...
      mockContextProxy.export.mockResolvedValue({ mode: 'code' });
      (fs.mkdir as Mock).mockRejectedValue(
    ...
    it('should use the correct default save location', async () => {
      ...
      expect(vscode.Uri.file).toHaveBeenCalledWith(
        path.join('/mock/home', 'Documents', 'roo-code-settings.json'),
      );
    });
    ...
    describe('codebase indexing export', () => {
        ...
        const mockGlobalSettings = {
          ...
          codebaseIndexConfig: {
            ...
            codebaseIndexOpenAiCompatibleBaseUrl:
              'https://custom-openai-api.example.com/v1',
      ...
      it('should export model dimension for OpenAI Compatible provider', async () => {
        ...
        expect(
          exportedData.globalSettings.codebaseIndexConfig
    ...
    describe('import with OpenAI Compatible codebase indexing settings', () => {
      it('should properly import OpenAI Compatible settings in codebaseIndexConfig', async () => {
        ...
        const importedProviderProfiles =
          mockProviderSettingsManager.import.mock.calls[0][0];
        const importedProvider =
          importedProviderProfiles.apiConfigs['openai-compatible-provider'];
        ...
        expect(
          importedProvider.codebaseIndexOpenAiCompatibleModelDimension,
        ).toBe(512);
      });
      ...
      it('should not modify provider settings for non-openai-compatible providers during import', async () => {
        ...
        const importedProviderProfiles =
          mockProviderSettingsManager.import.mock.calls[0][0];
        const importedProvider =
          importedProviderProfiles.apiConfigs['anthropic-provider'];

        expect(
          importedProvider.codebaseIndexOpenAiCompatibleBaseUrl,
        ).toBeUndefined();
        expect(
          importedProvider.codebaseIndexOpenAiCompatibleModelDimension,
        ).toBeUndefined();
    ...
    it('should preserve model dimension exactly in export/import roundtrip', async () => {
      ...
      const importedGlobalSettings =
        mockContextProxy.setValues.mock.calls[0][0];
      expect(
        importedGlobalSettings.codebaseIndexConfig
          ?.codebaseIndexEmbedderModelDimension,
      ).toBe(testModelDimension);
      expect(
        importedGlobalSettings.codebaseIndexConfig
          ?.codebaseIndexOpenAiCompatibleBaseUrl,
      ).toBe('https://api.example.com/v1');
    ...
    it('should handle edge case model dimension values (0, null) correctly', async () => {
      ...
      expect(
        setValuesCall.codebaseIndexConfig?.codebaseIndexEmbedderModelDimension,
      ).toBe(0);
    });
    ...
    it('should handle missing model dimension gracefully', async () => {
      ...
      mockContextProxy.getGlobalState = vi
        .fn()
    ...
    it('should handle provider mismatch during import - BUG REPRODUCTION', async () => {
      ...
      expect(providerA.codebaseIndexOpenAiCompatibleBaseUrl).toBe(
        'https://api-a.example.com/v1',
      );

      // Provider B should not have OpenAI Compatible settings
      expect(
        providerB.codebaseIndexOpenAiCompatibleModelDimension,
      ).toBeUndefined();
    ...
    it('should NOT copy OpenAI Compatible settings to provider profiles - FIXED BEHAVIOR', async () => {
      ...
      expect(
        importedGlobalSettings.codebaseIndexConfig
          ?.codebaseIndexOpenAiCompatibleBaseUrl,
      ).toBe('https://new-url.example.com/v1');
      expect(
        importedGlobalSettings.codebaseIndexConfig
      ...
      expect(
        openaiCompatibleProvider.codebaseIndexOpenAiCompatibleBaseUrl,
      ).toBeUndefined();
      expect(
        openaiCompatibleProvider.codebaseIndexOpenAiCompatibleModelDimension,
      ).toBeUndefined();
      expect(
        anthropicProvider.codebaseIndexOpenAiCompatibleBaseUrl,
      ).toBeUndefined();
      expect(
        anthropicProvider.codebaseIndexOpenAiCompatibleModelDimension,
      ).toBeUndefined();
    });
    ...
    it('should keep OpenAI Compatible settings in global state only - FIXED BEHAVIOR', async () => {
      ...
      expect(
        importedGlobalSettings.codebaseIndexConfig
          ?.codebaseIndexOpenAiCompatibleBaseUrl,
      ).toBe('https://updated.example.com/v1');
      expect(
        importedGlobalSettings.codebaseIndexConfig
          ?.codebaseIndexOpenAiCompatibleModelDimension,
      ).toBe(1536);
    ...
    it('should export OpenAI Compatible settings from global state when provider is openai-compatible', async () => {
      ...
      expect(
        exportedData.globalSettings.codebaseIndexConfig
          .codebaseIndexOpenAiCompatibleBaseUrl,
      ).toBe('https://custom-api.example.com/v1');
    });
    ...
    it.each([
      ...
      {
        testCase:
          'both supportsReasoningBudget and requiredReasoningBudget are false',
      ...
      async ({ providerName, modelId, providerId }) => {
        ...
        const realProviderSettingsManager = new ProviderSettingsManager(
          mockExtensionContext,
        );
        ...
        await realProviderSettingsManager.activateProfile({
          name: providerName,
        });
