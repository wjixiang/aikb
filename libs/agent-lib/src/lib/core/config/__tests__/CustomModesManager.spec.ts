...
import type { ModeConfig } from 'agent-lib/types';
...
describe('CustomModesManager', () => {
  ...
  describe('updateCustomMode', () => {
    ...
    it('queues write operations', async () => {
      ...
      await Promise.all([
        manager.updateCustomMode('mode1', mode1),
        manager.updateCustomMode('mode2', mode2),
      ]);

      // Verify final state in settings file
      expect(settingsContent.customModes).toHaveLength(2);
      expect(
        settingsContent.customModes.map((m: ModeConfig) => m.name),
      ).toContain('Mode 1');
      expect(
        settingsContent.customModes.map((m: ModeConfig) => m.name),
      ).toContain('Mode 2');
  ...
  describe('File Operations', () => {
    ...
    it("creates default config if file doesn't exist", async () => {
      ...
      expect(fs.writeFile).toHaveBeenCalledWith(
        settingsPath,
        expect.stringMatching(/^customModes: \[\]/),
      );
    });
  ...
  describe('deleteCustomMode', () => {
    it('deletes mode from settings file', async () => {
      ...
      (mockContext.globalState.update as Mock).mockImplementation(
    ...
    it('handles errors gracefully', async () => {
      ...
      expect(mockShowError).toHaveBeenCalledWith(
        'customModes.errors.deleteFailed',
      );
    });
  });
    ...
    describe('importModeWithRules', () => {
      ...
      it('should successfully import mode with rules files', async () => {
        ...
        expect(fs.mkdir).toHaveBeenCalledWith(
          expect.stringContaining('rules-imported-mode'),
          {
            recursive: true,
          },
        );
        expect(fs.mkdir).toHaveBeenCalledWith(
          expect.stringContaining(
            path.join('rules-imported-mode', 'subfolder'),
          ),
          { recursive: true },
        );

        // Verify file contents
        const rule1Path = Object.keys(writtenFiles).find((p) =>
          p.includes('rule1.md'),
        );
        const rule2Path = Object.keys(writtenFiles).find((p) =>
          p.includes('rule2.md'),
        );
      ...
      it('should prevent path traversal attacks in import', async () => {
        ...
        const mockWorkspacePath = path.resolve('/mock/workspace');
        const writtenRuleFiles = writtenFiles.filter(
          (p) => !p.includes('.roomodes'),
        );
        writtenRuleFiles.forEach((filePath) => {
          const normalizedPath = path.normalize(filePath);
          const expectedBasePath = path.normalize(
            path.join(mockWorkspacePath, '.roo'),
          );
        ...
        expect(writtenFiles.some((p) => p.includes('sensitive.txt'))).toBe(
          false,
        );
        expect(
          writtenFiles.some(
            (p) => path.isAbsolute(p) && !p.startsWith(mockWorkspacePath),
          ),
        ).toBe(false);
      });
	...
	customModes:
      ...
      it('should remove existing rules folder when importing mode without rules', async () => {
        ...
        expect(fs.rm).toHaveBeenCalledWith(
          expect.stringContaining(path.join('.roo', 'rules-test-mode')),
          {
            recursive: true,
            force: true,
          },
      ...
      it('should remove existing rules folder and create new ones when importing mode with rules', async () => {
        ...
        expect(fs.mkdir).toHaveBeenCalledWith(
          expect.stringContaining('rules-test-mode'),
          { recursive: true },
        );

        // Verify file contents
        const newRulePath = Object.keys(writtenFiles).find((p) =>
          p.includes('new-rule.md'),
        );
  ...
  describe('checkRulesDirectoryHasContent', () => {
    ...
    it("should work with global custom modes when .roomodes doesn't exist", async () => {
      ...
      const result =
        await freshManager.checkRulesDirectoryHasContent('test-mode');
  ...
  describe('exportModeWithRules', () => {
    ...
    it('should successfully export global mode with rules from global .roo directory', async () => {
      ...
      (fs.readFile as Mock).mockImplementation(async (path: string) => {
        ...
        if (
          path.includes('rules-global-test-mode') &&
          path.includes('rule1.md')
        ) {
