...
describe('ProviderSettingsManager', () => {
  ...
  describe('initialize', () => {
    ...
    it('should call migrateConsecutiveMistakeLimit if it has not done so already', async () => {
      ...
      const calls = mockSecrets.store.mock.calls;
      const storedConfig = JSON.parse(calls[calls.length - 1][1]);
      expect(storedConfig.apiConfigs.default.consecutiveMistakeLimit).toEqual(
        3,
      );
      expect(storedConfig.apiConfigs.test.consecutiveMistakeLimit).toEqual(3);
      expect(storedConfig.apiConfigs.existing.consecutiveMistakeLimit).toEqual(
        5,
      );
      expect(storedConfig.migrations.consecutiveMistakeLimitMigrated).toEqual(
        true,
      );
    });
    ...
    it('should apply model migrations for all providers', async () => {
      ...
      expect(storedConfig.apiConfigs.default.apiModelId).toEqual(
        'roo/code-supernova-1-million',
      );
      expect(storedConfig.apiConfigs.test.apiModelId).toEqual(
        'roo/code-supernova-1-million',
      );
      expect(storedConfig.apiConfigs.existing.apiModelId).toEqual(
        'roo/code-supernova-1-million',
      );

      // Non-roo provider configs should not be migrated
      expect(storedConfig.apiConfigs.otherProvider.apiModelId).toEqual(
        'roo/code-supernova',
      );
      expect(storedConfig.apiConfigs.noProvider.apiModelId).toEqual(
        'roo/code-supernova',
      );
    });
  ...
  describe('SaveConfig', () => {
    it('should save new config', async () => {
      ...
      expect(mockSecrets.store.mock.calls[0][0]).toEqual(
    ...
    it('should only save provider relevant settings', async () => {
      ...
      const storedConfig = JSON.parse(
        mockSecrets.store.mock.calls[
          mockSecrets.store.mock.calls.length - 1
        ][1],
      );
      const testConfigId = storedConfig.apiConfigs.test.id;
    ...
    it('should update existing config', async () => {
      ...
      ).toEqual('roo_cline_config_api_config');
      expect(storedConfig).toEqual(expectedConfig);
    });
  ...
  describe('DeleteConfig', () => {
    ...
    it('should throw error when trying to delete non-existent config', async () => {
      ...
      await expect(
        providerSettingsManager.deleteConfig('nonexistent'),
      ).rejects.toThrow("Config 'nonexistent' not found");
    });
    ...
    it('should throw error when trying to delete last remaining config', async () => {
      ...
      await expect(
        providerSettingsManager.deleteConfig('default'),
      ).rejects.toThrow(
  ...
  describe('LoadConfig', () => {
    it('should load config and update current config name', async () => {
      ...
      const { name, ...providerSettings } =
        await providerSettingsManager.activateProfile({ name: 'test' });
    ...
    it('should throw error when config does not exist', async () => {
      ...
      await expect(
        providerSettingsManager.activateProfile({ name: 'nonexistent' }),
      ).rejects.toThrow("Config with name 'nonexistent' not found");
    });
    ...
    it('should throw error if secrets storage fails', async () => {
      ...
      await expect(
        providerSettingsManager.activateProfile({ name: 'test' }),
      ).rejects.toThrow(
    ...
    it('should sanitize invalid/removed providers by resetting apiProvider to undefined', async () => {
      ...
      mockSecrets.get.mockResolvedValue(
        JSON.stringify(configWithRemovedProvider),
      );
      ...
      expect(
        storedConfig.apiConfigs.removedProvider.apiProvider,
      ).toBeUndefined();
    ...
    it('should sanitize invalid providers and remove non-object profiles during load', async () => {
      ...
      expect(
        storedConfig.apiConfigs.invalidProvider.apiProvider,
      ).toBeUndefined();
      ...
      expect(Object.keys(storedConfig.apiConfigs)).toEqual([
        'valid',
        'invalidProvider',
      ]);
  ...
  describe('syncCloudProfiles', () => {
    it('should add new cloud profiles without secret keys', async () => {
      ...
      const result =
        await providerSettingsManager.syncCloudProfiles(cloudProfiles);
    ...
    it('should track active profile changes when active profile is updated', async () => {
      ...
      const result = await providerSettingsManager.syncCloudProfiles(
        cloudProfiles,
        'active-profile',
      );
    ...
    it('should create default profile when all profiles are deleted', async () => {
      ...
      const storedConfig = JSON.parse(mockSecrets.store.mock.calls[0][1]);
      expect(storedConfig.apiConfigs['default']).toBeDefined();
      expect(storedConfig.apiConfigs['default'].id).toBe(
