...
describe('CustomModesManager - Export/Import with Slug Changes', () => {
  ...
  describe('Export Path Calculation', () => {
    it('should exclude rules-{slug} folder from exported relative paths', async () => {
      ...
      (fs.readFile as Mock).mockImplementation(async (path: string) => {
        ...
        if (
          path.includes('rules-test-mode') &&
          path.includes('subfolder') &&
          path.includes('rule2.md')
        ) {
  ...
  describe('Import with Slug Changes', () => {
    it('should place files in rules-{new-slug} folder when slug is changed', async () => {
      ...
      const rule1Path = Object.keys(writtenFiles).find(
        (p) => p.includes('rule1.md') && !p.includes('.roomodes'),
      );
      const rule2Path = Object.keys(writtenFiles).find(
        (p) => p.includes('rule2.md') && !p.includes('.roomodes'),
      );
      ...
      expect(rule1Path).toContain(
        path.join('.roo', 'rules-new-slug-name', 'rule1.md'),
      );
      expect(rule2Path).toContain(
        path.join('.roo', 'rules-new-slug-name', 'subfolder', 'rule2.md'),
      );

      // Verify directories were created with new slug
      expect(
        createdDirs.some((dir) => dir.includes('rules-new-slug-name')),
      ).toBe(true);
    });
    ...
    it('should handle mixed format paths correctly', async () => {
      ...
      const oldFormatPath = Object.keys(writtenFiles).find((p) =>
        p.includes('old-format.md'),
      );
      const newFormatPath = Object.keys(writtenFiles).find((p) =>
        p.includes('new-format.md'),
      );
      const nestedPath = Object.keys(writtenFiles).find((p) =>
        p.includes(path.join('nested', 'file.md')),
      );

      expect(oldFormatPath).toContain(
        path.join('.roo', 'rules-mixed-mode', 'old-format.md'),
      );
      expect(newFormatPath).toContain(
        path.join('.roo', 'rules-mixed-mode', 'new-format.md'),
      );
      expect(nestedPath).toContain(
        path.join('.roo', 'rules-mixed-mode', 'nested', 'file.md'),
      );
    });
  });
  ...
  describe('End-to-End Export/Import with Slug Change', () => {
    it('should successfully export and re-import with a different slug', async () => {
      ...
      expect(ruleFilePath).toContain(
        path.join('.roo', 'rules-renamed-mode', 'rule.md'),
      );
