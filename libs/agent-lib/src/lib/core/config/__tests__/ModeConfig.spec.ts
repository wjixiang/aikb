...
import { type ModeConfig, modeConfigSchema } from 'agent-lib/types';
...
describe('CustomModeSchema', () => {
  describe('validateCustomMode', () => {
    ...
    test('rejects empty strings in required fields', () => {
      ...
      expect(() => validateCustomMode(emptyNameMode)).toThrow(
        'Name is required',
      );
      expect(() => validateCustomMode(emptyRoleMode)).toThrow(
  ...
  describe('fileRegex', () => {
    ...
    it('prevents duplicate groups', () => {
      ...
      expect(() => modeConfigSchema.parse(modeWithDuplicates)).toThrow(
        /Duplicate groups/,
      );
    });
  });
