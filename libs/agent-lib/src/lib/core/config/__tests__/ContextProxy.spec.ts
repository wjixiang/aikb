...
describe('ContextProxy', () => {
  ...
  describe('constructor', () => {
    ...
    it('should initialize secret cache with all secret keys', () => {
      expect(mockSecrets.get).toHaveBeenCalledTimes(
        SECRET_STATE_KEYS.length + GLOBAL_SECRET_KEYS.length,
      );
  ...
  describe('getGlobalState', () => {
    it('should return value from cache when it exists', async () => {
      ...
      expect(mockGlobalState.get).toHaveBeenCalledTimes(
        GLOBAL_STATE_KEYS.length + 1,
      ); // From initialization + migration check
    });
  ...
  describe('setValue', () => {
    it('should route secret keys to storeSecret', async () => {
      ...
      expect(storeSecretSpy).toHaveBeenCalledWith(
        'openAiApiKey',
        'test-api-key',
      );
  ...
  describe('invalid apiProvider migration', () => {
    ...
    it('should not modify valid apiProvider during initialization', async () => {
      ...
      const apiProviderUpdateCalls = updateCalls.filter(
        (call: any[]) => call[0] === 'apiProvider',
      );
  ...
  describe('getProviderSettings', () => {
    it('should sanitize invalid apiProvider before parsing', async () => {
      ...
      await proxy.updateGlobalState(
        'apiProvider',
        'invalid-removed-provider' as any,
      );
