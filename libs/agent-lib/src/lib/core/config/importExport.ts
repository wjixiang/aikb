...
import { globalSettingsSchema } from 'agent-lib/types';
...
export async function importSettingsFromPath(
  ...
  try {
    ...
    const { providerProfiles: newProviderProfiles, globalSettings = {} } =
      schema.parse(JSON.parse(await fs.readFile(filePath, 'utf-8')));
    ...
    contextProxy.setValue(
      'listApiConfigMeta',
      await providerSettingsManager.listConfig(),
    );
...
export const exportSettings = async ({
  ...
  const uri = await vscode.window.showSaveDialog({
    ...
    defaultUri: vscode.Uri.file(
      path.join(os.homedir(), 'Documents', 'roo-code-settings.json'),
    ),
  });
...
export const importSettingsWithFeedback = async (
  ...
  if (result.success) {
    ...
    await vscode.window.showInformationMessage(
      t('common:info.settings_imported'),
    );
  } else if (result.error) {
    await vscode.window.showErrorMessage(
      t('common:errors.settings_import_failed', { error: result.error }),
    );
  }
};
