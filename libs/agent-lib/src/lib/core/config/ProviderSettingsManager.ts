...
export class ProviderSettingsManager {
  ...
  private readonly defaultModeApiConfigs: Record<string, string> =
    Object.fromEntries(modes.map((mode) => [mode.slug, this.defaultConfigId]));
  ...
  public async initialize() {
    try {
      return await this.lock(async () => {
        ...
        for (const [_name, apiConfig] of Object.entries(
  ...
  private async migrateRateLimitSeconds(providerProfiles: ProviderProfiles) {
    try {
      ...
      try {
        rateLimitSeconds =
          await this.context.globalState.get<number>('rateLimitSeconds');
      } catch (error) {
        console.error(
  ...
  private async migrateDiffSettings(providerProfiles: ProviderProfiles) {
    try {
      ...
      try {
        diffEnabled =
          await this.context.globalState.get<boolean>('diffEnabled');
        fuzzyMatchThreshold = await this.context.globalState.get<number>(
          'fuzzyMatchThreshold',
        );
      } catch (error) {
        console.error(
          '[MigrateDiffSettings] Error getting global diff settings:',
          error,
        );
      }
      ...
      console.error(
        `[MigrateDiffSettings] Failed to migrate diff settings:`,
        error,
      );
    }
  }
    ...
    try {
      for (const [_name, apiConfig] of Object.entries(
        ...
        if (
          ...
          (!apiConfig.openAiHeaders ||
            Object.keys(apiConfig.openAiHeaders || {}).length === 0)
        ) {
      ...
      console.error(
        `[MigrateOpenAiHeaders] Failed to migrate OpenAI headers:`,
        error,
      );
    }
  }

  private async migrateConsecutiveMistakeLimit(
    providerProfiles: ProviderProfiles,
  ) {
    try {
      for (const [name, apiConfig] of Object.entries(
        providerProfiles.apiConfigs,
      )) {
  ...
  private applyModelMigrations(providerProfiles: ProviderProfiles): boolean {
    ...
    try {
      ...
      console.error(
        `[ModelMigration] Failed to apply model migrations:`,
        error,
      );
    }
  ...
  public async listConfig(): Promise<ProviderSettingsEntry[]> {
    try {
      return await this.lock(async () => {
        ...
        return Object.entries(providerProfiles.apiConfigs).map(
          ([name, apiConfig]) => ({
            name,
            id: apiConfig.id || '',
  ...
  public async getProfile(
    ...
    try {
      ...
      throw new Error(
        `Failed to get profile: ${error instanceof Error ? error.message : error}`,
      );
    }
  }
  ...
  public async activateProfile(
    ...
    try {
      ...
      throw new Error(
        `Failed to activate profile: ${error instanceof Error ? error.message : error}`,
      );
    }
  }
    ...
    try {
      return await this.lock(async () => {
        ...
        for (const name in configs) {
          ...
          configs[name] = discriminatedProviderSettingsWithIdSchema.parse(
            configs[name],
          );
          ...
          try {
            ...
            console.warn(
              `Skipping token field filtering for config '${name}': ${error}`,
            );
          }
        }
        return profiles;
    ...
    try {
      ...
      const apiConfigs = Object.entries(providerProfiles.apiConfigs).reduce(
        (acc, [key, apiConfig]) => {
          ...
          const result =
            providerSettingsWithIdSchema.safeParse(sanitizedConfig);
    ...
    try {
      return await this.lock(async () => {
        ...
        if (
          ...
          providerProfiles.apiConfigs[currentActiveProfileName]
        ) {
          activeProfileId =
            providerProfiles.apiConfigs[currentActiveProfileName].id || '';
        }
        ...
        for (const [name, profile] of Object.entries(
          ...
          if (
            profile.id &&
            currentCloudIds.has(profile.id) &&
            !newCloudIds.has(profile.id)
          ) {
            ...
            if (existingName !== cloudName) {
              ...
              if (existingNames.has(cloudName)) {
                ...
                const conflictingProfile =
                  providerProfiles.apiConfigs[cloudName];
                if (conflictingProfile.id !== cloudProfile.id) {
                  const newName = this.findUniqueProfileName(
                    cloudName,
                    existingNames,
                  );
