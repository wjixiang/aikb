...
export class CustomModesManager {
  ...
  private cleanInvisibleCharacters(content: string): string {
    ...
    return content.replace(
      CustomModesManager.PROBLEMATIC_CHARS_REGEX,
      (match) => {
        switch (match) {
    ...
    try {
      ...
      if (filePath.endsWith(ROOMODES_FILENAME)) {
        try {
          ...
          console.error(
            `[CustomModesManager] Failed to parse YAML from ${filePath}:`,
            errorMsg,
          );
          ...
          vscode.window.showErrorMessage(
            t('common:customModes.errors.yamlParseError', { line }),
          );
  ...
  private async loadModesFromFile(filePath: string): Promise<ModeConfig[]> {
    try {
      ...
      if (!result.success) {
        console.error(
          `[CustomModesManager] Schema validation failed for ${filePath}:`,
          result.error,
        );
  ...
  public async getCustomModesFilePath(): Promise<string> {
    ...
    if (!fileExists) {
      await this.queueWrite(() =>
        fs.writeFile(
          filePath,
          yaml.stringify({ customModes: [] }, { lineWidth: 0 }),
        ),
      );
    }
  ...
  private async watchCustomModesFiles(): Promise<void> {
    ...
    const handleSettingsChange = async () => {
      try {
        ...
        const roomodesModes = roomodesPath
          ? await this.loadModesFromFile(roomodesPath)
          : [];
    ...
    if (workspaceFolders && workspaceFolders.length > 0) {
      ...
      const roomodesWatcher =
        vscode.workspace.createFileSystemWatcher(roomodesPath);
      ...
      const handleRoomodesChange = async () => {
        try {
          ...
          console.error(
            `[CustomModesManager] Error handling .roomodes file change:`,
            error,
          );
        }
      };
      ...
      this.disposables.push(
        roomodesWatcher.onDidDelete(async () => {
          ...
          try {
            ...
            console.error(
              `[CustomModesManager] Error handling .roomodes file deletion:`,
              error,
            );
          }
        }),
  ...
  public async updateCustomMode(
    ...
    try {
      ...
      if (isProjectMode) {
        ...
        if (!workspaceFolders || workspaceFolders.length === 0) {
          logger.error(
            'Failed to update project mode: No workspace folder found',
            { slug },
          );
  ...
  private async updateModesInFile(
    ...
    await fs.writeFile(
      filePath,
      yaml.stringify(settings, { lineWidth: 0 }),
      'utf-8',
    );
  }
    ...
    try {
      ...
      if (rulesFolderExists) {
        try {
          ...
          logger.info(
            `Deleted rules folder for mode ${slug}: ${rulesFolderPath}`,
          );
        } catch (error) {
          logger.error(
            `Failed to delete rules folder for mode ${slug}: ${error}`,
          );
      ...
      if (!mode) {
        ...
        try {
          ...
          if (roomodesExists) {
            ...
            const modeInRoomodes = roomodesModes.find(
              (m: any) => m.slug === slug,
            );
            if (!modeInRoomodes) {
  ...
  private async importRulesFiles(
    ...
    try {
      await fs.rm(rulesFolderPath, { recursive: true, force: true });
      logger.info(
        `Removed existing ${source} rules folder for mode ${importMode.slug}`,
      );
    } catch (error) {
    ...
    for (const ruleFile of rulesFiles) {
      if (ruleFile.relativePath && ruleFile.content) {
        ...
        const rulesMatch = normalizedRelativePath.match(
          /^rules-[^\/\\]+[\/\\]/,
        );
        if (rulesMatch) {
          // Strip the entire rules-* folder reference for backwards compatibility
          cleanedRelativePath = normalizedRelativePath.substring(
            rulesMatch[0].length,
          );
          logger.info(
  ...
  public async importModeWithRules(
    ...
    try {
      ...
      try {
        ...
        if (
          !parsed?.customModes ||
          !Array.isArray(parsed.customModes) ||
          parsed.customModes.length === 0
        ) {
          return {
            success: false,
            error:
