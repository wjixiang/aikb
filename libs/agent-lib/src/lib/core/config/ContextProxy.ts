...
export const isPassThroughStateKey = (key: string) =>
  PASS_THROUGH_STATE_KEYS.includes(key);
...
export class ContextProxy {
  ...
  public async initialize() {
    for (const key of GLOBAL_STATE_KEYS) {
      try {
        ...
        logger.error(
          `Error loading global ${key}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  ...
  private async migrateInvalidApiProvider() {
    try {
      ...
      if (apiProvider !== undefined && !isProviderName(apiProvider)) {
        logger.info(
          `[ContextProxy] Found invalid provider "${apiProvider}" in storage - clearing it`,
        );
  ...
  private async migrateImageGenerationSettings() {
    try {
      ...
      const oldNestedSettings = this.originalContext.globalState.get<any>(
        'openRouterImageGenerationSettings',
      );

      if (oldNestedSettings && typeof oldNestedSettings === 'object') {
        logger.info(
          'Migrating old nested image generation settings to flattened structure',
        );
        ...
        if (
          ...
          this.stateCache.openRouterImageGenerationSelectedModel =
            oldNestedSettings.selectedModel;
          logger.info(
            'Migrated openRouterImageGenerationSelectedModel to global state',
          );
        }
  ...
  getGlobalState<K extends GlobalStateKey>(
    key: K,
    defaultValue: GlobalState[K],
  ): GlobalState[K];
  getGlobalState<K extends GlobalStateKey>(
    key: K,
    defaultValue?: GlobalState[K],
  ...
  private getAllGlobalState(): GlobalState {
    return Object.fromEntries(
      GLOBAL_STATE_KEYS.map((key) => [key, this.getGlobalState(key)]),
    );
  }
  ...
  private getAllSecretState(): SecretState {
    return Object.fromEntries([
      ...SECRET_STATE_KEYS.map((key) => [
        key,
        this.getSecret(key as SecretStateKey),
      ]),
      ...GLOBAL_SECRET_KEYS.map((key) => [
        key,
        this.getSecret(key as SecretStateKey),
      ]),
    ]);
  ...
  private sanitizeProviderValues(values: RooCodeSettings): RooCodeSettings {
    if (
      values.apiProvider !== undefined &&
      !isProviderName(values.apiProvider)
    ) {
      logger.info(
        `[ContextProxy] Sanitizing invalid provider "${values.apiProvider}" - resetting to undefined`,
      );
  ...
  public async setProviderSettings(values: ProviderSettings) {
    ...
    if (values.openAiHeaders !== undefined) {
      ...
      if (
        ...
        Object.keys(values.openAiHeaders).length === 0
      ) {
        values.openAiHeaders = {};
      }
    }
  ...
  public async setValue<K extends RooCodeSettingsKey>(
    key: K,
