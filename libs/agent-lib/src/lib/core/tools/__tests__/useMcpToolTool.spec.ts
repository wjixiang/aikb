...
vi.mock('../../prompts/responses', () => ({
  formatResponse: {
    ...
    invalidMcpToolArgumentError: vi.fn(
      (server: string, tool: string) => `Invalid args for ${server}:${tool}`,
    ),
    unknownMcpToolError: vi.fn(
      (server: string, tool: string, availableTools: string[]) => {
        const toolsList =
    ...
    unknownMcpServerError: vi.fn(
      (server: string, availableServers: string[]) => {
        const list =
...
describe('useMcpToolTool', () => {
  ...
  describe('parameter validation', () => {
    it('should handle missing server_name', async () => {
      ...
      mockTask.sayAndCreateMissingParamError = vi
        .fn()
        .mockResolvedValue('Missing server_name error');
    ...
    it('should handle missing tool_name', async () => {
      ...
      mockTask.sayAndCreateMissingParamError = vi
        .fn()
        .mockResolvedValue('Missing tool_name error');
    ...
    it('should handle invalid JSON arguments', async () => {
      ...
      expect(mockPushToolResult).toHaveBeenCalledWith(
        'Tool error: Invalid args for test_server:test_tool',
      );
    });
  });
  ...
  describe('partial requests', () => {
    it('should handle partial requests', async () => {
      ...
      expect(mockTask.ask).toHaveBeenCalledWith(
        'use_mcp_server',
        expect.stringContaining('use_mcp_tool'),
        true,
      );
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('should reject unknown tool names', async () => {
      ...
      expect(mockPushToolResult).toHaveBeenCalledWith(
        expect.stringContaining('existing-tool-1'),
      );
      expect(mockPushToolResult).toHaveBeenCalledWith(
        expect.stringContaining('existing-tool-2'),
      );
    });
    ...
    it('should reject unknown server names when no servers are available', async () => {
      ...
      expect(mockPushToolResult).toHaveBeenCalledWith(
        expect.stringContaining('No servers available'),
      );
      expect(callToolMock).not.toHaveBeenCalled();
      expect(mockAskApproval).not.toHaveBeenCalled();
    });
