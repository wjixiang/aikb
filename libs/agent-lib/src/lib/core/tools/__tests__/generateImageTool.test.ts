  ...
  beforeEach(() => {
    ...
    mockCline = {
      ...
      sayAndCreateMissingParamError: vi
        .fn()
        .mockResolvedValue('Missing parameter error'),
      say: vi.fn(),
      rooIgnoreController: {
  ...
  describe('partial block handling', () => {
    it('should return early when block is partial', async () => {
      ...
      await generateImageTool.handle(
        mockCline as Task,
        partialBlock as ToolUse<'generate_image'>,
        {
    ...
    it('should process when block is not partial', async () => {
      ...
      await generateImageTool.handle(
        mockCline as Task,
        completeBlock as ToolUse<'generate_image'>,
        {
          askApproval: mockAskApproval,
          handleError: mockHandleError,
          pushToolResult: mockPushToolResult,
    ...
    it('should add cache-busting parameter to image URI', async () => {
      ...
      const mockWebviewUri =
        'https://file+.vscode-resource.vscode-cdn.net/test/workspace/test-image.png';
      mockCline.providerRef.deref().convertToWebviewUri = vi
        .fn()
        .mockReturnValue(mockWebviewUri);
      ...
      expect(mockCline.say).toHaveBeenCalledWith(
        'image',
        expect.stringMatching(/"imageUri":"[^"]+\?t=\d+"/),
      );

      // Verify the imageUri contains the cache-busting parameter
      const sayCall = mockCline.say.mock.calls.find(
        (call: any[]) => call[0] === 'image',
      );
      if (sayCall) {
