...
const mockRemoveClosingTag = vi.fn(
  (_name: string, value: string | undefined) => value ?? '',
);
...
describe('newTaskTool', () => {
  ...
  it('should handle missing todos parameter gracefully (backward compatibility)', async () => {
    ...
    expect(mockSayAndCreateMissingParamError).not.toHaveBeenCalledWith(
      'new_task',
      'todos',
    );
  ...
  it('should error when mode parameter is missing', async () => {
    ...
    expect(mockSayAndCreateMissingParamError).toHaveBeenCalledWith(
      'new_task',
      'mode',
    );
...
describe('newTaskTool delegation flow', () => {
  it('delegates to provider and does not call legacy startSubtask', async () => {
    ...
    const providerSpy = {
      ...
      delegateParentAndOpenChild: vi
        .fn()
        .mockResolvedValue({ taskId: 'child-1' }),
      handleModeSwitch: vi.fn(),
    } as any;
