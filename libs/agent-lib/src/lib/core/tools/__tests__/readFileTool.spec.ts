...
describe('read_file tool with maxReadFileLine setting', () => {
  ...
  const mockedParseSourceCodeDefinitionsForFile = vi.mocked(
...
10--20 | function foo() {`;
      ...
      const result = await executeReadFileTool(
        {},
        { maxReadFileLine: 30, totalLines: 100 },
      );
...
80--90 | function bar() {`;
  ...
  describe('when maxReadFileLine equals or exceeds file length', () => {
    it('should use extractTextFromFile when maxReadFileLine > totalLines', async () => {
      ...
      const result = await executeReadFileTool(
        {},
        { maxReadFileLine: 10, totalLines: 5 },
      );
    ...
    it('should read with extractTextFromFile when file has few lines', async () => {
      ...
      const result = await executeReadFileTool(
        {},
        { maxReadFileLine: 5, totalLines: 3 },
      );
  ...
  describe('when file is binary', () => {
    it('should always use extractTextFromFile regardless of maxReadFileLine', async () => {
      ...
      const result = await executeReadFileTool(
        {},
        { maxReadFileLine: 3, totalLines: 3 },
      );
...
describe('read_file tool XML output structure', () => {
  ...
  beforeEach(() => {
    ...
    mockCline.sayAndCreateMissingParamError = vi
      .fn()
      .mockResolvedValue('Missing required parameter');
  ...
  async function executeReadFileTool(
    ...
    mockCline.rooIgnoreController.validateAccess = vi
      .fn()
      .mockReturnValue(validateAccess);
    ...
    await readFileTool.handle(mockCline, toolUse, {
      ...
      removeClosingTag: (param: ToolParamName, content?: string) =>
        content ?? '',
      toolProtocol: 'xml',
    });
  ...
  describe('Basic XML Structure Tests', () => {
    ...
    describe('Total Image Memory Limit', () => {
      ...
      async function executeReadMultipleImagesTool(
        imagePaths: string[],
      ): Promise<ToolResponse | undefined> {
        ...
        const filesXml = imagePaths
          .map((path) => `<file><path>${path}</path></file>`)
          .join('');
        ...
        await readFileTool.handle(mockCline, toolUse, {
          ...
          removeClosingTag: (_: ToolParamName, content?: string) =>
            content ?? '',
          toolProtocol: 'xml',
        });
      ...
      it('should allow multiple images under the total memory limit', async () => {
        ...
        fsPromises.stat = vi.fn().mockImplementation((filePath) => {
          ...
          const image = smallImages.find((img) =>
            normalizedFilePath.includes(path.normalize(img.path)),
          );
        ...
        // Execute
        const result = await executeReadMultipleImagesTool(
          smallImages.map((img) => img.path),
        );
      ...
      it('should skip images that would exceed the total memory limit', async () => {
        ...
        fsPromises.stat = vi.fn().mockImplementation((filePath) => {
          const normalizedFilePath = path.normalize(filePath.toString());
          const image = largeImages.find((img) =>
            normalizedFilePath.includes(path.normalize(img.path)),
          );
          return Promise.resolve({ size: (image?.sizeKB || 1024) * 1024 });
        });
        ...
        const textPart = Array.isArray(result)
          ? result.find((p) => p.type === 'text')?.text
          : result;
        const imageParts = Array.isArray(result)
          ? result.filter((p) => p.type === 'image')
          : [];
      ...
      it('should track memory usage correctly across multiple images', async () => {
        ...
        fsPromises.stat = vi.fn().mockImplementation((filePath) => {
          const normalizedFilePath = path.normalize(filePath.toString());
          const image = exactLimitImages.find((img) =>
            normalizedFilePath.includes(path.normalize(img.path)),
          );
          if (image) {
        ...
        // Execute
        const result = await executeReadMultipleImagesTool(
          exactLimitImages.map((img) => img.path),
        );
      ...
      it('should correctly calculate total memory and skip the last image', async () => {
        ...
        fsPromises.stat.mockImplementation(async (filePath) => {
          const normalizedFilePath = path.normalize(filePath.toString());
          const file = testImages.find((f) =>
            normalizedFilePath.includes(path.normalize(f.path)),
          );
          if (file) {
      ...
      it('should handle a folder with many images just under the individual size limit', async () => {
        ...
        fsPromises.stat = vi.fn().mockImplementation((filePath) => {
          const normalizedFilePath = path.normalize(filePath.toString());
          const image = manyImages.find((img) =>
            normalizedFilePath.includes(path.normalize(img.path)),
          );
          return Promise.resolve({ size: (image?.sizeKB || 1024) * 1024 });
        });
      ...
      it('should reset memory tracking between separate tool invocations more explicitly', async () => {
        ...
        // Execute first batch
        const result1 = await executeReadMultipleImagesTool(
          firstBatch.map((img) => img.path),
        );
        ...
        // Execute second batch
        const result2 = await executeReadMultipleImagesTool(
          secondBatch.map((img) => img.path),
        );
  ...
  describe('Error Handling Tests', () => {
    it('should include error tag for invalid path', async () => {
      ...
      expect(toolResult).toBe(
        `<files><error>Missing required parameter</error></files>`,
      );
    });
...
describe('read_file tool with image support', () => {
  ...
  async function executeReadImageTool(
    imagePath: string = testImagePath,
  ): Promise<ToolResponse | undefined> {
    ...
    console.log(
      'Supports images:',
      localMockCline.api?.getModel?.()?.info?.supportsImages,
    );
    ...
    console.log(
      'Result type:',
      Array.isArray(toolResult) ? 'array' : typeof toolResult,
    );
  ...
  describe('Image Format Detection', () => {
    it.each([
      ...
      'should detect %s as an image format',
      async (ext, filename, expectedMimeType) => {
        // Setup
  ...
  describe('Image Reading Functionality', () => {
    ...
    it('should handle errors when reading image files', async () => {
      ...
      expect(toolResult).toContain(
        '<error>Error reading image file: Failed to read image</error>',
      );
      // Verify that say was called to show error to user
      expect(localMockCline.say).toHaveBeenCalledWith(
        'error',
        expect.stringContaining('Failed to read image'),
      );
    });
  });
