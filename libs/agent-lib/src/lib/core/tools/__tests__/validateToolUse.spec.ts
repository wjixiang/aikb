...
const architectMode =
  modes.find((m) => m.slug === 'architect')?.slug || 'architect';
...
describe('mode-validator', () => {
  describe('isToolAllowedForMode', () => {
    describe('code mode', () => {
      ...
      it('disallows unknown tools', () => {
        expect(isToolAllowedForMode('unknown_tool' as any, codeMode, [])).toBe(
          false,
        );
      });
    });
    ...
    describe('custom modes', () => {
      it('allows tools from custom mode configuration', () => {
        ...
        expect(
          isToolAllowedForMode('read_file', 'custom-mode', customModes),
        ).toBe(true);
        expect(
          isToolAllowedForMode('write_to_file', 'custom-mode', customModes),
        ).toBe(true);
        // Should not allow tools from other groups
        expect(
          isToolAllowedForMode('execute_command', 'custom-mode', customModes),
        ).toBe(false);
      });
      ...
      it('allows custom mode to override built-in mode', () => {
        ...
        expect(isToolAllowedForMode('read_file', codeMode, customModes)).toBe(
          true,
        );
        // Should not allow tools from other groups
        expect(
          isToolAllowedForMode('write_to_file', codeMode, customModes),
        ).toBe(false);
      });
    ...
    describe('dynamic MCP tools', () => {
      it('allows dynamic MCP tools when mcp group is in mode groups', () => {
        ...
        expect(
          isToolAllowedForMode('mcp_context7_resolve-library-id', codeMode, []),
        ).toBe(true);
        expect(
          isToolAllowedForMode('mcp_serverName_toolName', codeMode, []),
        ).toBe(true);
      });
    ...
    describe('tool requirements', () => {
      it('respects tool requirements when provided', () => {
        const requirements = { apply_diff: false };
        expect(
          isToolAllowedForMode('apply_diff', codeMode, [], requirements),
        ).toBe(false);

        const enabledRequirements = { apply_diff: true };
        expect(
          isToolAllowedForMode('apply_diff', codeMode, [], enabledRequirements),
        ).toBe(true);
      });
      ...
      it('handles undefined and empty requirements', () => {
        expect(
          isToolAllowedForMode('apply_diff', codeMode, [], undefined),
        ).toBe(true);
  ...
  describe('validateToolUse', () => {
    it('throws error for unknown/invalid tools', () => {
      ...
      ).toThrow('Unknown tool "unknown_tool". This tool does not exist.');
    });
    ...
    it('throws error when tool requirement is not met', () => {
      ...
      ).toThrow('Tool "apply_diff" is not allowed in code mode.');
    });
