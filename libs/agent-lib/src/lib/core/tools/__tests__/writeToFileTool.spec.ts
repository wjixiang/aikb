...
describe('writeToFileTool', () => {
  ...
  const absoluteFilePath =
    process.platform === 'win32' ? 'C:\\test\\file.txt' : '/test/file.txt';
  const testContent = 'Line 1\nLine 2\nLine 3';
  const testContentWithMarkdown = '```javascript\nLine 1\nLine 2\n```';

  // Mocked functions with correct types
  const mockedFileExistsAtPath = fileExistsAtPath as MockedFunction<
    typeof fileExistsAtPath
  >;
  const mockedCreateDirectoriesForFile =
    createDirectoriesForFile as MockedFunction<typeof createDirectoriesForFile>;
  const mockedIsPathOutsideWorkspace = isPathOutsideWorkspace as MockedFunction<
    typeof isPathOutsideWorkspace
  >;
  const mockedGetReadablePath = getReadablePath as MockedFunction<
    typeof getReadablePath
  >;
  const mockedUnescapeHtmlEntities = unescapeHtmlEntities as MockedFunction<
    typeof unescapeHtmlEntities
  >;
  const mockedEveryLineHasLineNumbers =
    everyLineHasLineNumbers as MockedFunction<typeof everyLineHasLineNumbers>;
  const mockedStripLineNumbers = stripLineNumbers as MockedFunction<
    typeof stripLineNumbers
  >;
  ...
  beforeEach(() => {
    ...
    mockCline.sayAndCreateMissingParamError = vi
      .fn()
      .mockResolvedValue('Missing param error');
  ...
  async function executeWriteFileTool(
    ...
    await writeToFileTool.handle(
      mockCline,
      toolUse as ToolUse<'write_to_file'>,
      {
  ...
  describe('file existence detection', () => {
    it.skipIf(process.platform === 'win32')(
      'detects existing file and sets editType to modify',
      async () => {
    ...
    it.skipIf(process.platform === 'win32')(
      'detects new file and sets editType to create',
      async () => {
  ...
  describe('content preprocessing', () => {
    ...
    it('strips line numbers from numbered content', async () => {
      ...
      expect(mockedEveryLineHasLineNumbers).toHaveBeenCalledWith(
        contentWithLineNumbers,
      );
      expect(mockedStripLineNumbers).toHaveBeenCalledWith(
        contentWithLineNumbers,
      );
      expect(mockCline.diffViewProvider.update).toHaveBeenCalledWith(
        'line one\nline two',
  ...
  describe('file operations', () => {
    it('successfully creates new files with full workflow', async () => {
      ...
      expect(
        mockCline.fileContextTracker.trackFileContext,
      ).toHaveBeenCalledWith(testFilePath, 'roo_edited');
