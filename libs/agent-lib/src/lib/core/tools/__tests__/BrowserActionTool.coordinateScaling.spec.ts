...
describe('Browser Action Coordinate Scaling', () => {
  ...
  describe('Coordinate scaling logic', () => {
    it('should correctly scale coordinates from image to viewport', () => {
      ...
      expect(scaleCoordinate('1024,768@1024x768', 1920, 1080)).toBe(
        '1920,1080',
      );
    });
    ...
    it('should throw error for invalid coordinate format', () => {
      ...
      expect(() => scaleCoordinate('450,300', 900, 600)).toThrow(
        'Invalid coordinate format',
      );
      expect(() => scaleCoordinate('450,300@1024', 900, 600)).toThrow(
        'Invalid coordinate format',
      );
      expect(() => scaleCoordinate('invalid', 900, 600)).toThrow(
        'Invalid coordinate format',
      );
    });
