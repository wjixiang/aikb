...
export class ListFilesTool extends BaseTool<'list_files'> {
  ...
  async execute(
    ...
    const { askApproval, handleError, pushToolResult, removeClosingTag } =
      callbacks;
    ...
    try {
      if (!relDirPath) {
        ...
        pushToolResult(
          await task.sayAndCreateMissingParamError('list_files', 'path'),
        );
        return;
      }
      ...
      const { showRooIgnoredFiles = false } =
