...
export class AskFollowupQuestionTool extends BaseTool<'ask_followup_question'> {
    ...
    if (follow_up_xml) {
      ...
      try {
        ...
        const rawSuggestions = Array.isArray(parsedSuggest?.suggest)
          ...
          : [parsedSuggest?.suggest].filter(
              (sug): sug is ParsedSuggestion => sug !== undefined,
            );
  ...
  async execute(
    ...
    try {
      ...
      const { text, images } = await task.ask(
        'followup',
  ...
  override async handlePartial(
    ...
    await task
      .ask(
        'followup',
        this.removeClosingTag('question', question, block.partial),
        block.partial,
      )
      .catch(() => {});
  }
