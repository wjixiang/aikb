...
import { TodoItem, TodoStatus, todoStatusSchema } from 'agent-lib/types';
...
export class UpdateTodoListTool extends BaseTool<'update_todo_list'> {
  ...
  async execute(
    ...
    const { pushToolResult, handleError, askApproval, toolProtocol } =
      callbacks;
    ...
    try {
      ...
      if (!valid) {
        ...
        pushToolResult(
          formatResponse.toolError(
            error || 'todos parameter validation failed',
          ),
        );
        return;
      }
      ...
      const isTodoListChanged =
        ...
        JSON.stringify(normalizedTodos) !== JSON.stringify(approvedTodoList);
      ...
      if (isTodoListChanged) {
        ...
        pushToolResult(
          formatResponse.toolResult('Todo list updated successfully.'),
        );
      }
    } catch (error) {
  ...
  override async handlePartial(
    task: Task,
    block: ToolUse<'update_todo_list'>,
  ): Promise<void> {
