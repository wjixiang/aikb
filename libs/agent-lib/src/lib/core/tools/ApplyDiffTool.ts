  ...
  async execute(
    ...
    try {
      if (!relPath) {
        ...
        pushToolResult(
          await task.sayAndCreateMissingParamError('apply_diff', 'path'),
        );
        return;
      }
      ...
      if (!diffContent) {
        ...
        task.recordToolError('apply_diff');
        pushToolResult(
          await task.sayAndCreateMissingParamError('apply_diff', 'diff'),
        );
        return;
      }
      ...
      if (!diffResult.success) {
        task.consecutiveMistakeCount++;
        const currentCount =
          (task.consecutiveMistakeCountForApplyDiff.get(relPath) || 0) + 1;
      ...
      if (isPreventFocusDisruptionEnabled) {
        ...
        if (task.diffStrategy && task.diffStrategy.getProgressStatus) {
          ...
          toolProgressStatus = task.diffStrategy.getProgressStatus(
            block,
            diffResult,
          );
        }
