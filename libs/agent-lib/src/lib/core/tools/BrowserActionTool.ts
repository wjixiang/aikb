...
export async function browserActionTool(
  ...
  if (!action || !browserActions.includes(action)) {
    ...
    if (!block.partial) {
      ...
      pushToolResult(
        await cline.sayAndCreateMissingParamError('browser_action', 'action'),
      );
    ...
    if (block.partial) {
      ...
      if (action === 'launch') {
        if (!url) {
          ...
          cline.didToolFailInCurrentTurn = true;
          pushToolResult(
            await cline.sayAndCreateMissingParamError('browser_action', 'url'),
          );
          // Do not close the browser on parameter validation errors
          return;
        }
        ...
        if (action === 'click' || action === 'hover') {
          ...
          try {
            processedCoordinate = scaleCoordinate(
              coordinate,
              viewportWidth,
              viewportHeight,
            );
        ...
        const sayPayload: ClineSayBrowserAction & {
        ...
        switch (action) {
          case 'click':
            browserActionResult = await cline.browserSession.click(
              processedCoordinate!,
            );
            break;
          case 'hover':
            browserActionResult = await cline.browserSession.hover(
              processedCoordinate!,
            );
            break;
          case 'type':
      ...
      switch (action) {
        ...
        case 'resize': {
          ...
          const images = browserActionResult?.screenshot
          ...
          if (
            browserActionResult?.viewportWidth &&
            browserActionResult?.viewportHeight
          ) {
