...
export class SearchFilesTool extends BaseTool<'search_files'> {
  ...
  async execute(
    ...
    if (!relDirPath) {
      ...
      pushToolResult(
        await task.sayAndCreateMissingParamError('search_files', 'path'),
      );
      return;
    }
    ...
    if (!regex) {
      ...
      task.didToolFailInCurrentTurn = true;
      pushToolResult(
        await task.sayAndCreateMissingParamError('search_files', 'regex'),
      );
      return;
    }
  ...
  override async handlePartial(
    ...
    const sharedMessageProps: ClineSayTool = {
      ...
      path: getReadablePath(
        task.cwd,
        this.removeClosingTag('path', relDirPath, block.partial),
      ),
      regex: this.removeClosingTag('regex', regex, block.partial),
      filePattern: this.removeClosingTag(
        'file_pattern',
        filePattern,
        block.partial,
      ),
