...
export class GenerateImageTool extends BaseTool<'generate_image'> {
  ...
  async execute(
    ...
    if (!prompt) {
      ...
      pushToolResult(
        await task.sayAndCreateMissingParamError('generate_image', 'prompt'),
      );
      return;
    }
    ...
    if (!relPath) {
      ...
      task.recordToolError('generate_image');
      pushToolResult(
        await task.sayAndCreateMissingParamError('generate_image', 'path'),
      );
      return;
    }
    ...
    if (inputImagePath) {
      ...
      const inputImageAccessAllowed =
        task.rooIgnoreController?.validateAccess(inputImagePath);
      if (!inputImageAccessAllowed) {
        await task.say('rooignore_error', inputImagePath);
        pushToolResult(
          formatResponse.rooIgnoreError(inputImagePath, toolProtocol),
        );
        return;
      }
    ...
    if (selectedModel) {
      modelInfo = IMAGE_GENERATION_MODELS.find(
      ...
      if (!modelInfo) {
        ...
        const providerModels = IMAGE_GENERATION_MODELS.filter(
          (m) => m.provider === imageProvider,
        );
    ...
    try {
      ...
      if (modelProvider === 'roo') {
        ...
        result = await rooHandler.generateImage(
          prompt,
          selectedModel,
          inputImageData,
          apiMethod,
      ...
      if (!result.success) {
        ...
        task.didToolFailInCurrentTurn = true;
        pushToolResult(
          formatResponse.toolError(result.error || 'Failed to generate image'),
        );
        return;
      }
      ...
      const base64Match = result.imageData.match(
      ...
      let imageUri =
        provider?.convertToWebviewUri?.(fullImagePath) ??
        vscode.Uri.file(fullImagePath).toString();

      const cacheBuster = Date.now();
      imageUri = imageUri.includes('?')
        ? `${imageUri}&t=${cacheBuster}`
        : `${imageUri}?t=${cacheBuster}`;

      await task.say(
        'image',
        JSON.stringify({ imageUri, imagePath: fullImagePath }),
      );
      pushToolResult(
        formatResponse.toolResult(getReadablePath(task.cwd, finalPath)),
      );
    } catch (error) {
