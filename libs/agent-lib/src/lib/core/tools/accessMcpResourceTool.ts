...
export class AccessMcpResourceTool extends BaseTool<'access_mcp_resource'> {
  ...
  async execute(
    ...
    try {
      ...
      resourceResult?.contents.forEach(
        (item: { mimeType: string; blob: string }) => {
  ...
  override async handlePartial(
    task: Task,
    block: ToolUse<'access_mcp_resource'>,
  ): Promise<void> {
    const server_name = this.removeClosingTag(
      'server_name',
    ...
    await task
      .ask('use_mcp_server', partialMessage, block.partial)
      .catch(() => {});
  }
}
