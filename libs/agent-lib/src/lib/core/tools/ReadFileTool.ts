...
import type { FileEntry, LineRange } from 'agent-lib/types';
import {
  isNativeProtocol,
...
export class ReadFileTool extends BaseTool<'read_file'> {
  ...
  parseLegacy(params: Partial<Record<string, string>>): { files: FileEntry[] } {
    ...
    if (argsXmlTag) {
      ...
      for (const file of files) {
        ...
        if (file.line_range) {
          const ranges = Array.isArray(file.line_range)
            ? file.line_range
            : [file.line_range];
  ...
  async execute(
    ...
    if (!fileEntries || fileEntries.length === 0) {
      ...
      const errorMsg = await task.sayAndCreateMissingParamError(
        'read_file',
        'args (containing valid file paths)',
      );
      const errorResult = useNative
        ? `Error: ${errorMsg}`
        : `<files><error>${errorMsg}</error></files>`;
    ...
    try {
      ...
      for (const fileResult of fileResults) {
        ...
        if (fileResult.lineRanges) {
          ...
          for (const range of fileResult.lineRanges) {
            if (range.start > range.end) {
              const errorMsg =
                'Invalid line range: end line cannot be less than start line';
              ...
              await task.say(
                'error',
                `Error reading file ${relPath}: ${errorMsg}`,
              );
              hasRangeError = true;
              break;
            }
        ...
        if (fileResult.status === 'pending') {
          const accessAllowed =
            task.rooIgnoreController?.validateAccess(relPath);
          if (!accessAllowed) {
        ...
        try {
          ...
          if (isBinary) {
            ...
            if (isSupportedImageFormat(fileExtension)) {
              try {
                ...
                await task.say(
                  'error',
                  `Error reading image file ${relPath}: ${errorMsg}`,
                );
                continue;
              }
            }
            ...
            if (
              supportedBinaryFormats &&
              supportedBinaryFormats.includes(fileExtension)
            ) {
          ...
          if (maxReadFileLine > 0 && totalLines > maxReadFileLine) {
            ...
            try {
              ...
              if (defResult) {
                const truncatedDefs = truncateDefinitionsToLineLimit(
                  defResult,
                  maxReadFileLine,
                );
          ...
          if (
            ...
            budgetResult.maxChars !== undefined
          ) {
            ...
            const lineRangeAttr =
              displayedLines > 0 ? ` lines="1-${displayedLines}"` : '';
            xmlInfo =
              content.length > 0
                ? `<content${lineRangeAttr}>\n${content}</content>\n`
                : `<content/>`;
      ...
      const deniedWithFeedback = fileResults.find(
        (result) => result.status === 'denied' && result.feedbackText,
      );

      if (deniedWithFeedback && deniedWithFeedback.feedbackText) {
        statusMessage = formatResponse.toolDeniedWithFeedback(
          deniedWithFeedback.feedbackText,
        );
        ...
        if (approvedWithFeedback && approvedWithFeedback.feedbackText) {
          statusMessage = formatResponse.toolApprovedWithFeedback(
            approvedWithFeedback.feedbackText,
          );
      ...
      const finalModelSupportsImages =
  ...
  getReadFileToolDescription(blockName: string, second: any): string {
    ...
    if (
      ...
      const paths = (second.files as FileEntry[])
        .map((f) => f?.path)
    ...
    if (blockParams?.args) {
      try {
        ...
        console.error(
          'Failed to parse read_file args XML for description:',
          error,
        );
      ...
      try {
        ...
        console.error(
          'Failed to parse native files JSON for description:',
          error,
        );
  ...
  override async handlePartial(
    ...
    if (
      !filePath &&
      block.nativeArgs &&
      'files' in block.nativeArgs &&
      Array.isArray(block.nativeArgs.files)
    ) {
