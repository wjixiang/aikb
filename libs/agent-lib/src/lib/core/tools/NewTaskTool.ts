...
import { TodoItem } from 'agent-lib/types';
  ...
  async execute(
    ...
    try {
      ...
      if (!mode) {
        ...
        pushToolResult(
          await task.sayAndCreateMissingParamError('new_task', 'mode'),
        );
        return;
      }
      ...
      if (!message) {
        ...
        task.didToolFailInCurrentTurn = true;
        pushToolResult(
          await task.sayAndCreateMissingParamError('new_task', 'message'),
        );
        return;
      }
      ...
      if (requireTodos && todos === undefined) {
        ...
        task.didToolFailInCurrentTurn = true;
        pushToolResult(
          await task.sayAndCreateMissingParamError('new_task', 'todos'),
        );
        return;
      }
        ...
        try {
          ...
          pushToolResult(
            formatResponse.toolError(
              'Invalid todos format: must be a markdown checklist',
            ),
          );
          return;
        }
