  ...
  async execute(
    ...
    try {
      if (!mode_slug) {
        ...
        pushToolResult(
          await task.sayAndCreateMissingParamError('switch_mode', 'mode_slug'),
        );
        return;
      }
      ...
      const targetMode = getModeBySlug(
        mode_slug,
        (await task.providerRef.deref()?.getState())?.customModes,
      );
      ...
      const currentMode =
        (await task.providerRef.deref()?.getState())?.mode ?? defaultModeSlug;
