...
export class FetchInstructionsTool extends BaseTool<'fetch_instructions'> {
  ...
  async execute(
    ...
    const { handleError, pushToolResult, askApproval, toolProtocol } =
      callbacks;
    ...
    try {
      ...
      const content = await fetchInstructions(taskParam, {
        mcpHub,
        diffStrategy,
        context,
      });
      ...
      if (!content) {
        pushToolResult(
          formatResponse.toolError(
            `Invalid instructions request: ${taskParam}`,
          ),
        );
        return;
      }
  ...
  override async handlePartial(
    task: Task,
    block: ToolUse<'fetch_instructions'>,
  ): Promise<void> {
