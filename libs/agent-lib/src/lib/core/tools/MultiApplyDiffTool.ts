...
import { DEFAULT_WRITE_DELAY_MS } from 'agent-lib/types';
...
export async function applyDiffTool(
  ...
  if (argsXmlTag) {
    ...
    try {
      ...
      const parsed = parseXmlForDiff(argsXmlTag, [
        'file.diff.content',
      ]) as ParsedXmlResult;
...
Original error: ${errorMessage}`;
      ...
      diff: [
        {
          ...
          startLine: legacyStartLineStr
            ? parseInt(legacyStartLineStr)
            : undefined,
        },
      ],
    };
  ...
  const updateOperationResult = (
    path: string,
    updates: Partial<OperationResult>,
  ) => {
  ...
  try {
    ...
    if (operationsToApprove.length > 1) {
      ...
      const hasProtectedFiles = operationsToApprove.some(
        (opResult) =>
          cline.rooProtectedController?.isWriteProtected(opResult.path) ||
          false,
      );
          ...
          const processed = !cline.api.getModel().id.includes('claude')
            ? (opResult.diffItems || []).map((item) => ({
                ...
                content: item.content
                  ? unescapeHtmlEntities(item.content)
                  : item.content,
              }))
            : opResult.diffItems || [];

          const applyRes =
            (await cline.diffStrategy?.applyDiff(original, processed)) ??
            ({ success: false } as any);
          const newContent =
            applyRes.success && applyRes.content ? applyRes.content : original;
          unified = formatResponse.createPrettyPatch(
            opResult.path,
            original,
            newContent,
          );
      ...
      if (response === 'yesButtonClicked') {
        ...
        try {
          ...
          if (
            parsedResponse.action === 'applyDiff' &&
            parsedResponse.approvedFiles
          ) {
    ...
    for (const opResult of operationResults) {
      ...
      try {
        let originalContent: string | null = await fs.readFile(
          absolutePath,
          'utf-8',
        );
        ...
        const diffResult = (await cline.diffStrategy?.applyDiff(
          ...
          success: false,
          error:
            'No diff strategy available - please ensure a valid diff strategy is configured',
        };
        ...
        if (!diffResult.success) {
          cline.consecutiveMistakeCount++;
          const currentCount =
            (cline.consecutiveMistakeCountForApplyDiff.get(relPath) || 0) + 1;
          ...
          if (diffResult.failParts && diffResult.failParts.length > 0) {
            for (let i = 0; i < diffResult.failParts.length; i++) {
              ...
              allDiffErrors.push(
                `${relPath} - Diff ${i + 1}: ${failPart.error}`,
              );
...
${errorDetails ? `\nTechnical details:\n${errorDetails}\n` : ''}
        ...
        if (successCount === 0) {
          if (formattedError) {
            const currentCount =
              cline.consecutiveMistakeCountForApplyDiff.get(relPath) || 0;
            if (currentCount >= 2) {
            ...
            if (operationsToApprove.length === 1) {
              ...
              await cline
                .ask('tool', JSON.stringify(sharedMessageProps), false)
                .catch(() => {});
            }
          }
          continue;
        ...
        if (operationsToApprove.length === 1) {
          ...
          const diffContents = diffItems
            .map((item) => item.content)
            .join('\n\n');
          const unifiedPatchRaw = formatResponse.createPrettyPatch(
            relPath,
            beforeContent!,
            originalContent!,
          );
    ...
    for (const operation of operations) {
      for (const diffItem of operation.diff) {
        const searchBlocks = (diffItem.content.match(/<<<<<<< SEARCH/g) || [])
          .length;
