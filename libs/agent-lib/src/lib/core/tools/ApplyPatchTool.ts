...
export class ApplyPatchTool extends BaseTool<'apply_patch'> {
  ...
  async execute(
    ...
    try {
      ...
      if (!patch) {
        ...
        pushToolResult(
          await task.sayAndCreateMissingParamError('apply_patch', 'patch'),
        );
        return;
      }
  ...
  private async handleUpdateFile(
    ...
    if (change.movePath) {
      ...
      const moveAccessAllowed = task.rooIgnoreController?.validateAccess(
        change.movePath,
      );
      ...
      const isMovePathWriteProtected =
        task.rooProtectedController?.isWriteProtected(change.movePath) || false;
  ...
  override async handlePartial(
    ...
    await task
      .ask('tool', JSON.stringify(sharedMessageProps), block.partial)
      .catch(() => {});
  }
}
