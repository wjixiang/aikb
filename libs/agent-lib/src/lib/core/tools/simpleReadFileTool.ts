...
import { ToolProtocol, isNativeProtocol } from 'agent-lib/types';
import {
...
export async function simpleReadFileTool(
  ...
  try {
    ...
    if (!accessAllowed) {
      ...
      pushToolResult(
        `<file><path>${relPath}</path><error>${errorMsg}</error></file>`,
      );
      return;
    }
 ...
 */
export function getSimpleReadFileToolDescription(
  blockName: string,
  blockParams: any,
): string {
  if (blockParams.path) {
