...
import { ToolProgressStatus } from 'agent-lib/types';
...
Each file requires its own path, start_line, and diff elements.
  ...
  private validateMarkerSequencing(diffContent: string): {
    ...
    const searchCount = lines.filter((l) =>
      SEARCH_PATTERN.test(l.trim()),
    ).length;
    const sepCount = lines.filter((l) => l.trim() === SEP).length;
    const replaceCount = lines.filter((l) =>
      REPLACE_PATTERN.test(l.trim()),
    ).length;

    const likelyBadStructure =
      searchCount !== replaceCount || sepCount < searchCount;
    ...
    for (const line of diffContent.split('\n')) {
      ...
      switch (state.current) {
        case State.START:
          ...
          if (REPLACE_PATTERN.test(marker))
            return reportInvalidDiffError(REPLACE, SEARCH);
          if (REPLACE_PREFIX_PATTERN.test(marker))
            return reportMergeConflictError(marker, SEARCH);
          if (SEARCH_PATTERN.test(marker)) state.current = State.AFTER_SEARCH;
          else if (SEARCH_PREFIX_PATTERN.test(marker))
            return reportMergeConflictError(marker, SEARCH);
          break;

        case State.AFTER_SEARCH:
          if (SEARCH_PATTERN.test(marker))
            return reportInvalidDiffError(SEARCH, SEP);
          if (SEARCH_PREFIX_PATTERN.test(marker))
            return reportMergeConflictError(marker, SEARCH);
          if (REPLACE_PATTERN.test(marker))
            return reportInvalidDiffError(REPLACE, SEP);
          if (REPLACE_PREFIX_PATTERN.test(marker))
            return reportMergeConflictError(marker, SEARCH);
        ...
        case State.AFTER_SEPARATOR:
          if (SEARCH_PATTERN.test(marker))
            return reportInvalidDiffError(SEARCH, REPLACE);
          if (SEARCH_PREFIX_PATTERN.test(marker))
          ...
          else if (REPLACE_PREFIX_PATTERN.test(marker))
            return reportMergeConflictError(marker, REPLACE);
          break;
      }
    }
  ...
  private async applySingleDiff(
    ...
    for (const replacement of replacements) {
      ...
      let startLine =
        replacement.startLine + (replacement.startLine === 0 ? 0 : delta);
      ...
      if (startLine) {
        ...
        if (similarity >= this.fuzzyThreshold) {
          ...
          searchEndIndex = Math.min(
            resultLines.length,
            startLine + searchLines.length + this.bufferLines,
          );
        }
      }
