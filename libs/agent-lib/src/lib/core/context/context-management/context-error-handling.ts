...
function checkIsOpenRouterContextWindowError(error: unknown): boolean {
  try {
    ...
    const status =
      err.status ?? err.code ?? err.error?.status ?? err.response?.status;
...
function checkIsOpenAIContextWindowError(error: unknown): boolean {
  try {
    ...
    return (
      ...
      KNOWN_CONTEXT_ERROR_SUBSTRINGS.some((substring) =>
...
function checkIsCerebrasContextWindowError(response: unknown): boolean {
  try {
    ...
    const res = response as Record<string, any>;
    const status =
      res.status ?? res.code ?? res.error?.status ?? res.response?.status;
