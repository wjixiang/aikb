...
export class MessageEnhancer {
   ...
   */
  static async enhanceMessage(
    options: MessageEnhancerOptions,
  ): Promise<MessageEnhancerResult> {
    try {
      const {
        text,
        apiConfiguration,
      ...
      if (
        ...
        const { name: _, ...providerSettings } =
          await providerSettingsManager.getProfile({
      ...
      const enhancedText = await singleCompletionHandler(
        configToUse,
        enhancementPrompt,
      );
