...
import type { ClineMessage, TodoItem } from 'agent-lib/types';
import {
  readApiMessages,
  saveApiMessages,
  saveTaskMessages,
    ...
    TaskProviderLike
{
  ...
  private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
    ...
    try {
      ...
      if (fs.existsSync(portFilePath)) {
        ...
        console.log(
          `[ClineProvider:Vite] Using Vite server port from ${portFilePath}: ${localPort}`,
        );
      } else {
        console.log(
  ...
  > {
    ...
    const apiProvider: ProviderName = stateValues.apiProvider
  ...
  public async cancelTask(): Promise<void> {
    ...
    // Final race check before rehydrate to avoid duplicate rehydration
    {
      ...
      if (
        currentAfterCheck &&
        currentAfterCheck.instanceId !== originalInstanceId
      ) {
        this.log(
