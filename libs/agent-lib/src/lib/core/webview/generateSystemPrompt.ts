...
import {
  experiments as experimentsModule,
  EXPERIMENT_IDS,
...
export const generateSystemPrompt = async (
  ...
  const rooIgnoreInstructions = provider
    .getCurrentTask()
  ...
  try {
    ...
    console.error(
      'Error checking if model supports browser capability:',
      error,
    );
  }
  ...
  const modelSupportsBrowser =
    modelInfo && (modelInfo as any)?.supportsImages === true;

  // Only enable browser tools if the model supports it, the mode includes browser tools,
  // and browser tools are enabled in settings
  const canUseBrowserTool =
    modelSupportsBrowser && modeSupportsBrowser && (browserToolEnabled ?? true);
