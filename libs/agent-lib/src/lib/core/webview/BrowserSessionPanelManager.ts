...
import type { ClineMessage } from 'agent-lib/types';
...
export class BrowserSessionPanelManager {
  private static instances: WeakMap<ClineProvider, BrowserSessionPanelManager> =
    new WeakMap();
  ...
  public async show(): Promise<void> {
    ...
    if (task) {
      ...
      await this.updateBrowserSession(
    ...
    this.panel.webview.onDidReceiveMessage(
      async (message: any) => {
        try {
          ...
          console.error(
            '[BrowserSessionPanel] onDidReceiveMessage error:',
            err,
          );
        }
      },
  ...
  public async updateBrowserSession(
  ...
  private async getHMRHtmlContent(
    ...
    try {
      ...
      console.error(
        '[BrowserSessionPanel:Vite] Failed to read port file:',
        err,
      );
    }
