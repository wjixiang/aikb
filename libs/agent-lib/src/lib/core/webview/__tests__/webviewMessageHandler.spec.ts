...
describe('webviewMessageHandler - deleteCustomMode', () => {
  beforeEach(() => {
    ...
    vi.mocked(ensureSettingsDirectoryExists).mockResolvedValue(
      '/mock/global/storage/.roo',
    );
  });
  ...
  it('should delete a project mode and its rules folder', async () => {
    ...
    vi.mocked(
      mockClineProvider.customModesManager.getCustomModes,
    ).mockResolvedValue([
      {
    ...
    vi.mocked(
      mockClineProvider.customModesManager.deleteCustomMode,
