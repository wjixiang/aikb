...
import { TelemetryEventName, type TelemetrySetting } from 'agent-lib/types';
...
describe('Telemetry Settings Tracking', () => {
  ...
  describe('when telemetry is turned OFF', () => {
    it('should fire event BEFORE disabling telemetry', () => {
      ...
      ).toHaveBeenCalledWith('enabled', 'disabled');
      expect(
        mockTelemetryService.captureTelemetrySettingsChanged,
      ).toHaveBeenCalledBefore(
        mockTelemetryService.updateTelemetryState as any,
      );
      expect(mockTelemetryService.updateTelemetryState).toHaveBeenCalledWith(
        false,
      );
    });
    ...
    it('should fire event when going from unset to disabled', () => {
      ...
      expect(
        mockTelemetryService.captureTelemetrySettingsChanged,
      ).toHaveBeenCalledWith('unset', 'disabled');
    });
  });
  ...
  describe('when telemetry is turned ON', () => {
    it('should fire event AFTER enabling telemetry', () => {
      ...
      expect(
        mockTelemetryService.captureTelemetrySettingsChanged,
      ).toHaveBeenCalledWith('disabled', 'enabled');
