...
import { getModelId } from 'agent-lib/types';
...
describe('ClineProvider - API Handler Rebuild Guard', () => {
    ...
    mockContext = {
      ...
      globalState: {
        ...
        update: vi
          .fn()
          .mockImplementation(
            (key: string, value: any) => (globalState[key] = value),
          ),
  ...
  describe('upsertProviderProfile', () => {
    test('calls updateApiConfiguration when provider/model unchanged but profile settings changed (explicit save)', async () => {
      ...
      expect((mockTask as any).apiConfiguration.openRouterModelId).toBe(
        'openai/gpt-4',
      );
  ...
  describe('activateProviderProfile', () => {
    test('calls updateApiConfiguration when provider/model unchanged but settings differ (explicit profile switch)', async () => {
      ...
      (provider as any).providerSettingsManager.activateProfile = vi
        .fn()
        .mockResolvedValue({
          name: 'test-config',
          id: 'test-id',
    ...
    test('calls updateApiConfiguration when provider changes and syncs task.apiConfiguration', async () => {
      ...
      expect((mockTask as any).apiConfiguration.apiModelId).toBe(
  ...
  describe('getModelId helper', () => {
    test('correctly extracts model ID from different provider configurations', () => {
      ...
      expect(
        getModelId({ apiProvider: 'openai', openAiModelId: 'gpt-4-turbo' }),
      ).toBe('gpt-4-turbo');
      expect(
        getModelId({ apiProvider: 'glama', glamaModelId: 'some-model' }),
      ).toBe('some-model');
      expect(
        getModelId({
          apiProvider: 'bedrock',
