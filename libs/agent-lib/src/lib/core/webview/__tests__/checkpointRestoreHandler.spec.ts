...
describe('checkpointRestoreHandler', () => {
  ...
  describe('handleCheckpointRestoreOperation', () => {
    it('should abort task before checkpoint restore for delete operations', async () => {
      ...
      const restoreOrder =
        mockCline.checkpointRestore.mock.invocationCallOrder[0];
    ...
    it('should handle edit operations with pending edit data', async () => {
      ...
      expect(mockProvider.setPendingEditOperation).toHaveBeenCalledWith(
        'task-test-task-123',
        {
          messageTs: 3,
          editedContent: 'Edited content',
    ...
    it('should handle errors gracefully', async () => {
      ...
      mockCline.checkpointRestore.mockRejectedValue(
        new Error('Checkpoint restore failed'),
      );
