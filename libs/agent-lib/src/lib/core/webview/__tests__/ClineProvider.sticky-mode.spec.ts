...
import type { HistoryItem, ProviderName } from 'agent-lib/types';
...
describe('ClineProvider - Sticky Mode', () => {
  ...
  describe('createTaskWithHistoryItem', () => {
    it('should restore mode from history item when reopening task', async () => {
      ...
      const updateGlobalStateSpy = vi
        .spyOn(provider as any, 'updateGlobalState')
    ...
    it('should use current mode if history item has no saved mode', async () => {
      ...
      const handleModeSwitchSpy = vi
        .spyOn(provider, 'handleModeSwitch')
        .mockResolvedValue();
  ...
  describe('Error handling', () => {
    it('should handle errors gracefully when saving mode fails', async () => {
      ...
      vi.spyOn(mockTask as any, 'saveClineMessages').mockRejectedValue(
    ...
    it('should restore API configuration when restoring task from history with mode', async () => {
      ...
      const architectConfigId =
        provider.getProviderProfileEntry('architect-config')?.id;
  ...
  describe('Concurrent mode switches', () => {
    it('should handle concurrent mode switches on the same task', async () => {
      ...
      const lastCall =
        updateTaskHistorySpy.mock.calls[
          updateTaskHistorySpy.mock.calls.length - 1
        ];
  ...
  describe('Mode switch failure scenarios', () => {
    ...
    it('should handle errors during mode switch gracefully', async () => {
      ...
      await expect(provider.handleModeSwitch('architect')).rejects.toThrow(
        'Emit failed',
      );
    ...
    it('should handle updateTaskHistory failures', async () => {
      ...
      vi.spyOn(provider, 'updateTaskHistory').mockRejectedValue(
  ...
  describe('Multiple tasks switching modes simultaneously', () => {
    it('should handle multiple tasks switching modes independently', async () => {
      ...
      expect(task1.emit).toHaveBeenCalledWith(
        'taskModeSwitched',
        task1.taskId,
        'architect',
      );
      expect(task2.emit).toHaveBeenCalledWith(
        'taskModeSwitched',
        task2.taskId,
        'debug',
      );
      expect(task3.emit).toHaveBeenCalledWith(
        'taskModeSwitched',
        task3.taskId,
        'code',
      );
