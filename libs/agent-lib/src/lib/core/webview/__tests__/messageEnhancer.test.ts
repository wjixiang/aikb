import { ProviderSettings, ClineMessage } from 'agent-lib/types';
...
describe('MessageEnhancer', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(singleCompletionHandlerModule).singleCompletionHandler =
  ...
  describe('enhanceMessage', () => {
    ...
    it('should limit task history to last 10 messages', async () => {
      // Create 15 messages
      const mockClineMessages: ClineMessage[] = Array.from(
        { length: 15 },
        (_, i) => ({
          type: i % 2 === 0 ? 'ask' : 'say',
    ...
    it('should truncate long messages in task history', async () => {
      ...
      const mockClineMessages: ClineMessage[] = [
        { type: 'ask', text: longText, ts: 1000 },
      ];
  ...
  describe('extractTaskHistory', () => {
    ...
    it('should handle malformed messages gracefully', () => {
      ...
      // Access private method through any type assertion for testing
      const history = (MessageEnhancer as any).extractTaskHistory(
        malformedMessages,
      );
