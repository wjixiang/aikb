...
const mockAddCustomInstructions = vi
  .fn()
  .mockResolvedValue('Combined instructions');

(
  vi.mocked(await import('../../prompts/sections/custom-instructions')) as any
...
describe('ClineProvider', () => {
    ...
    mockContext = {
      ...
      secrets: {
        ...
        store: vi
          .fn()
          .mockImplementation(
            (key: string, value: string | undefined) => (secrets[key] = value),
          ),
        delete: vi
          .fn()
          .mockImplementation((key: string) => delete secrets[key]),
      },
      subscriptions: [],
      extension: {
  ...
  test('handles webviewDidLaunch message', async () => {
    ...
    const messageHandler = (mockWebviewView.webview.onDidReceiveMessage as any)
      .mock.calls[0][0];
  ...
  describe('clearTask message handler', () => {
    ...
    test('calls clearTask (delegation handled via metadata)', async () => {
      ...
      const clearTaskSpy = vi
        .spyOn(provider, 'clearTask')
        .mockResolvedValue(undefined);
      const postStateToWebviewSpy = vi
      ...
      const messageHandler = (
        mockWebviewView.webview.onDidReceiveMessage as any
      ).mock.calls[0][0];
  ...
  test('handles writeDelayMs message', async () => {
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { writeDelayMs: 2000 },
    });
  ...
  test('updates sound utility when sound setting changes', async () => {
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { soundEnabled: false },
    });
    expect(mockContext.globalState.update).toHaveBeenCalledWith(
      'soundEnabled',
      false,
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { ttsEnabled: true },
    });
  ...
  test('handles autoCondenseContext message', async () => {
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { autoCondenseContext: false },
    });
  ...
  test('handles autoCondenseContextPercent message', async () => {
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { autoCondenseContextPercent: 75 },
    });
  ...
  it('loads saved API config when switching modes', async () => {
    ...
    expect(
      provider.providerSettingsManager.getModeConfigId,
    ).toHaveBeenCalledWith('architect');
    expect(
      provider.providerSettingsManager.activateProfile,
    ).toHaveBeenCalledWith({ name: 'test-config' });
    expect(mockContext.globalState.update).toHaveBeenCalledWith(
  ...
  it('load API configuration by ID works and updates mode config', async () => {
    ...
    expect(
      provider.providerSettingsManager.activateProfile,
    ).toHaveBeenCalledWith({ id: 'config-id-123' });
  });
  ...
  test('handles browserToolEnabled setting', async () => {
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { browserToolEnabled: true },
    });
    expect(mockContext.globalState.update).toHaveBeenCalledWith(
      'browserToolEnabled',
      true,
  ...
  test('handles showRooIgnoredFiles setting', async () => {
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { showRooIgnoredFiles: true },
    });
    expect(mockContext.globalState.update).toHaveBeenCalledWith(
      'showRooIgnoredFiles',
      true,
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { showRooIgnoredFiles: false },
    });
    expect(mockContext.globalState.update).toHaveBeenCalledWith(
      'showRooIgnoredFiles',
      false,
  ...
  test('handles request delay settings messages', async () => {
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { alwaysApproveResubmit: true },
    });
    expect(updateGlobalStateSpy).toHaveBeenCalledWith(
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { requestDelaySeconds: 10 },
    });
    expect(mockContext.globalState.update).toHaveBeenCalledWith(
      'requestDelaySeconds',
      10,
  ...
  test('handles maxWorkspaceFiles message', async () => {
    ...
    await messageHandler({
      type: 'updateSettings',
      updatedSettings: { maxWorkspaceFiles: 300 },
    });
  ...
  describe('deleteMessage', () => {
    ...
    test('handles deletion with confirmation dialog', async () => {
      ...
      expect(
        (provider as any).createTaskWithHistoryItem,
  ...
  describe('editMessage', () => {
    ...
    test('handles edit with confirmation dialog', async () => {
      ...
      expect(
        (mockWebviewView.webview.onDidReceiveMessage as any).mock.calls.length,
  ...
  describe('getSystemPrompt', () => {
    ...
    const getMessageHandler = () => {
      const mockCalls = (mockWebviewView.webview.onDidReceiveMessage as any)
        .mock.calls;
  ...
  describe('handleModeSwitch', () => {
    ...
    it('loads saved API config when switching modes', async () => {
      ...
      expect(
        provider.providerSettingsManager.activateProfile,
      ).toHaveBeenCalledWith({ name: 'saved-config' });
      expect(mockContext.globalState.update).toHaveBeenCalledWith(
        'currentApiConfigName',
        'saved-config',
      );
    ...
    test('saves current config when switching to mode without config', async () => {
      ...
      expect(
        provider.providerSettingsManager.setModeConfig,
      ).toHaveBeenCalledWith('architect', 'current-id');
  ...
  describe('createTaskWithHistoryItem mode validation', () => {
    test('validates and falls back to default mode when restored mode no longer exists', async () => {
      ...
      expect(getModeBySlug).toHaveBeenCalledWith(
        'non-existent-mode',
        expect.any(Array),
      );
    ...
    test('preserves mode when it exists in custom modes', async () => {
      ...
      (provider as any).providerSettingsManager = {
        ...
        listConfig: vi
          .fn()
          .mockResolvedValue([
            { name: 'test-config', id: 'config-id', apiProvider: 'anthropic' },
          ]),
    ...
    test('continues with task restoration even if mode config loading fails', async () => {
      ...
      (provider as any).providerSettingsManager = {
        ...
        activateProfile: vi
          .fn()
          .mockRejectedValue(new Error('Failed to load config')),
      };
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "Failed to restore API configuration for mode 'code'",
        ),
      );
    });
  });
  ...
  describe('upsertApiConfiguration', () => {
    test('handles error in upsertApiConfiguration gracefully', async () => {
      ...
      (provider as any).providerSettingsManager = {
        setModeConfig: vi
          .fn()
          .mockRejectedValue(new Error('Failed to update mode config')),
        listConfig: vi
          .fn()
          .mockResolvedValue([
            { name: 'test-config', id: 'test-id', apiProvider: 'anthropic' },
  ...
  describe('browser connection features', () => {
    ...
    test('handles testBrowserConnection with auto-discovery', async () => {
      ...
      const { discoverChromeHostUrl } = await import(
        '../../../services/browser/browserDiscovery'
      );
      ...
      expect(mockPostMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          text: expect.stringContaining(
            'Auto-discovered and tested connection to Chrome',
          ),
        }),
      );
    });
...
describe('ClineProvider - Router Models', () => {
  ...
  test('handles requestRouterModels with successful responses', async () => {
    ...
    const { getModels } = await import(
      '../../../api/providers/fetchers/modelCache'
    );
...
describe('ClineProvider - Comprehensive Edit/Delete Edge Cases', () => {
  ...
  describe('Edit Messages with Images and Attachments', () => {
    ...
    test('handles editing messages containing images', async () => {
      ...
      expect(mockCline.submitUserMessage).toHaveBeenCalledWith(
  ...
  describe('Network Failure Scenarios', () => {
    ...
    test('handles network timeout during edit submission', async () => {
      ...
      mockCline.overwriteClineMessages = vi.fn();
      mockCline.overwriteApiConversationHistory = vi.fn();
      mockCline.handleWebviewAskResponse = vi
        .fn()
    ...
    test('handles connection drops during edit operation', async () => {
      ...
      mockCline.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as any[];
      mockCline.overwriteClineMessages = vi
        .fn()
      ...
      expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
        'errors.message.error_editing_message',
      );
    });
  });
  ...
  describe('Edit Permissions and Authorization', () => {
    ...
    describe('Resource Cleanup During Failed Operations', () => {
      ...
      test('validates proper cleanup during failed delete operations', async () => {
        ...
        expect(cleanupSpy).toHaveBeenCalled();
        expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
          'errors.message.error_deleting_message',
        );
      });
    });
    ...
    describe('Error Messaging and User Feedback', () => {
      ...
      test('handles user cancellation gracefully', async () => {
        ...
        expect(mockCline.overwriteClineMessages).not.toHaveBeenCalled();
        expect(
          mockCline.overwriteApiConversationHistory,
        ).not.toHaveBeenCalled();
        expect(mockCline.handleWebviewAskResponse).not.toHaveBeenCalled();
        expect(vscode.window.showErrorMessage).not.toHaveBeenCalled();
      });
