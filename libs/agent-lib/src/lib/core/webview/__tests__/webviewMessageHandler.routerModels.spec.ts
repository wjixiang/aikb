...
describe('webviewMessageHandler - requestRouterModels provider filter', () => {
  ...
  beforeEach(() => {
    ...
    getModelsMock.mockImplementation(async (options: any) => {
      switch (options?.provider) {
        case 'roo':
          return {
            'roo/sonnet': { contextWindow: 8192, supportsPromptCache: false },
          };
        case 'openrouter':
          return {
            'openrouter/qwen2.5': {
        ...
        case 'glama':
          return {
            'glama/model': { contextWindow: 8192, supportsPromptCache: false },
          };
        case 'unbound':
          return {
            'unbound/model': {
        ...
        case 'vercel-ai-gateway':
          return {
            'vercel/model': { contextWindow: 8192, supportsPromptCache: false },
          };
  ...
  it("fetches only requested provider when values.provider is present ('roo')", async () => {
    ...
    expect(mockProvider.postMessageToWebview).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'routerModels',
        routerModels: expect.any(Object),
      }),
    );
    ...
    const routerModels = payload.routerModels as Record<
      string,
      Record<string, any>
    >;
    ...
    const providersCalled = getModelsMock.mock.calls.map(
      (c: any[]) => c[0]?.provider,
    );
  ...
  it('defaults to aggregate fetching when no provider filter is sent', async () => {
    ...
    expect(call).toBeTruthy();
    const routerModels = call[0].routerModels as Record<
      string,
      Record<string, any>
    >;
  ...
  it("supports filtering another single provider ('openrouter')", async () => {
    ...
    expect(Object.keys(routerModels.openrouter || {})).toContain(
      'openrouter/qwen2.5',
    );
