...
describe('RooProtectedController', () => {
  ...
  describe('isWriteProtected', () => {
    ...
    it('should protect .code-workspace files', () => {
      expect(controller.isWriteProtected('myproject.code-workspace')).toBe(
        true,
      );
      ...
      expect(
        controller.isWriteProtected('folder/workspace.code-workspace'),
      ).toBe(true);
    });
  ...
  describe('getProtectedFiles', () => {
    it('should return set of protected files from a list', () => {
      ...
      expect(protectedFiles).toEqual(
        new Set(['.rooignore', '.roo/config.json']),
      );
    });
