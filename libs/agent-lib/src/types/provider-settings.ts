...
export const isLocalProvider = (key: string): key is LocalProvider =>
  localProviders.includes(key as LocalProvider);
...
export const isFauxProvider = (key: string): key is FauxProvider =>
...
const minimaxSchema = apiModelIdProviderModelSchema.extend({
  minimaxBaseUrl: z
    .union([
      z.literal('https://api.minimax.io/v1'),
      z.literal('https://api.minimaxi.com/v1'),
    ])
    .optional(),
...
export const zaiApiLineSchema = z.enum([
...
export const providerSettingsSchemaDiscriminated = z.discriminatedUnion(
  'apiProvider',
  [
    ...
    openAiNativeSchema.merge(
      z.object({ apiProvider: z.literal('openai-native') }),
    ),
    ...
    huggingFaceSchema.merge(
      z.object({ apiProvider: z.literal('huggingface') }),
    ),
    ...
    featherlessSchema.merge(
      z.object({ apiProvider: z.literal('featherless') }),
    ),
    ioIntelligenceSchema.merge(
      z.object({ apiProvider: z.literal('io-intelligence') }),
    ),
    qwenCodeSchema.merge(z.object({ apiProvider: z.literal('qwen-code') })),
    rooSchema.merge(z.object({ apiProvider: z.literal('roo') })),
    vercelAiGatewaySchema.merge(
      z.object({ apiProvider: z.literal('vercel-ai-gateway') }),
    ),
    defaultSchema,
  ],
);
...
export const discriminatedProviderSettingsWithIdSchema =
  providerSettingsSchemaDiscriminated.and(
...
export type TypicalProvider = Exclude<
  ProviderName,
  InternalProvider | CustomProvider | FauxProvider
>;

export const isTypicalProvider = (key: unknown): key is TypicalProvider =>
  isProviderName(key) &&
  !isInternalProvider(key) &&
  !isCustomProvider(key) &&
  !isFauxProvider(key);
