...
describe('Text normalization utilities', () => {
  describe('normalizeString', () => {
    it('normalizes smart quotes by default', () => {
      ...
      ).toBe('These are "smart quotes" and \'single quotes\'');
    });

    it('normalizes typographic characters by default', () => {
      expect(
        normalizeString('This has an em dash \u2014 and ellipsis\u2026'),
      ).toBe('This has an em dash - and ellipsis...');
    });

    it('normalizes whitespace by default', () => {
      expect(normalizeString('Multiple   spaces and\t\ttabs')).toBe(
    ...
    it('real-world example with mixed characters', () => {
      const input =
        'Let\u2019s test this\u2014with some \u201Cfancy\u201D punctuation\u2026 and   spaces';
      expect(normalizeString(input)).toBe(
        'Let\'s test this-with some "fancy" punctuation... and spaces',
      );
    });
  });
  ...
  describe('unescapeHtmlEntities', () => {
    it('unescapes basic HTML entities', () => {
      expect(unescapeHtmlEntities('&lt;div&gt;Hello&lt;/div&gt;')).toBe(
        '<div>Hello</div>',
      );
    });
    ...
    it('unescapes quote entities', () => {
      expect(
        unescapeHtmlEntities('&quot;quoted&quot; and &#39;single-quoted&#39;'),
      ).toBe('"quoted" and \'single-quoted\'');
    });
    ...
    it('handles empty or undefined input', () => {
      ...
      expect(unescapeHtmlEntities(undefined as unknown as string)).toBe(
        undefined,
      );
    });
    ...
    it('unescapes square bracket entities (numeric)', () => {
      ...
      expect(unescapeHtmlEntities('matrix&#91;i&#93;&#91;j&#93;')).toBe(
        'matrix[i][j]',
      );
    });
    ...
    it('unescapes square bracket entities (named)', () => {
      ...
      expect(unescapeHtmlEntities('matrix&lsqb;i&rsqb;&lsqb;j&rsqb;')).toBe(
        'matrix[i][j]',
      );
    });
