...
export interface IVirtualWorkspace {
  ...
  // ==================== External Renderer Management ====================

  /**
   * Register an external renderer for supplemental content
   * External renderers are called during workspace render to include
   * content from sub-modules that are not registered as components
   */
  ...
  /**
   * Unregister an external renderer
   */
...
export interface ExportConfig {
  /** Whether to auto-export results */
  autoExport: boolean;
  /** S3 bucket for export */
  bucket: string;
  /** Default export path with placeholders like {expertId}, {timestamp} */
  defaultPath: string;
}
