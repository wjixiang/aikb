...
describe('convertToMistralMessages', () => {
  ...
  it('should handle user messages with only tool results', () => {
    ...
    expect(mistralMessages[0].content).toBe(
      'Current temperature in London: 20°C',
    );
  });
  ...
  it('should handle a conversation with mixed message types', () => {
    const anthropicMessages: Anthropic.Messages.MessageParam[] = [
      ...
      },
      {
        role: 'assistant',
        content:
          'Based on the search results, I can tell you more about the mountains in the image.',
      },
    ];
    ...
    expect(mistralMessages[1].content).toBe(
      'This image shows a landscape with mountains.',
    );
    ...
    expect(mistralMessages[2].content).toBe(
      'Found information about different mountain types.',
    );
