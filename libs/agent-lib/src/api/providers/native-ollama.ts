...
export class NativeOllamaHandler
  ...
  private convertToolsToOllama(
    ...
    return tools
      .filter(
        (tool): tool is OpenAI.Chat.ChatCompletionTool & { type: 'function' } =>
          tool.type === 'function',
      )
      .map((tool) => ({
        type: tool.type,
    ...
    try {
      ...
      const chatOptions: OllamaChatOptions = {
        temperature:
          ...
          (useR1Format ? DEEP_SEEK_DEFAULT_TEMPERATURE : 0),
      };
      ...
      try {
        ...
        throw new Error(
          `Ollama stream processing error: ${streamError.message || 'Unknown error'}`,
        );
      }
    } catch (error: any) {
      ...
      console.error(
        `Ollama API error (${statusCode || 'unknown'}): ${errorMessage}`,
      );
      throw error;
    }
  }
