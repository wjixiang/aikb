...
export class AwsBedrockHandler
  ...
  constructor(options: ProviderSettings) {
    ...
    if (this.options.awsCustomArn) {
      ...
      if (
        this.arnInfo.region &&
        this.arnInfo.region !== this.options.awsRegion
      ) {
      ...
      if (this.arnInfo.awsUseCrossRegionInference)
  ...
  override async *createMessage(
    ...
    const inferenceConfig: BedrockInferenceConfig = {
      maxTokens:
        modelConfig.maxTokens || (modelConfig.info.maxTokens as number),
      temperature:
        modelConfig.temperature ?? (this.options.modelTemperature as number),
    };
    ...
    try {
      ...
      for await (const chunk of response.stream) {
        ...
        if (streamEvent.metadata?.usage) {
          ...
          const cacheReadTokens =
            usage.cacheReadInputTokens || usage.cacheReadInputTokenCount || 0;
          const cacheWriteTokens =
            usage.cacheWriteInputTokens || usage.cacheWriteInputTokenCount || 0;
        ...
        if (streamEvent?.trace?.promptRouter?.invokedModelId) {
          try {
            ...
            let invokedArnInfo = this.parseArn(
              streamEvent.trace.promptRouter.invokedModelId,
            );
            ...
            if (streamEvent?.trace?.promptRouter?.usage) {
              ...
              // Check both field naming conventions for cache tokens
              const cacheReadTokens =
                routerUsage.cacheReadTokens ||
                routerUsage.cacheReadInputTokenCount ||
                0;
              const cacheWriteTokens =
                routerUsage.cacheWriteTokens ||
                routerUsage.cacheWriteInputTokenCount ||
                0;
        ...
        if (streamEvent.contentBlockStart) {
          ...
          // cbStart.content_block: alternative structure seen in some AWS SDK versions
            ...
            cbStart.contentBlock?.type === 'thinking' ||
            cbStart.content_block?.type === 'thinking'
          ) {
          ...
          // Handle tool use block start
            ...
            const toolUse =
              cbStart.start?.toolUse || cbStart.contentBlock?.toolUse;
            if (toolUse) {
              yield {
      ...
      const enhancedErrorMessage = this.formatErrorMessage(
        error,
  ...
  private convertToBedrockConverseMessages(
    ...
    const convertedMessages = sharedConverter(
      anthropicMessages as Anthropic.Messages.MessageParam[],
      {
        useNativeTools,
      },
    );
    ...
    if (!usePromptCache) {
      return {
        system: systemMessage
          ? [{ text: systemMessage } as SystemContentBlock]
          : [],
  ...
  private parseArn(arn: string, region?: string) {
    ...
    if (match && match[1] && match[3] && match[4]) {
      ...
      if (originalModelId && result.modelId !== originalModelId) {
        ...
        result.crossRegionInference =
          AwsBedrockHandler.isSystemInferenceProfile(prefix);
      }
    ...
    return {
      ...
      modelId: undefined,
      errorMessage:
        'Invalid ARN format. ARN should follow the Amazon Bedrock ARN pattern.',
  ...
  getModelById(
    ...
    if (baseModelId in bedrockModels) {
      ...
      model = {
        ...
        info: JSON.parse(
          JSON.stringify(bedrockModels[bedrockDefaultPromptRouterModelId]),
        ),
      };
    } else {
    ...
    if (
      ...
      this.options.awsModelContextWindow > 0
    ) {
  ...
  override getModel(): {
    ...
    if (this.options.awsCustomArn) {
      modelConfig = this.getModelById(
        this.arnInfo.modelId,
        this.arnInfo.modelType,
      );
      ...
      // Otherwise, add cross-region inference prefix if enabled
        ...
        const prefix = AwsBedrockHandler.getPrefixForRegion(
          this.options.awsRegion,
        );
        if (prefix) {
  ...
  private static getPrefixForRegion(region: string): string | undefined {
    ...
    for (const [
      regionPattern,
      inferenceProfile,
    ] of AWS_INFERENCE_PROFILE_MAPPING) {
...
- Model ID is correct for the requested features`,
  ...
  private getErrorType(error: unknown): string {
    ...
    if (
      ...
      (error as any).__type === 'ThrottlingException'
    ) {
      return 'THROTTLING';
    }
  ...
  private formatErrorMessage(
    ...
    const definition =
      AwsBedrockHandler.ERROR_TYPES[errorType] ||
      AwsBedrockHandler.ERROR_TYPES['GENERIC'];
    ...
    if (error instanceof Error) {
      ...
      templateVars['contextWindow'] = String(
        modelConfig.info.contextWindow || 'unknown',
      );
    }
