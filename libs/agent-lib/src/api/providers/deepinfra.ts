  ...
  implements SingleCompletionHandler
{
  ...
  override async *createMessage(
    ...
    const useNativeTools =
      supportsNativeTools &&
      _metadata?.tools &&
      _metadata.tools.length > 0 &&
      _metadata?.toolProtocol !== 'xml';
    ...
    const requestOptions: OpenAI.Chat.Completions.ChatCompletionCreateParamsStreaming =
      {
        ...
        ...(useNativeTools &&
          _metadata.tool_choice && { tool_choice: _metadata.tool_choice }),
        ...(useNativeTools && {
    ...
    if (this.options.includeMaxTokens === true && info.maxTokens) {
      (requestOptions as any).max_completion_tokens =
        this.options.modelMaxTokens || info.maxTokens;
    }
