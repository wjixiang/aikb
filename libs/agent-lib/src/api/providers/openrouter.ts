...
import type { ApiHandlerOptions, ModelRecord } from '../index';

import { convertToOpenAiMessages } from '../transform/openai-format';
import { resolveToolProtocol } from '../../utils/resolveToolProtocol';
import { TOOL_PROTOCOL } from '../../types';
  ...
  implements SingleCompletionHandler
{
  ...
  constructor(options: ApiHandlerOptions) {
    ...
    this.loadDynamicModels().catch((error) => {
      console.error(
        '[OpenRouterHandler] Failed to load dynamic models:',
        error,
      );
    });
  }
  ...
  getReasoningDetails(): any[] | undefined {
    return this.currentReasoningDetails.length > 0
  ...
  override async *createMessage(
    ...
    if (isNativeProtocol && isGemini) {
      openAiMessages = openAiMessages.map((msg) => {
        if (msg.role === 'assistant') {
          ...
          const existingDetails = (msg as any).reasoning_details as
            | any[]
            | undefined;
          ...
          if (toolCalls && toolCalls.length > 0) {
            const hasEncrypted =
              existingDetails?.some((d) => d.type === 'reasoning.encrypted') ??
              false;
    ...
    for await (const chunk of stream) {
      ...
      if (finishReason) {
        const endEvents =
          NativeToolCallParser.processFinishReason(finishReason);
    ...
    if (reasoningDetailsAccumulator.size > 0) {
      this.currentReasoningDetails = Array.from(
    ...
    if (lastUsage) {
      yield {
        ...
        totalCost:
          (lastUsage.cost_details?.upstream_inference_cost || 0) +
          (lastUsage.cost || 0),
      };
    }
  }
  ...
  override getModel() {
    ...
    if (
      this.options.openRouterSpecificProvider &&
      this.endpoints[this.options.openRouterSpecificProvider]
    ) {
