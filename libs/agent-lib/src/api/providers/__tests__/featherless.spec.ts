...
describe('FeatherlessHandler', () => {
  ...
  it('should return default model when no model is specified', () => {
    ...
    expect(model.info).toEqual(
      expect.objectContaining(featherlessModels[featherlessDefaultModelId]),
    );
  });
  ...
  it('should return specified model when valid model is provided', () => {
    ...
    expect(model.info).toEqual(
      expect.objectContaining(featherlessModels[testModelId]),
    );
  });
  ...
  it('createMessage should pass correct parameters to Featherless client for DeepSeek R1', async () => {
    ...
    const messages: Anthropic.Messages.MessageParam[] = [
      { role: 'user', content: 'Test message for Featherless' },
    ];
