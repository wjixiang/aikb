...
import { type ModelInfo, geminiDefaultModelId } from '../../../types';
...
describe('GeminiHandler', () => {
  ...
  describe('createMessage', () => {
    ...
    it('should handle text messages correctly', async () => {
      ...
      expect(
        handler['client'].models.generateContentStream,
    ...
    it('should handle API errors', async () => {
      ...
      (handler['client'].models.generateContentStream as any).mockRejectedValue(
        mockError,
      );
  ...
  describe('completePrompt', () => {
    ...
    it('should handle API errors', async () => {
      const mockError = new Error('Gemini API error');
      (handler['client'].models.generateContent as any).mockRejectedValue(
        mockError,
      );
  ...
  describe('calculateCost', () => {
    ...
    it('should calculate cost correctly based on input and output tokens', () => {
      ...
      const expectedCost =
        (inputTokens / 1_000_000) * mockInfo.inputPrice! +
        (outputTokens / 1_000_000) * mockInfo.outputPrice!;
    ...
    it('should handle only input tokens', () => {
      ...
      expect(
        handler.calculateCost({ info: mockInfo, inputTokens, outputTokens: 0 }),
    ...
    it('should handle only output tokens', () => {
      ...
      expect(
        handler.calculateCost({ info: mockInfo, inputTokens: 0, outputTokens }),
      ).toBeCloseTo(expectedCost);
    });
    ...
    it('should calculate cost with cache read tokens', () => {
      ...
      const expectedInputCost =
        (uncachedReadTokens / 1_000_000) * mockInfo.inputPrice!;
      const expectedOutputCost =
        (outputTokens / 1_000_000) * mockInfo.outputPrice!;
      const expectedCacheReadCost =
        mockInfo.cacheReadsPrice! * (cacheReadTokens / 1_000_000);
      const expectedCost =
        expectedInputCost + expectedOutputCost + expectedCacheReadCost;
