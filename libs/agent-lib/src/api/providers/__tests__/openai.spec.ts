...
describe('OpenAiHandler', () => {
  ...
  describe('createMessage', () => {
    ...
    it('should handle tool calls in streaming responses', async () => {
      ...
      const toolCallPartialChunks = chunks.filter(
    ...
    it('should not include max_tokens when includeMaxTokens is false', async () => {
      ...
      const handlerWithoutMaxTokens = new OpenAiHandler(
        optionsWithoutMaxTokens,
      );
      const stream = handlerWithoutMaxTokens.createMessage(
        systemPrompt,
        messages,
      );
      // Consume the stream to trigger the API call
    ...
    it('should not include max_tokens when includeMaxTokens is undefined', async () => {
      ...
      const handlerWithDefaultMaxTokens = new OpenAiHandler(
        optionsWithUndefinedMaxTokens,
      );
      const stream = handlerWithDefaultMaxTokens.createMessage(
        systemPrompt,
        messages,
      );
      // Consume the stream to trigger the API call
    ...
    it('should use user-configured modelMaxTokens instead of model default maxTokens', async () => {
      ...
      const handlerWithUserMaxTokens = new OpenAiHandler(
        optionsWithUserMaxTokens,
      );
      const stream = handlerWithUserMaxTokens.createMessage(
        systemPrompt,
        messages,
      );
      // Consume the stream to trigger the API call
    ...
    it('should fallback to model default maxTokens when user modelMaxTokens is not set', async () => {
      ...
      const handlerWithoutUserMaxTokens = new OpenAiHandler(
        optionsWithoutUserMaxTokens,
      );
      const stream = handlerWithoutUserMaxTokens.createMessage(
        systemPrompt,
        messages,
      );
      // Consume the stream to trigger the API call
