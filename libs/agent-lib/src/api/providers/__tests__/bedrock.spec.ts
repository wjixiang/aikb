...
import { BEDROCK_1M_CONTEXT_MODEL_IDS } from '../../../types';
...
describe('AwsBedrockHandler', () => {
  ...
  describe('getModel', () => {
    ...
    it('should use custom ARN when provided', () => {
      ...
      const customArnHandler = new AwsBedrockHandler({
        ...
        awsRegion: 'us-east-1',
        awsCustomArn:
          'arn:aws:bedrock:us-east-1::inference-profile/custom-model',
      });
  ...
  describe('region mapping and cross-region inference', () => {
    describe('getPrefixForRegion', () => {
      it('should return correct prefix for US regions', () => {
        ...
        const getPrefixForRegion = (AwsBedrockHandler as any)
          .getPrefixForRegion;
    ...
    describe('isSystemInferenceProfile', () => {
      it('should return true for AWS inference profile prefixes', () => {
        const isSystemInferenceProfile = (AwsBedrockHandler as any)
    ...
    describe('parseBaseModelId', () => {
      it('should remove defined inference profile prefixes', () => {
        ...
        const parseBaseModelId = (handler as any).parseBaseModelId.bind(
          handler,
        );

        expect(
          parseBaseModelId('us.anthropic.claude-3-5-sonnet-20241022-v2:0'),
        ).toBe('anthropic.claude-3-5-sonnet-20241022-v2:0');
        expect(
          parseBaseModelId('eu.anthropic.claude-3-haiku-20240307-v1:0'),
        ).toBe('anthropic.claude-3-haiku-20240307-v1:0');
        expect(
          parseBaseModelId('apac.anthropic.claude-3-opus-20240229-v1:0'),
        ).toBe('anthropic.claude-3-opus-20240229-v1:0');
      });
      ...
      it('should not modify model IDs without defined prefixes', () => {
        ...
        expect(
          parseBaseModelId('anthropic.claude-3-5-sonnet-20241022-v2:0'),
        ).toBe('anthropic.claude-3-5-sonnet-20241022-v2:0');
        expect(parseBaseModelId('amazon.titan-text-express-v1')).toBe(
      ...
      it('should not modify model IDs with other prefixes', () => {
        ...
        expect(
          parseBaseModelId('ap.anthropic.claude-3-5-sonnet-20241022-v2:0'),
        ).toBe('ap.anthropic.claude-3-5-sonnet-20241022-v2:0');
        expect(
          parseBaseModelId('apne1.anthropic.claude-3-5-sonnet-20241022-v2:0'),
        ).toBe('apne1.anthropic.claude-3-5-sonnet-20241022-v2:0');
        expect(
          parseBaseModelId('use1.anthropic.claude-3-5-sonnet-20241022-v2:0'),
        ).toBe('use1.anthropic.claude-3-5-sonnet-20241022-v2:0');
      });
    });
    ...
    describe('cross-region inference integration', () => {
      ...
      it('should apply correct prefix for different regions', () => {
        ...
        expect(euHandler.getModel().id).toBe(
          'eu.anthropic.claude-3-5-sonnet-20241022-v2:0',
        );
        expect(apacHandler.getModel().id).toBe(
    ...
    describe('ARN parsing with inference profiles', () => {
      ...
      it('should detect cross-region inference for defined prefixes', () => {
        ...
        expect(euResult.modelId).toBe(
          'anthropic.claude-3-5-sonnet-20241022-v2:0',
        );

        expect(apacResult.crossRegionInference).toBe(true);
        expect(apacResult.modelId).toBe(
          'anthropic.claude-3-5-sonnet-20241022-v2:0',
        );
      });
  ...
  describe('1M context beta feature', () => {
    ...
    it('should include anthropic_beta parameter when 1M context is enabled', async () => {
      ...
      expect(commandArg.additionalModelRequestFields.anthropic_beta).toEqual([
        'context-1m-2025-08-07',
      ]);
      // Should not include anthropic_version since thinking is not enabled
      expect(
        commandArg.additionalModelRequestFields.anthropic_version,
      ).toBeUndefined();
    });
