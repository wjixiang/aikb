...
import { xaiDefaultModelId, xaiModels } from '../../../types';
...
describe('XAIHandler', () => {
  ...
  it('should not include reasoning_effort parameter for non-mini models', async () => {
    ...
    // Start generating a message
    const messageGenerator = regularModelHandler.createMessage(
      'test prompt',
      [],
    );
    await messageGenerator.next(); // Start the generator
  ...
  it('should handle errors in completePrompt', async () => {
    ...
    await expect(handler.completePrompt('test prompt')).rejects.toThrow(
      `xAI completion error: ${errorMessage}`,
    );
  });
  ...
  describe('Native Tool Calling', () => {
    ...
    it('should include tools in request when model supports native tools and tools are provided', async () => {
      ...
      const messageGenerator = handlerWithTools.createMessage(
        'test prompt',
        [],
        {
          taskId: 'test-task-id',
    ...
    it('should yield tool_call_end events when finish_reason is tool_calls', async () => {
      ...
      const { NativeToolCallParser } = await import('agent-lib');
      ...
      const endChunks = chunks.filter(
        (chunk) => chunk.type === 'tool_call_end',
      );
