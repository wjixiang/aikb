...
export class QwenCodeHandler
  ...
  private async loadCachedQwenCredentials(): Promise<QwenOAuthCredentials> {
    try {
      const keyFile = getQwenCachedCredentialPath(
        this.options.qwenCodeOauthPath,
      );
  ...
  private getBaseUrl(creds: QwenOAuthCredentials): string {
    let baseUrl =
      creds.resource_url || 'https://dashscope.aliyuncs.com/compatible-mode/v1';
  ...
  override getModel(): { id: string; info: ModelInfo } {
    ...
    const info =
      qwenCodeModels[id as keyof typeof qwenCodeModels] ||
      qwenCodeModels[qwenCodeDefaultModelId];
    return { id, info };
  }
