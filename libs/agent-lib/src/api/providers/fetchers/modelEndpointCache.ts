...
async function readModelEndpoints(
  key: string,
...
export const getModelEndpoints = async ({
  ...
  if (Object.keys(modelProviders).length > 0) {
    ...
    if (parentModel) {
      ...
      for (const endpointKey of Object.keys(modelProviders)) {
        modelProviders[endpointKey].supportsNativeTools =
          parentModel.supportsNativeTools;
        modelProviders[endpointKey].supportsReasoningEffort =
  ...
  if (Object.keys(modelProviders).length > 0) {
    ...
    try {
      ...
      console.error(
        `[getModelProviders] error writing ${key} endpoints to file cache`,
        error,
      );
    }
  ...
  try {
    ...
    console.error(
      `[getModelProviders] error reading ${key} endpoints from file cache`,
      error,
    );
  }
...
export const flushModelProviders = async (
  router: RouterName,
  modelId: string,
