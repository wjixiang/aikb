...
describe('OpenRouter API', () => {
  describe('getOpenRouterModels', () => {
    it('fetches models and validates schema', async () => {
      ...
      expect(models['google/gemini-2.5-flash-preview-05-20'].maxTokens).toEqual(
        65535,
      );
  ...
  describe('getOpenRouterModelEndpoints', () => {
    it('fetches model endpoints and validates schema', async () => {
      ...
      const getSpy = vi
        ...
        .mockResolvedValue(mockEndpointsResponse);

      const endpoints = await getOpenRouterModelEndpoints(
        'google/gemini-2.5-pro-preview',
      );
      ...
      if (parentModel) {
        for (const key of Object.keys(endpoints)) {
          ...
          endpoints[key].supportsReasoningEffort =
