...
describe('Ollama Fetcher', () => {
  ...
  describe('getOllamaModels', () => {
    it('should fetch model list from /api/tags and details for each model from /api/show', async () => {
        ...
        models: [
          {
            ...
            size: 14333928010,
            digest:
              '6a5f0c01d2c96c687d79e32fdd25b87087feb376bf9838f854d10be8cf3c10a5',
            details: {
              family: 'llama',
              families: ['llama'],
              format: 'gguf',
      ...
      const expectedParsedDetails = parseOllamaModel(
        mockApiShowResponse as any,
      );
    ...
    it('should log an info message and return an empty object on ECONNREFUSED', async () => {
      ...
      expect(consoleInfoSpy).toHaveBeenCalledWith(
        `Failed connecting to Ollama at ${baseUrl}`,
      );
      expect(result).toEqual({});
    ...
    it('should handle models with null families field in API response', async () => {
      ...
      expect(result[modelName].description).toBe(
        'Family: llama, Context: 4096, Size: 23.6B',
      );
    });
