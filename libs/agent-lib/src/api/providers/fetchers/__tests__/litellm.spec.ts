...
describe('getLiteLLMModels', () => {
  ...
  it('handles base URLs with query parameters correctly', async () => {
    ...
    await getLiteLLMModels(
      'test-api-key',
      'http://localhost:4000/litellm?key=value',
    );
  ...
  it('handles base URLs with fragments correctly', async () => {
    ...
    await getLiteLLMModels(
      'test-api-key',
      'http://localhost:4000/litellm#section',
    );
  ...
  it('throws error for unexpected response format', async () => {
    ...
    await expect(
      getLiteLLMModels('test-api-key', 'http://localhost:4000'),
    ).rejects.toThrow(
  ...
  it('throws detailed error for HTTP error responses', async () => {
    ...
    await expect(
      getLiteLLMModels(DUMMY_INVALID_KEY, 'http://localhost:4000'),
    ).rejects.toThrow(
  ...
  it('throws network error for request failures', async () => {
    ...
    await expect(
      getLiteLLMModels('test-api-key', 'http://invalid-url'),
    ).rejects.toThrow(
  ...
  it('throws generic error for other failures', async () => {
    ...
    await expect(
      getLiteLLMModels('test-api-key', 'http://localhost:4000'),
    ).rejects.toThrow('Failed to fetch LiteLLM models: Network timeout');
  });
