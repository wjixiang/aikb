...
import { type ModelInfo, chutesModels } from '../../../types';
...
export async function getChutesModels(
  ...
  try {
    ...
    if (parsed.success) {
      ...
      console.error(
        `Error parsing Chutes models: ${JSON.stringify(parsed.error.format(), null, 2)}`,
      );
    }
  } catch (error) {
    console.error(
      `Error fetching Chutes models: ${error instanceof Error ? error.message : String(error)}`,
    );
