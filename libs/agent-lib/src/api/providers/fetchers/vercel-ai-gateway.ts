...
} from '../../../types/providers/vercel-ai-gateway';
...
type VercelAiGatewayModelsResponse = z.infer<
  typeof vercelAiGatewayModelsResponseSchema
>;
...
export async function getVercelAiGatewayModels(
  ...
  try {
    const response = await axios.get<VercelAiGatewayModelsResponse>(
      `${baseURL}/models`,
    );
    ...
    if (!result.success) {
      console.error(
        `Vercel AI Gateway models response is invalid ${JSON.stringify(result.error.format())}`,
      );
    }
...
export const parseVercelAiGatewayModel = ({
  ...
  const cacheReadsPrice = model.pricing?.input_cache_read
    ? parseApiPrice(model.pricing?.input_cache_read)
    : undefined;

  const supportsPromptCache =
    typeof cacheWritesPrice !== 'undefined' &&
    typeof cacheReadsPrice !== 'undefined';
