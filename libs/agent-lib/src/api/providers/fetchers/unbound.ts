...
export async function getUnboundModels(
  ...
  try {
      ...
      for (const [modelId, model] of Object.entries(rawModels)) {
        const modelInfo: ModelInfo = {
          ...
          contextWindow: model?.contextWindow
            ? parseInt(model.contextWindow)
            : 0,
          supportsImages: model?.supportsImages ?? false,
          supportsPromptCache: model?.supportsPromptCaching ?? false,
          supportsNativeTools: true,
          inputPrice: model?.inputTokenPrice
            ? parseFloat(model.inputTokenPrice)
            : undefined,
          outputPrice: model?.outputTokenPrice
            ? parseFloat(model.outputTokenPrice)
            : undefined,
          cacheWritesPrice: model?.cacheWritePrice
            ? parseFloat(model.cacheWritePrice)
            : undefined,
          cacheReadsPrice: model?.cacheReadPrice
            ? parseFloat(model.cacheReadPrice)
            : undefined,
        };
    ...
    throw new Error(
      `Failed to fetch Unbound models: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
