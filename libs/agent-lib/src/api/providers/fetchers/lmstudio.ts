...
import { lMStudioDefaultModelInfo } from '../../../types/providers/lm-studio';
...
export const hasLoadedFullDetails = (modelId: string): boolean =>
  modelsWithLoadedDetails.has(modelId);
...
export async function getLMStudioModels(
  ...
  try {
    ...
    const loadedModels = (await client.llm
      .listLoaded()
      .then((models: LLM[]) => {
