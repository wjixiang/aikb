...
import { type ModelInfo, IO_INTELLIGENCE_CACHE_DURATION } from '../../../types';
...
type IOIntelligenceApiResponse = z.infer<
  typeof ioIntelligenceApiResponseSchema
>;
...
export async function getIOIntelligenceModels(
  ...
  try {
    ...
    if (!result.success) {
      console.error(
        'IO Intelligence models response validation failed:',
        result.error.format(),
      );
