...
import { ModelInfo, ollamaDefaultModelInfo } from '../../../types';
import { z } from 'zod';
...
export const parseOllamaModel = (
  ...
  const contextKey = Object.keys(rawModel.model_info).find((k) =>
    k.includes('context_length'),
  );
  const contextWindow =
    contextKey && typeof rawModel.model_info[contextKey] === 'number'
...
export async function getOllamaModels(
  ...
  try {
    ...
    if (parsedResponse.success) {
      ...
      console.error(
        `Error parsing Ollama models response: ${JSON.stringify(parsedResponse.error, null, 2)}`,
      );
    }
  } catch (error) {
    if (
