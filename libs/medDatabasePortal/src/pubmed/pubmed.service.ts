...
export class PubmedService {
  ...
  async searchByPattern(params: PubmedSearchParams) {
    ...
    // Get total results
    const totalResult = $('div.results-amount')
      .find('h3')
      .find('span.value')
      .text()
      .replace(',', '');
  ...
  buildUrl(params: PubmedSearchParams) {
      ...
      params.filter.forEach((e) => {
        if (e.match(/^\d{4}:\d{4}$/)) {
          ...
          switch (e.toLocaleLowerCase()) {
            ...
            default:
              throw new Error(
                `Parse Pubmed search filter failed: unsupported filter "${e}"`,
              );
          }
        }
      });
    ...
    urlParams.append('term', searchTerm);
    if (params.sort) {
  ...
  getTotalPages($: cheerio.CheerioAPI) {
    const totalPagesResult = $('label.of-total-pages')
      .first()
      .text()
      .replace('of ', '')
      .replace(',', '');
  ...
  getArticleProfileList($: cheerio.CheerioAPI): ArticleProfile[] {
    ...
    try {
      ...
      articleElements.each((index, element) => {
        ...
        const fullAuthors = $article('.docsum-authors.full-authors')
          .text()
          .trim();
        const shortAuthors = $article('.docsum-authors.short-authors')
          .text()
          .trim();
        const authors = fullAuthors || shortAuthors || '';

        // Extract journal citation (prefer full citation, fallback to short citation)
        const fullJournal = $article(
          '.docsum-journal-citation.full-journal-citation',
        )
          .text()
          .trim();
        const shortJournal = $article(
        ...
        const doiMatch = journalCitation.match(
          /10\.\d{4}\/[^\s]+(?<![.,;:!?)(\]\'])/,
        );
        let doi;
        if (!doiMatch) {
  ...
  async loadArticle(pmid: string): Promise<cheerio.CheerioAPI> {
    try {
      ...
      throw new Error(
        `Load article detail page request failed: ${error instanceof Error ? error.message : JSON.stringify(error)}`,
      );
    }
  }
  ...
  private extractAuthors($: cheerio.CheerioAPI): Author[] {
    ...
    $('div.authors-list')
      .find('span.authors-list-item')
      .each((index, element) => {
        const $author = $(element);
        const name =
          $author.find('a[data-ga-action="author"]').text().trim() ||
          $author.text().trim();
  ...
  private extractAbstract($: cheerio.CheerioAPI): string {
    const abstract =
      $('#abstract').find('.abstract-content').text().trim() ||
      $('.abstract').text().trim() ||
  ...
  private extractConflictOfInterestStatement($: cheerio.CheerioAPI): string {
    const statement =
      $('.conflict-of-interest').find('.statement').text().trim() ||
  ...
  private extractFullTextSources($: cheerio.CheerioAPI): FullTextSource[] {
    ...
    $('.full-text-links')
      ...
      .each((_, element) => {
        ...
        const type =
          $link.attr('data-ga-label') || $link.attr('data-source-type');
        if (url && name) {
  ...
  async getArticleDetail(pmid: string): Promise<ArticleDetail> {
    ...
    return {
      ...
      conflictOfInterestStatement:
        this.extractConflictOfInterestStatement($article),
