...
export const semantic_search_tool: Tool = {
    ...
    resolve: async function (args: any): Promise<ToolResponse> {
        ...
        const chunkEmbedGroupId = args['chunkEmbedGroupId'] || "9d8c8135-cce3-4802-93b5-9c91e3b74b0c";
        ...
        try {
            ...
            const formattedResults = results.map((item: SemanticSearchResult, index: number) =>
                `Result ${index + 1}:\n${item.content}`
            ).join('\n\n');
