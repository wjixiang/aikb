...
export class AuthenticationError extends ApiClientError {
  constructor(message: string, statusCode?: number) {
    super(message, 'AUTHENTICATION_ERROR', statusCode, false, [
...
export class ServiceUnavailableError extends ApiClientError {
  constructor(message: string, statusCode: number = 503) {
    super(message, 'SERVICE_UNAVAILABLE', statusCode, true, [
...
export class ContentPolicyError extends ApiClientError {
  constructor(message: string, statusCode: number = 400) {
    super(message, 'CONTENT_POLICY_VIOLATION', statusCode, false, [
...
export class ResponseParsingError extends ApiClientError {
  constructor(
    ...
    super(message, 'RESPONSE_PARSING_ERROR', undefined, false, [
...
export class UnknownApiError extends ApiClientError {
  constructor(
    ...
    super(message, 'UNKNOWN_ERROR', undefined, false, [
  ...
  override toJSON(): {
    ...
    retryable: boolean;
    recoverySuggestions: string[] | undefined;
    originalError?: string | { name: string; message: string; stack?: string };
  } {
    const json = super.toJSON() as {
      name: string;
      message: string;
      code: string;
      statusCode: number | undefined;
...
export function getErrorMessageWithSuggestions(error: unknown): string {
  ...
  if (parsed.recoverySuggestions && parsed.recoverySuggestions.length > 0) {
    ...
    message += parsed.recoverySuggestions
      .map((s, i) => `${i + 1}. ${s}`)
      .join('\n');
  }
