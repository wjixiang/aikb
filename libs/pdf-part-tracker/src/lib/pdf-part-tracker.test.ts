...
describe('PdfPartTracker', () => {
  ...
  it('should mark parts as completed', async () => {
    ...
    const result1 = await tracker.markPartCompleted(trackingId, 1, { content: 'Part 1 content' });
    ...
    const result2 = await tracker.markPartCompleted(trackingId, 2, { content: 'Part 2 content' });
    ...
    const result3 = await tracker.markPartCompleted(trackingId, 3, { content: 'Part 3 content' });
  ...
  it('should not mark the same part twice', async () => {
    ...
    const result2 = await tracker.markPartCompleted(trackingId, 1, { content: 'Part 1 content again' });
  ...
  it('should return completed parts data', async () => {
    ...
    await tracker.markPartCompleted(trackingId, 1, { content: 'Part 1 content' });
    await tracker.markPartCompleted(trackingId, 2, { content: 'Part 2 content' });
  ...
  it('should handle non-existent tracking IDs', async () => {
    ...
    await expect(tracker.getTrackingProgress('non-existent-id')).rejects.toThrow();
    await expect(tracker.getCompletedParts('non-existent-id')).rejects.toThrow();
  });
});
