...
export interface ChunkingConfig {
  ...
  strategy?: ChunkingStrategy; // 策略特定参数
}
 ...
 */
export const CHUNKING_STRATEGY_REGISTRY: Record<
  string,
...
export class ChunkingStrategyUtils {
   ...
   */
  static getStrategiesByCategory(
    category: ChunkingStrategyCategory,
  ): ChunkingStrategy[] {
  ...
  static fromString(strategyName: string): ChunkingStrategy {
    ...
    if (
      (Object.values(ChunkingStrategy) as ChunkingStrategy[]).includes(
        upperStrategy,
      )
    ) {
...
export class ChunkingStrategyCompatibility {
  ...
  static fromString(strategyName: string): ChunkingStrategy {
    ...
    const legacyValue = Object.values(ChunkingStrategyType).find(
      (v) => v === strategyName,
    );
    if (legacyValue) {
