 ...
 * Note: This file has dependencies on knowledgeBase types that are outside this package
 * For now, we'll define the types locally but they should be moved to a shared types package
 */

// Temporary type definitions - these should be moved to a shared types package
...
export class ChunkSearchUtils {
  ...
  private static calculateContentSimilarity(
    ...
    const countNum1 = parseInt(count1 || '0') || 0;
    const countNum2 = parseInt(count2 || '0') || 0;
  ...
  private static levenshteinDistance(str1: string, str2: string): number {
    ...
    for (let j = 1; j <= str2.length; j++) {
      for (let i = 1; i <= str1.length; i++) {
        ...
        if (matrix[j] && matrix[j]![i - 1] !== undefined && matrix[j - 1] && matrix[j - 1]![i] !== undefined) {
          matrix[j]![i] = Math.min(
