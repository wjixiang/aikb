// npx vitest run sllm-shared/__tests__/combineApiRequests.spec.ts

import type { ClineMessage, ClineSay } from 'llm-types';
...
describe('combineApiRequests', () => {
  ...
  const createStartMessage = (
    text: string = '{"request":"GET /api/data"}',
    ts: number = 1000,
  ): ClineMessage => ({
    type: 'say',
    say: 'api_req_started',
  ...
  describe('Bug verification', () => {
    it('should correctly replace api_req_started messages with combined ones', () => {
      ...
      const otherMessage = createOtherMessage();
      const startMessage1 = createStartMessage(
        '{"request":"GET /api/data1"}',
        1000,
      );
      const finishMessage1 = createFinishMessage('{"cost":0.005}', 1001);
      const startMessage2 = createStartMessage(
        '{"request":"GET /api/data2"}',
        2000,
      );
    ...
    it('should handle property overwrites correctly', () => {
      const startMessage = createStartMessage(
        '{"request":"GET /api/data", "cost": 0.001}',
        1000,
      );
      const finishMessage = createFinishMessage(
        '{"cost":0.005, "request": "OVERWRITTEN"}',
        1001,
      );
    ...
    it('should handle array values in JSON properly', () => {
      const startMessage = createStartMessage(
        '{"request":"GET /api/data", "tags": ["api", "get"]}',
        1000,
      );
      const finishMessage = createFinishMessage(
        '{"cost":0.005, "results": [1, 2, 3]}',
        1001,
      );
