// npx vitest run sllm-shared/__tests__/ProfileValidator.spec.ts

import type { ProviderSettings, OrganizationAllowList } from 'llm-types';
  ...
  describe('isProfileAllowed', () => {
    ...
    apiModelProviders.forEach((provider) => {
      it(`should extract apiModelId for ${provider} provider`, () => {
        ...
        expect(ProfileValidator.isProfileAllowed(profile, allowList)).toBe(
          true,
        );
      });
    });
