...
export function isToolAllowedForMode(
  ...
  for (const group of mode.groups) {
    ...
    const isCustomTool =
      groupConfig.customTools?.includes(tool) && includedTools?.includes(tool);
...
export async function getAllModesWithPrompts(
  ...
  return allModes.map((mode) => ({
    ...
    customInstructions:
      customModePrompts[mode.slug]?.customInstructions ??
...
export async function getFullModeDetails(
  ...
  const baseMode =
    getModeBySlug(modeSlug, customModes) ||
