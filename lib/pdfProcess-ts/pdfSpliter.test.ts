...
describe('PdfSpliterWorker', () => {
    ...
    describe('splitPdf', () => {
        ...
        it('should handle invalid page ranges', async () => {
            ...
            await expect(pdfSpliter.splitPdf(buffer, -1, 2)).rejects.toThrow('Invalid page range')
            await expect(pdfSpliter.splitPdf(buffer, 0, 10)).rejects.toThrow('Invalid page range')
            await expect(pdfSpliter.splitPdf(buffer, 3, 2)).rejects.toThrow('Invalid page range')
        })
    })
