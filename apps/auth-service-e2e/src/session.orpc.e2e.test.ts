...
describe('Session Management ORPC Endpoints', () => {
  ...
  describe('Session Lifecycle', () => {
    it('should handle session expiration correctly', async () => {
      ...
      expect(sessions.some(s => s.sessionToken === loginResponse.refreshToken)).toBe(true);
      ...
      expect(sessionsAfterLogout.some(s => s.sessionToken === loginResponse.refreshToken)).toBe(false);
    });
  });
});