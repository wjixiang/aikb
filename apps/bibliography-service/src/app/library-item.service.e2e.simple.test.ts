...
describe('LibraryItemService - Simple E2E Test', () => {
  ...
  it('should send message to RabbitMQ and receive it', async () => {
    ...
    const consumer1 = await channel.consume(uniqueServiceQueueName, (msg) => {
      if (msg && !messageReceived) {
        const parsedMessage = JSON.parse(msg.content.toString());
        receivedMessage = parsedMessage.data || parsedMessage;
        messageReceived = true;
        console.log('Message received in pdf_2_markdown_queue:', parsedMessage);
        channel.ack(msg);
      }
    }, { noAck: false });
    
    const consumer2 = await channel.consume(uniqueQueueName, (msg) => {
      if (msg && !messageReceived) {
        const parsedMessage = JSON.parse(msg.content.toString());
        receivedMessage = parsedMessage.data || parsedMessage;
        messageReceived = true;
