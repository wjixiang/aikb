...
export async function getQuizIdsByTags(
  ...
  // 添加调试日志
  console.log(
    `[DEBUG] getQuizIdsByTags: tags=${JSON.stringify(tagFilters)}, userId=${userId}, filterMode=${filterMode}`,
  );
  ...
  if (filterMode === 'AND') {
    ...
    if (hasTypeFilters) {
      ...
      filteredResults = results.filter((result) => {
        ...
        return tagFilters.every((tagFilter) => {
          const matchingTag = quizTagDoc.tags.find(
            (t: any) =>
    ...
    const quizIds = await db.collection('quiztags').distinct('quizId', query);
...
export async function getQuizIdsToExcludeByTags(
  ...
  // 添加调试日志
  console.log(
    `[DEBUG] getQuizIdsToExcludeByTags: tags=${JSON.stringify(tagFilters)}, userId=${userId}, filterMode=${filterMode}`,
  );
  ...
  if (filterMode === 'AND') {
    ...
    console.log(
      `[DEBUG] Exclude AND mode results: ${JSON.stringify(filteredResults)}`,
    );
    return filteredResults.map((r) => new ObjectId(r._id));
  } else {
