...
interface CustomQuizSliderProps {
  quizzesWithCombinedAnswers: QuizWithUserAnswer[];
  quizSetId?: string;
...
const CustomQuizSlider: React.FC<CustomQuizSliderProps> = ({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (
        newIndex !== currentQuizIndex &&
        newIndex >= 0 &&
        newIndex < quizzesWithCombinedAnswers.length
      ) {
    ...
    currentQuizIndex,
    setCurrentQuizIndex,
    quizzesWithCombinedAnswers.length,
    isTransitioning,
  ]);
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: `${quizzesWithCombinedAnswers.length * 100}%` }}
      >
