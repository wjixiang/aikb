    ...
    const sendResult = await processingService.processAndSendQuizzes(
      processResult.quizzes.slice(0, 5), // 只发送前5题作为示例
      {
        ...
        onProgress: (processed, total, current) => {
          console.log(
            `  进度: ${processed}/${total} (成功: ${current.successCount}, 失败: ${current.failureCount})`,
          );
        },
      },
    );
...
async function examPaperManagementExample() {
  ...
  try {
    ...
    if (papers.length > 0) {
      ...
      // 更新试卷
      console.log(`\n更新试卷信息...`);
      const updatedPaper = await examPaperService.updateExamPaper(
        firstPaper.id,
        {
...
async function runExamples() {
  ...
  console.log(
    '  npm run quiz:cli -- --use-service                    # 使用服务层模式',
  );
  console.log(
    '  npm run quiz:cli -- --use-service --chunks 3        # 使用服务层模式并分块处理',
  );
  console.log(
    '  npm run quiz:cli -- --retry-failed                  # 重试失败的题目',
  );
  console.log(
    '  npm run quiz:cli -- --check-failed                  # 检查失败题目数量',
  );
}
