  ...
  describe('CreateChunkEmbedGroup', () => {
    it('should create a chunk embedding group successfully', async () => {
      ...
      try {
        ...
        const response = await firstValueFrom(client.createChunkEmbedGroup(request));
        ...
        console.log('Group received for validation:', JSON.stringify(group, null, 2));
        ...
        console.log('Expected isActive:', request.isActive, 'Actual isActive:', group.isActive);
        console.log('Expected createdBy:', request.createdBy, 'Actual createdBy:', group.createdBy);
        ...
        expect(group.chunkingConfig!.strategy).toBe(request.chunkingConfig!.strategy);
        
        // Verify embedding config
        expect(group.embeddingConfig).toBeDefined();
        expect(group.embeddingConfig!.provider).toBe(request.embeddingConfig!.provider);
        expect(group.embeddingConfig!.model).toBe(request.embeddingConfig!.model);
        expect(group.embeddingConfig!.dimension).toBe(request.embeddingConfig!.dimension);
        ...
        console.log('LibraryItemVectorService may not be running:', error.message);
    ...
    it('should handle minimal request data', async () => {
      ...
      try {
        ...
        const response = await firstValueFrom(client.createChunkEmbedGroup(minimalRequest));
        
        console.log('Received minimal response:', JSON.stringify(response, null, 2));
