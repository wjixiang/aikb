...
export class TestGrpcClient {
  ...
  constructor() {
    // Load the proto file
    this.packageDefinition = protoLoader.loadSync('/workspace/protos/libraryItemVector.proto', {
      keepCase: true,
      longs: String,
      enums: String,
      defaults: true,
  ...
  createChunkEmbedGroup(
    ...
    return new Observable<libraryItemVectorProto.CreateChunkEmbedGroupResponse>((observer) => {
      this.client.createChunkEmbedGroup(request, (error: any, response: any) => {
        if (error) {
