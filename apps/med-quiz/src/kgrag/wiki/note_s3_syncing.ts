...
export class S3SyncService {
  ...
  constructor() {
    ...
    if (process.env.S3_ENDPOINT) {
      ...
      this.logger.debug(
        `Using custom S3 endpoint: {endpoint: ${process.env.S3_ENDPOINT}}`,
      );
    }
    ...
    if (process.env.S3_REGION) {
      ...
      this.logger.debug(
        `Using custom S3 region: {region: ${process.env.S3_REGION}}`,
      );
    } else {
    ...
    this.logger.debug(
      `Creating S3 client with config: ${JSON.stringify(s3Config)}`,
    );
    this.s3Client = new S3Client(s3Config);
    this.logger.debug(
      `Creating MongoDB client with URI: {uri: ${process.env.MONGODB_URI}}`,
    );
  }
  ...
  private async generateSyncPlan(config: NoteS3SyncConfig): Promise<SyncPlan> {
    this.logger.info(
      `Generating sync plan: {bucket: ${config.s3Bucket}, prefix: ${config.s3Prefix}, collection: ${config.mongoCollection}, direction: ${config.syncDirection}}`,
    );

    // Get files from S3
    const s3Files = await this.listS3Files(config.s3Bucket, config.s3Prefix);
    this.logger.debug(
      `Found S3 files: {count: ${s3Files.length}, sample: [${s3Files.slice(0, 5).join(', ')}]}`,
    );
    ...
    this.logger.debug(
      `Found MongoDB notes: {count: ${mongoNotes.length}, sample: [${mongoNotes
        .slice(0, 5)
        .map((n) => n.key)
        .join(', ')}]}`,
    );
    ...
    this.logger.info(
      `Detailed sync plan generated: {filesToDownload: ${plan.filesToDownload.length}, filesToUpload: ${plan.filesToUpload.length}, stats: ${JSON.stringify(plan.stats)}, syncDirection: ${config.syncDirection}, estimatedSize: ${(plan.filesToDownload.length * 100).toLocaleString()} KB (estimate)}`,
    );

    if (plan.filesToDownload.length > 0) {
      this.logger.debug(
        `Files to download details: {firstFile: ${plan.filesToDownload[0]}, lastFile: ${plan.filesToDownload[plan.filesToDownload.length - 1]}, sampleFiles: [${plan.filesToDownload.slice(0, 5).join(', ')}]}`,
      );
    }
  ...
  private async listS3Files(bucket: string, prefix: string): Promise<string[]> {
    this.logger.debug(
      `Listing S3 files: {bucket: ${bucket}, prefix: ${prefix}}`,
    );
    ...
    try {
      do {
        ...
        this.logger.debug(
          `S3 list batch: {bucket: ${bucket}, prefix: ${prefix}, batchSize: ${response.Contents?.length || 0}, totalSoFar: ${allFiles.length}, hasMore: ${response.IsTruncated}}`,
        );
      ...
      this.logger.debug(
        `S3 list complete: {bucket: ${bucket}, prefix: ${prefix}, totalFiles: ${allFiles.length}, mdFiles: ${mdFiles.length}, firstFile: ${mdFiles[0]}, sampleFiles: [${mdFiles.slice(0, 5).join(', ')}]}`,
      );

      return mdFiles;
    } catch (error) {
      this.logger.error(
        `Failed to list S3 files: {error: ${error}, bucket: ${bucket}, prefix: ${prefix}}`,
      );
      throw error;
    }
  }
  ...
  private async downloadFromS3(bucket: string, key: string): Promise<Buffer> {
    ...
    try {
      ...
      if (!body) {
        this.logger.error(
          `S3 response body is undefined: {bucket: ${bucket}, key: ${key}}`,
        );
      ...
      this.logger.error(
        `Failed to download from S3: {error: ${error}, bucket: ${bucket}, key: ${key}}`,
      );
      throw error;
    }
  }
  ...
  public async getFileMetadata(bucket: string, key: string) {
    this.logger.debug(
      `Getting file metadata: {bucket: ${bucket}, key: ${key}}`,
    );
    ...
    try {
      const response = await this.s3Client.send(command);
      this.logger.debug(
        `File metadata response: {bucket: ${bucket}, key: ${key}, etag: ${response.ETag}, lastModified: ${response.LastModified}, size: ${response.ContentLength}}`,
      );
      return {
        etag: response.ETag,
      ...
      this.logger.error(
        `Failed to get file metadata: {error: ${error}, bucket: ${bucket}, key: ${key}}`,
      );
      throw error;
    }
  }
  ...
  public async sync(config: NoteS3SyncConfig) {
    this.logger.info(
      `Starting sync: {direction: ${config.syncDirection}, bucket: ${config.s3Bucket}, prefix: ${config.s3Prefix}, collection: ${config.mongoCollection}}`,
    );

    if (config.syncDirection !== 'download-only') {
      this.logger.error(
        `Unsupported sync direction: {direction: ${config.syncDirection}}`,
      );
