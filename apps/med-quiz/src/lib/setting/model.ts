...
export function sanitizeSettings(settings: CopilotSettings): CopilotSettings {
  ...
  if (!settingsToSanitize.activeEmbeddingModels) {
    settingsToSanitize.activeEmbeddingModels = BUILTIN_EMBEDDING_MODELS.map(
      (model) => ({
        ...model,
        enabled: true,
      }),
    ...
    settingsToSanitize.activeEmbeddingModels =
      settingsToSanitize.activeEmbeddingModels.map((m) => {
        return {
          ...m,
          provider:
  ...
  const embeddingRequestsPerMin = Number(
    settingsToSanitize.embeddingRequestsPerMin,
  );
