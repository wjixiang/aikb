...
// 角色权限验证的响应函数
export function requireRole(session: any, requiredRole: string): NextResponse | null {
  const authError = requireAuth(session);
  if (authError) return authError;
  
  if (!hasRole(session, requiredRole)) {
    return NextResponse.json({ error: `Role '${requiredRole}' required` }, { status: 403 });
  }
...
// 权限验证的响应函数
export function requirePermission(session: any, permission: string): NextResponse | null {
  const authError = requireAuth(session);
  if (authError) return authError;
  
  if (!hasPermission(session, permission)) {
    return NextResponse.json({ error: `Permission '${permission}' required` }, { status: 403 });
  }
