...
export class QuizParser {
  ...
  constructor(questionsText: string, answersText: string, concurrencyLimit?: number) {
   ...
   * @param config Default values for quiz metadata fields
   */
  async parse(config?: Partial<QuizType.quiz>, withExplanation: boolean = false): Promise<QuizWithoutId[]> {
   ...
   */
  private createOptions(optionTexts: string[]): { oid: QuizType.oid; text: string }[] {
   ...
   */
  private normalizeAnswer(answer: string, options: { oid: QuizType.oid }[]): QuizType.oid | QuizType.oid[] {
  ...
  async chunkInput(chunkNum: number): Promise<QuizParser[]> {
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const answersText = chunk.answer.map((value, index)=>{
          return `${index+1}. ${value}`
        }).join('\n')
        
        // Create a new QuizParser instance for this chunk
        const chunkedParser = new QuizParser(questionsText, answersText, this.concurrencyLimit);
