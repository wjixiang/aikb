...
type EventHandler<T extends EventTypes.KnowledgeEvent> = EventBusInterfaces.EventHandler<T>;
...
export class EventBusService implements IEventBus {
  ...
  async unsubscribe(subscriptionId: string): Promise<void> {
    ...
    if (subscription) {
      ...
      const eventHandlers = this.handlersByEventType.get(subscription.eventType);
      if (eventHandlers) {
