...
const prismaPluginFunc = async (fastify: any, options: PrismaPluginOptions): Promise<void> => {
  ...
  fastify.log.info({ databaseUrl: databaseUrl.replace(/:[^:@]+@/, ':****@') }, 'Prisma connected');
...
export const prismaPlugin = fp(prismaPluginFunc) as unknown as FastifyPluginAsync<PrismaPluginOptions>;
