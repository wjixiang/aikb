...
class LocalEntityStorage extends AbstractEntityStorage {
  ...
  private async writeEntities(entities: entity[]): Promise<void> {
    try {
      await fs.promises.writeFile(this.entitiesFile, JSON.stringify(entities, null, 2));
    } catch (error) {
  ...
  async create_new_entity(entity: entity): Promise<entity> {
    try {
      ...
      const existingEntity = entities.find(e => this.generateEntityId(e.name) === entityId);
      if (existingEntity) {
  ...
  async get_entity_by_name(name: string[]): Promise<entity | null> {
    try {
      ...
      const entity = entities.find(e => this.generateEntityId(e.name) === entityId);
  ...
  async update_entity(entity: entity): Promise<entity> {
    try {
      ...
      const index = entities.findIndex(e => this.generateEntityId(e.name) === entityId);
      if (index === -1) {
  ...
  async delete_entity(name: string[]): Promise<boolean> {
    try {
      ...
      const filteredEntities = entities.filter(e => this.generateEntityId(e.name) !== entityId);
  ...
  async search_entities(query: string): Promise<entity[]> {
    try {
      ...
      this.logger.info(`Found ${results.length} entities matching query: ${query}`);
      return results;
    } catch (error) {
...
export type { entity };
export { AbstractEntityStorage, LocalEntityStorage };