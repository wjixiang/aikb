...
import { AbstractEntityContentStorage, AbstractEntityGraphStorage, AbstractEntityVectorStorage, AbstractEntityStorage } from "./storage/abstract-storage";
...
describe(Entity, ()=>{
    const test_entity_data: EntityData ={
        name: ["Hypertension"],
        tags: ["disease"],
        definition: "Hypertension is the medical term for high blood pressure. It is a chronic condition where the force of blood pushing against the walls of your arteries is consistently too high."
    }
    ...
    const createMockEntityStorage = () => {
        ...
        const mockContentStorage = {
            create_new_entity_content: vi.fn().mockImplementation((entity: EntityData, id: string) => {
    ...
    it("create new entity with elastic and direct data input", async()=>{
        ...
        const creat_res = await Entity.create_entity_with_entity_data(test_entity_data).save(mockStorage)
    ...
    it.skip("use real elasticsearch content storage to create&save entity", async()=>{
        const storage = new EntityStorage(
            new ElasticsearchEntityContentStorage(process.env.ELASTICSEARCH_URL || 'http://localhost:9200'),
            {} as unknown as AbstractEntityGraphStorage,
            {} as unknown as AbstractEntityVectorStorage
        );

        const creat_res = await Entity.create_entity_with_entity_data(test_entity_data).save(storage)
    }, 30000)
