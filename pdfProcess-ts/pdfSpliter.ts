...
export class PdfSpliterWorker {
    async getPdfSize(existingPdfBytes: Buffer): Promise<{ height: number; width: number }> {
    ...
    async splitPdf(existingPdfBytes: Buffer, startPage: number, endPage: number): Promise<Uint8Array> {
        try {
            logger.info(`Starting PDF split: startPage=${startPage}, endPage=${endPage}`)
            ...
            if (startPage < 0 || endPage >= totalPages || startPage > endPage) {
                throw new Error(`Invalid page range: startPage=${startPage}, endPage=${endPage}, totalPages=${totalPages}`)
            }
            ...
            logger.info(`Copying pages: startPage=${startPage}, endPage=${endPage}, count=${pagesToCopy}`)
            
            const copiedPages = await newPdfDoc.copyPages(pdfDoc, Array.from({ length: pagesToCopy }, (_, i) => startPage + i))
    ...
    async splitPdfIntoChunks(existingPdfBytes: Buffer, chunkSize: number = 10): Promise<Uint8Array[]> {
        try {
            ...
            for (let i = 0; i < numChunks; i++) {
                ...
                logger.info(`Creating chunk ${i + 1}/${numChunks}: pages ${startPage}-${endPage}`)
                
                const chunkBytes = await this.splitPdf(existingPdfBytes, startPage, endPage)
