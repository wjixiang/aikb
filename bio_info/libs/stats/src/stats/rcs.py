...
def load_data(
    ...
    exposure_lf = scan_table(exposure_table).with_columns(
...
def rcs_predict_single(
    ...
    """拟合单个 exposure 的 RCS Logistic 模型，生成预测曲线和分位数参考表。"""
    ...
    try:
        ...
        for i, q in enumerate(quantiles):
            ...
            ci_lo = float(
                np.exp(pred_logit_q_centered[i] - 1.96 * se_q_centered[i])
            )
            ci_hi = float(
                np.exp(pred_logit_q_centered[i] + 1.96 * se_q_centered[i])
            )
